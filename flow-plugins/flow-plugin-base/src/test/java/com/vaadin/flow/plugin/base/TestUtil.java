...
public class TestUtil {
    public static void stubPluginAdapterBase(PluginAdapterBase adapter, File baseDir) throws URISyntaxException {
        ...
        Mockito.when(adapter.projectBaseDirectory()).thenReturn(baseDir.toPath());
        ...
        Mockito.when(adapter.javaSourceFolder()).thenReturn(new File(baseDir, "src/main/java"));
        Mockito.when(adapter.javaResourceFolder()).thenReturn(new File(baseDir, "src/main/resources"));
        ...
        Mockito.when(adapter.getClassFinder())
                .thenReturn(new ClassFinder.DefaultClassFinder(TestUtil.class.getClassLoader()));
    }
}

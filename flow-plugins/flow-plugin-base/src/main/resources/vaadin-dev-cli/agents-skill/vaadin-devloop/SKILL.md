...
That holds for a **WAR project** too, and there the daemon runs the project's own
`jetty:run` for you — a WAR has no main class, and its servlet container is a build plugin
rather than a dependency, so the build is the only thing that knows how to start it. `status`
names the runtime it chose (`runtime=jetty-ee10`). Nothing in the pom needs changing: the
daemon turns the plugin's own rescanner off for its run, because two things redeploying the
app on separate schedules is what the loop exists to prevent, and it says so in the app log.

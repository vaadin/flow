...
public class GenerateNpmBOMMojo extends FlowModeAbstractMojo {
    ...
    /**
     * Whether to ignore errors of NPM. This might be used, if "npm install" was run with "--force" or
     * "--legacy-peer-deps".
     */
     ...
     * details in and the tree described by the "npm-shrinkwrap.json" or "package-lock.json", rather than the contents
    ...
    /**
     * Dependency types to omit from the installation tree. (can be set multiple times) (choices: "dev", "optional",
     * "peer", default: "dev" if the NODE_ENV environment variable is set to "production", otherwise empty)
     */
    @Parameter(defaultValue = "dev")
    private String omit;

    /**
     * Whether to flatten the components. This means the actual nesting of node packages is not represented in the SBOM
     * result.
     */
    @Parameter(defaultValue = "false")
    ...
    /**
     * Omit all qualifiers from PackageURLs. This causes information loss in trade-off shorter PURLs, which might
     * improve ingesting these strings.
     */
    @Parameter(defaultValue = "false")
    private boolean shortPURLs;

    /**
     * Whether to go the extra mile and make the output reproducible. This requires more resources, and might result in
     * loss of time- and random-based-values.
     */
    @Parameter(defaultValue = "false")
    ...
    /**
     * Type of the main component. (choices: "application", "firmware", "library")
     */
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            ...
            if (!nodeModulesDir.exists()) {
                ...
                File jarFrontendResourcesFolder = new File(new File(frontendDirectory(), FrontendUtils.GENERATED),
                        FrontendUtils.JAR_RESOURCES_FOLDER);
                Options options = new Options(lookup, npmFolder()).withFrontendDirectory(frontendDirectory())
                        .withBuildDirectory(buildFolder()).withJarFrontendResourcesFolder(jarFrontendResourcesFolder)
                        .createMissingPackageJson(true).enableImportsUpdate(true).enablePackagesUpdate(true)
                        .withRunNpmInstall(true).withFrontendGeneratedFolder(generatedTsFolder())
                        .withNodeVersion(nodeVersion()).withNodeDownloadRoot(nodeDownloadRoot())
                        .setNodeAutoUpdate(nodeAutoUpdate()).withHomeNodeExecRequired(requireHomeNodeExec())
                        .setJavaResourceFolder(javaResourceFolder()).withProductionMode(productionMode)
            ...
            if (result.getExitCode() != 0) {
                throw new MojoFailureException("Frontend SBOM generation failed.", result.getExecutionException());
            }
    ...
    private Properties getProperties() {
        ...
        properties.setProperty("exec.args", "@cyclonedx/cyclonedx-npm" + (ignoreNpmErrors ? " --ignore-npm-errors" : "")
                + (packageLockOnly ? " --package-lock-only" : "") + (flattenComponents ? " --flatten-components" : "")
                + (shortPURLs ? " --short-PURLs" : "") + (outputReproducible ? " --output-reproducible" : "")
                + (validate ? " --validate" : " --no-validate") + " --mc-type " + mcType + " --omit " + omit
                + " --spec-version " + specVersion + " --output-file " + outputFilePath + " --output-format "
                + outputFormat + " -- " + packageManifest);
        return properties;
    }

    private boolean createDirectoryIfNotExists() {
        int lastIndex = outputFilePath.lastIndexOf(FrontendUtils.isWindows() ? '\\' : '/');

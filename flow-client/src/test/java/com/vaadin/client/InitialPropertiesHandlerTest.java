...
public class InitialPropertiesHandlerTest {
    ...
    public void flushPropertyUpdates_updateIsNotInProgress_collectInitialProperties() {
        ...
        property1.setValue("updated");
        MapProperty property3 = properties.getProperty("client");
        property3.setValue("baz");

        handler.handlePropertyUpdate(property1);
        handler.handlePropertyUpdate(property3);
        ...
        Mockito.verify(tree, Mockito.times(0))
                .sendNodePropertySyncToServer(property1);
        Mockito.verify(tree, Mockito.times(0))
                .sendNodePropertySyncToServer(property2);
        Mockito.verify(tree).sendNodePropertySyncToServer(property3);
    }
}

...
export class Binder<T, M extends AbstractModel<T>> extends BinderNode<T, M> {
  ...
  constructor(
    ...
    this[emptyValueSymbol] = (this.model[parentSymbol] as ValueRoot<T>).value;
  ...
  private *getPendingValidations(): IterableIterator<Promise<ValueError<any> | void>> {
    for (const [, validations] of this[validationsSymbol]) {
      for (const [, promise] of validations) {
        yield promise;
      }
    }
  }

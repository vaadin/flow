...
public class SimpleElementBindingStrategy implements BindingStrategy<Element> {
    ...
    private void handleDomEvent(Event event, Node element, StateNode node) {
        ...
        /*-
         * -1: filter encountered, but non passed
         *  0: no filter encountered
         *  1: at least one passing filter encountered
         */
        int filterStatus = 0;
        ...
        if (expressions.length == 0) {
            ...
            for (String expressionString : expressions) {
                ...
                // Update filter status if not yet positive and this expression
                // is used as a filter
                if (filterStatus != 1
                        && expressionSettings.getBoolean(expressionString)) {
                    boolean filterMatched = expressionValue.asBoolean();
                    filterStatus = filterMatched ? 1 : -1;
                }
            }
        }

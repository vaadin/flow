...
public class SimpleElementBindingStrategy implements BindingStrategy<Element> {
    ...
    /*-{
        ...
        var callbacks = {
            handlePropertiesChanged: $entry(function(changedProps) {
                self.@SimpleElementBindingStrategy::handlePropertiesChanged(*)(changedProps, node);
            }),
            fireReadyEvent: $entry(function(elem) {
                @com.vaadin.client.PolymerUtils::fireReadyEvent(*)(elem);
            }),
            handleListItemPropertyChange: $entry(function(nodeId, host, propertyName, value) {
                @SimpleElementBindingStrategy::handleListItemPropertyChange(*)(nodeId, host, propertyName, value, tree);
            })
        };
        $wnd.Vaadin.Flow.internal.SimpleElementBindingStrategy.bindPolymerModelProperties(element, callbacks);
    }-*/;

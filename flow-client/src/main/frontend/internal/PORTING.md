...
| 13.9 | `ClientJsonCodecTests` names `GwtClientJsonCodecTest` in its header but cites none of that class's 13 cases individually, mapping them by paraphrase instead (`decodeWithTypeInfo_primitives` → *"passes primitives through unchanged"*). The reverse-direction count of `PORTING-REVIEW.md` §1 therefore cannot be run against it: every case reads as uncited. Needs one `// Ported from` line per case, which means re-deriving each mapping | the branch that next revisits the codec suite | open |

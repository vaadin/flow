...
| 13.1 | `ExecuteJavaScriptElementUtilsTests` ports 2 of `GwtExecuteJavaScriptElementUtilsTest`'s 5 `attachExistingElement` cases; `_elementHasServersideCounterpart`, `_elementIsAlreadyAssociated`, `_noRequestedElement` and `testReturnChannel_passedToExecJavaScript_messageSentToServer` have no `it()` | a follow-up on the support-services layer | open |
| 5 | `ResourceLoader`, `SystemErrorHandler`, `ExecuteJavaScriptProcessor` and `UILifecycle` declare their members in a different order from the Java classes. Their public members interleave with private helpers, so the reorder has to move both together rather than just the exported ones | a follow-up on the support-services layer | open |

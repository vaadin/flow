//#region src/checkers/tscUtils.ts
/**
...
function forceNoEmitOnSolutionBuilderHost(ts, host) {
	...
	host.getParsedCommandLine = (fileName) => {
		const parsed = original ? original(fileName) : ts.getParsedCommandLineOfConfigFile(fileName, { tscBuild: true }, parseConfigHost);
		if (parsed && parsed.errors.length > 0) parsed.options.noEmit = true;
		return parsed;
	};
	return host;
}

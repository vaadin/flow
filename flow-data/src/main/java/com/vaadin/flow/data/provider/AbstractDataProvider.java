...
public abstract class AbstractDataProvider<T, F> implements DataProvider<T, F> {
    ...
    /**
     * Registers a new listener with the specified activation method to listen events generated by this component. If
     * the activation method does not have any arguments the event object will not be passed to it when it's called.
     *
     * @param eventType
     *            the type of the listened event. Events of this type or its subclasses activate the listener.
     * @param method
    ...
    protected <E> Registration addListener(Class<E> eventType, SerializableConsumer<E> method) {
        List<DataListenerWrapper> list = listeners.computeIfAbsent(eventType, key -> new ArrayList<>());

        DataListenerWrapper wrapper = new DataListenerWrapper(method);

        final Registration registration = Registration.addAndRemove(list, wrapper);
    ...
    protected void fireEvent(EventObject event) {
        listeners.entrySet().stream().filter(entry -> entry.getKey().isAssignableFrom(event.getClass())).forEach(
                entry -> new ArrayList<>(entry.getValue()).forEach(wrapper -> fireEventForListener(event, wrapper)));
    ...
    private void fireEventForListener(EventObject event, DataListenerWrapper wrapper) {
        if (event instanceof DataChangeEvent<?>) {
            ...
            dataEvent.setUnregisterListenerCommand(wrapper.registration::remove);

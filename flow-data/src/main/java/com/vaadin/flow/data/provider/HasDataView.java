...
/**
 * An interface for components that get items from the generic data provider types {@link DataProvider} and
 * {@link InMemoryDataProvider}. The methods return a {@link DataView} which has the generic API for getting information
 * on the items.
 *
 * @param <T>
 *            data type
...
public interface HasDataView<T, F, V extends DataView<T>> extends Serializable {
    ...
    /**
     * Set a generic data provider for the component to use and returns the base {@link DataView} that provides API to
     ...
     * {@link ListDataView} API by using {@link HasListDataView#setItems(ListDataProvider)}.
     *
     * @param dataProvider
     ...
     * <p>
     * The returned DataView only contains a minimal common API. Use of {@link HasListDataView#getListDataView} or
     * {@link HasLazyDataView#getLazyDataView} should be used for more targeted helper features
     *

...
public class BeanPropertySetTest {
    ...
    private void comparePropertySet(PropertySet<?> propertySetA, PropertySet<?> propertySetB, String message) {
        ...
        PropertyDefinition<?, ?>[] propertiesA = propertySetA.getProperties()
                .sorted(Comparator.comparing(PropertyDefinition::getName)).toArray(PropertyDefinition<?, ?>[]::new);
    ...
    public void testSerializeDeserialize_propertySet_cacheCleared() throws Exception {
        ...
        Assert.assertNotSame("Deserialized instance should not be the same as the original", originalPropertySet,
                deserializedPropertySet);
    }

    @Test
    public void testSerializeDeserialize_propertyDefinition() throws Exception {
        PropertyDefinition<Person, ?> definition = BeanPropertySet.get(Person.class).getProperty("born")
        ...
        Assert.assertSame("Deserialized instance should be the same as in the cache",
                BeanPropertySet.get(Person.class).getProperty("born").orElseThrow(RuntimeException::new),
    ...
    public void testSerializeDeserialize_nestedPropertyDefinition() throws Exception {
        PropertyDefinition<com.vaadin.flow.tests.data.bean.Person, ?> definition = BeanPropertySet
                .get(com.vaadin.flow.tests.data.bean.Person.class, true, PropertyFilterDefinition.getDefaultFilter())
        ...
        com.vaadin.flow.tests.data.bean.Person person = new com.vaadin.flow.tests.data.bean.Person("Jon", "Doe",
                "jon.doe@vaadin.com", 32, Sex.MALE, address);
    ...
    public void testSerializeDeserializeRecord() throws Exception {
        PropertyDefinition<TestRecord, ?> definition = BeanPropertySet.get(TestRecord.class).getProperty("name")
                .orElseThrow(AssertionFailedError::new);

        PropertyDefinition<TestRecord, ?> deserializedDefinition = ClassesSerializableUtils
                .serializeAndDeserialize(definition);

        ValueProvider<TestRecord, ?> getter = deserializedDefinition.getGetter();

        TestRecord testRecord = new TestRecord("someone", 42);

        String name = (String) getter.apply(testRecord);

        Assert.assertEquals("Deserialized definition should be functional", "someone", name);

        PropertyDescriptor namePropertyDescriptor = BeanUtil.getPropertyDescriptor(TestRecord.class, "name");
        Assert.assertNotNull(namePropertyDescriptor);
        Assert.assertEquals("Property has unexpected name", namePropertyDescriptor.getName(), "name");
        Assert.assertEquals("Property read method has unexpected name",
                namePropertyDescriptor.getReadMethod().getName(), "name");

        Class<?> namePropertyType = BeanUtil.getPropertyType(TestRecord.class, "name");
        Assert.assertEquals("Property type is unexpected", namePropertyType, String.class);

        // Ensure props for Record are not sorted, but are in code order
        List<PropertyDefinition<TestRecord, ?>> propertyList = definition.getPropertySet().getProperties().toList();
    ...
    public void nestedPropertyDefinition_samePropertyNameOnMultipleLevels() {
        PropertyDefinition<FatherAndSon, ?> definition = BeanPropertySet.get(FatherAndSon.class)
                .getProperty("father.father.firstName").orElseThrow(RuntimeException::new);
    ...
    public void nestedPropertyDefinition_propertyChainBroken() {
        PropertyDefinition<FatherAndSon, ?> definition = BeanPropertySet.get(FatherAndSon.class)
                .getProperty("father.firstName").orElseThrow(RuntimeException::new);
    ...
    public void properties() {
        ...
        Set<String> propertyNames = propertySet.getProperties().map(PropertyDefinition::getName)
    ...
    public void isSubProperty() {
        PropertySet<FatherAndSon> propertySet = BeanPropertySet.get(FatherAndSon.class);

        Assert.assertTrue("Dot-separated property chain \"father.firstName\" should refer to a sub-property",
                propertySet.getProperty("father.firstName").get().isSubProperty());
        Assert.assertFalse("Property name without dot-separated parent properties should not refer to a sub-property",
                propertySet.getProperty("father").get().isSubProperty());
        Assert.assertTrue("Dot-separated property chain \"father.son.father.son\" should refer to a sub-property",
                propertySet.getProperty("father.son.father.son").get().isSubProperty());
    }
    ...
    public void getFullName_returnsFullPropertyChain() {
        ...
        PropertyDefinition<FatherAndSon, ?> subProperty = propertySet.getProperty(subPropertyFullName).get();
        Assert.assertEquals("Name of a sub-property should be the simple name of the property", "firstName",
                subProperty.getTopLevelName());
        Assert.assertEquals("Full name of a sub-property should be the full property chain with parent properties",
    ...
    public void getParentForDirectProperty_returnsNull() {
        PropertySet<FatherAndSon> propertySet = BeanPropertySet.get(FatherAndSon.class);
        Assert.assertNull("Direct property of a property set should not have a parent",
    ...
    public void getParentForSubProperty_returnsParent() {
        PropertySet<FatherAndSon> propertySet = BeanPropertySet.get(FatherAndSon.class);
        Assert.assertEquals("Parent property of \"father.son.father\" should be \"father.son\"", "father.son",
                propertySet.getProperty("father.son.father").get().getParent().getName());
    }

    @Test
    public void get_beanImplementsSameInterfaceSeveralTimes_interfacePropertyIsNotDuplicated() {
        PropertySet<Iface3> set = BeanPropertySet.get(Iface3.class, false, PropertyFilterDefinition.getDefaultFilter());

        List<PropertyDefinition<Iface3, ?>> defs = set.getProperties().collect(Collectors.toList());
    ...
    public void get_beanImplementsInterfaceWithDefaultMethod_propertyFound() {
        PropertySet<DefaultMethodIface> set = BeanPropertySet.get(DefaultMethodIface.class);

        List<PropertyDefinition<DefaultMethodIface, ?>> defs = set.getProperties().collect(Collectors.toList());
    ...
    public void get_beanImplementsGenericInterfaceSubclassWithDefaultMethod_interfacePropertyIsNotDuplicated() {
        PropertySet<DefaultMethodSubclassIface> set = BeanPropertySet.get(DefaultMethodSubclassIface.class);

        List<PropertyDefinition<DefaultMethodSubclassIface, ?>> defs = set.getProperties().collect(Collectors.toList());

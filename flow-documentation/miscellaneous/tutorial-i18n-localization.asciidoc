...
as a Servlet init parameter in the `web.xml`, using the `@WebServlet` annotation or
adding the property to `application.properties`.
...
Using `application.properties`, the properties file should be located in `src/main/resources` with the content:

.src/main/resources/application.properties
[source,text]
----
i18n.provider=com.vaadin.example.ui.TranslationProvider

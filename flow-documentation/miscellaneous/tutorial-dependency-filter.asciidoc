...
The DependencyFilters are called in two particular situations: when a <<../polymer-templates/tutorial-include-css#,PolymerTemplate>> is parsed for the first time, and when a set of dependencies are about to be sent to the client.

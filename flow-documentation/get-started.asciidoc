...
happens, the constructor initializes the UI structure. In this case, the main view
...
UI components as you wish. It would be equally easy to build your UI completely
...
parameterized with a [classname]#Model# that describes all properties that are
...
`VaadinIcons.CLOSE_CIRCLE` here, which most users will recognize as a functionality
...
are calling the `Binder.setBean` method. This will initialize all fields in the

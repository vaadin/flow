...
In some cases you may want to execute some client side logic after component
is updated from the server during a roundtrip.
E.g. component constructor is called to create a component on the client side but
...
for the component it will be called each time after the components is updated 

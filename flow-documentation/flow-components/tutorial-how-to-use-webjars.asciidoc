...
* Artifact will be present in the repository, but there would be a conflict between
...
Currently, the more webjars are added as a dependencies, the more probability that dependency resolution will go wrong.
...
For Maven this can be achieved with `mvn dependency:tree` command.
...
Currently no better webjars' analog was found, but Flow eases the pain having predefined
bom and dependency with all Flow components' webjars resolved.

...
You can set strings describing backend properties to be used when sorting the column. Different from the `addColumn`, 
...
so you can toggle the `sortable` flag on and off without having to reconfigured it every time.

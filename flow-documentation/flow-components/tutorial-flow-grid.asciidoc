...
Columns can be configured to use Renderers to show the data in a more suitable way inside the cells. The renderers are separated in three categories:
...
There are several basic renderers that can be used to configure Grid columns:
...
An easy way to create a clickable button inside the grid cells. It creates a native `<button>` in the client side, and the click and tap events (for touch devices) are treated in the server side.

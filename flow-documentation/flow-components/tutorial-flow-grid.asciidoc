...
You can define how a column is sorted in two different ways:

==== 1. Using a property name at the column construction

You can set the names of properties that will be used to sort the column at the moment you add it to the grid. For example:
...
The `Age` column will use the `age` property of the bean to sort the values.
...
When using multiple properties, the comparisons are made in the order they are declared.

You can use properties created for your `TemplateRenderer` as well. For example:
...
[NOTE]
The sortable properties declared at the `addColumn` method must implement `Comparable` for the automatic sorting to work.

==== 2. Using a Comparator

When your model is complex enough, or when you need a custom logic to compare items to sort them properly, 
...
[NOTE]
Setting a `Comparator` will override any sortable property set at the `addColumn` method.

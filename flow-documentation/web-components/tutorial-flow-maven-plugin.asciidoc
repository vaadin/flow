...
The intention of the goal is to process application web files in order to optimize them for production usage: minify them, transpile to ES5, if required and bunbdle them (squash all frontend files into bigger files to improve their loading time).

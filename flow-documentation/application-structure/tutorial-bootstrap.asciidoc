...
----
There is another common case that you have html contents to be appended to your page, for an example you will have a `favicon`. There are a few ways of doing this:

 - using `InitialPageSettings#addLink()`

[source,java]
----
public class Layout extends Div implements RouterLayout, PageConfigurator {
...
But most commonly, `favicon` contains quite a lot small icons, in this case, you can see that you will easily do a lot of hard coding. To avoid this, we are highly suggest you to move all the html contents into a file, (e.g. `favicon.html`) and inline this file in your `PageConfigurator`
[source,java]
----
...
Inline is highly recommended, when you have a lot of html contents to be appended to your inital page. In order to avoid long coding for modifying the page, we suggest you to move all the HTMLs to another file and import it via using `@Inline` annotation. 

For an example: it is very common that you will have a lot of small icons for your `favicon`. Instead of adding the tags one by one by using `InitialPageSettings#addLink()` and `BootstrapListener#modifyBootstrapPage()`, you could move all the icons' `rel`, `href` settings to another file (e.g. `favicon.html`) and inline the `favicon.html` to your `pageConfigurator` (code example is shown below). 
Following code example is showing to using inline in `PageConfigurator`:

[source,java]
----
@Route(value = "", layout = Layout.class)
public class RootPage extends Div {
}

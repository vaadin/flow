...
class VaadinModule extends AbstractModule {
    ...
    protected void configure() {
        ...
        for (Class<?> controllerClass : guiceVaadinServlet.getControllerClasses()) {
            final Class<?> classToControl = controllerClass.getAnnotation(Controller.class).value();

            bindListener(
                new ControllerClassMatcher(classToControl),
                new ControllerCreator(controllerClass)
            );
        }
    }

    private static class ControllerClassMatcher extends AbstractMatcher<Binding<?>> {
        private final Class<?> triggeringClass;

        ControllerClassMatcher(Class<?> triggeringClass) {
        ...
        public boolean matches(Binding<?> binding) {
            ...
            return triggeringClass.equals(rawType);
        }
    }

    private class ControllerCreator implements ProvisionListener {
        private final Class<?> controllerClass;

        ControllerCreator(Class<?> controllerClass) {

...
public class RouteConfigurationTest {
    ...
    public void routeConfigurationUpdateLock_configurationIsUpdatedOnlyAfterUnlock() {
        ...
        RouteConfiguration routeConfiguration = RouteConfiguration.forRegistry(getRegistry(session));
    ...
    public void routeConfiguration_getMethodsReturnCorrectly() {
        ...
        routeConfiguration.update(() -> {
            ...
            routeConfiguration.setRoute("parents", MiddleLayout.class, MainLayout.class);
            routeConfiguration.setAnnotatedRoute(ParameterView.class);
        });

        Assert.assertEquals("After unlock registry should be updated for others to configure with new data", 4,
                routeConfiguration.getAvailableRoutes().size());
        Assert.assertTrue("Expected path '' to be registered", routeConfiguration.isPathAvailable(""));
        Assert.assertTrue("Expected path 'path' to be registered", routeConfiguration.isPathAvailable("path"));
        Assert.assertTrue("Expected path 'parents' to be registered", routeConfiguration.isPathAvailable("parents"));
        ...
        Assert.assertEquals("Url should have only been 'parents'", "parents", template.get());

        Optional<Class<? extends Component>> pathRoute = routeConfiguration.getRoute("path");
        Assert.assertTrue("'path' should have returned target class", pathRoute.isPresent());
        Assert.assertEquals("'path' registration should be Secondary", Secondary.class, pathRoute.get());
        ...
        Assert.assertEquals("ParameterView template is not correctly generated from Route and RoutePrefix",
                "category/:int(" + RouteParameterRegex.INTEGER + ")/item/:long(" + RouteParameterRegex.LONG + ")",
                template.get());

        Assert.assertTrue("ParameterView template not registered.", routeConfiguration.isPathAvailable(
                "category/:int(" + RouteParameterRegex.INTEGER + ")/item/:long(" + RouteParameterRegex.LONG + ")"));

        Assert.assertEquals("ParameterView url with RouteParameters not generated correctly.",
                "category/1234567890/item/12345678900",
                routeConfiguration.getUrl(ParameterView.class, new RouteParameters(new RouteParam("int", "1234567890"),
        ...
        Assert.assertFalse("Removing the path 'path' should have cleared it from the registry",
                routeConfiguration.isPathAvailable("path"));

        Assert.assertTrue("Expected path 'url' to be registered",
                routeConfiguration.isPathAvailable(HasUrlParameterFormat.getTemplate("url", Url.class)));

        Optional<Class<? extends Component>> urlRoute = routeConfiguration.getRoute("url");

        Assert.assertFalse("'url' with no parameters should not have returned a class", urlRoute.isPresent());

        urlRoute = routeConfiguration.getRoute("url", Collections.singletonList("param"));
        Assert.assertTrue("'url' with parameters should have returned a class", urlRoute.isPresent());
        Assert.assertEquals("'url' registration should be Url", Url.class, urlRoute.get());
    }
    ...
    public void routeConfiguration_routeTemplatesWorkCorrectly() {
        ...
        final Optional<String> template = routeConfiguration.getTemplate(ComponentView.class);
        Assert.assertTrue("Missing template", template.isPresent());
        ...
        Assert.assertEquals("component/button/api/com/vaadin/flow/button",
                routeConfiguration.getUrl(ComponentView.class,
                        new RouteParameters(new RouteParam("identifier", "button"), new RouteParam("tab", "api"),
        ...
        Assert.assertEquals("component/button/com/vaadin/flow/button",
                routeConfiguration.getUrl(ComponentView.class, new RouteParameters(
                        new RouteParam("identifier", "button"), new RouteParam("path", "com/vaadin/flow/button"))));

        // url produced by @RouteAlias(value =
        // ":tab(overview|samples|links|reviews|discussions)")
        Assert.assertEquals("component/button/reviews", routeConfiguration.getUrl(ComponentView.class,
                new RouteParameters(new RouteParam("identifier", "button"), new RouteParam("tab", "reviews"))));

        // url produced by @RouteAlias(value =
        // ":tab(overview|samples|links|reviews|discussions)")
        Assert.assertEquals("component/button/overview", routeConfiguration.getUrl(ComponentView.class,
                new RouteParameters(new RouteParam("identifier", "button"), new RouteParam("tab", "overview"))));

        try {
            // Asking the url of target with invalid parameter values.
            routeConfiguration.getUrl(ComponentView.class,
                    new RouteParameters(new RouteParam("identifier", "button"), new RouteParam("tab", "examples")));
    ...
    public void setRoutes_allExpectedRoutesAreSet() {
        ...
        routeConfiguration.update(() -> {
            ...
            Arrays.asList(MyRoute.class, MyInfo.class, MyPalace.class, MyModular.class)
    ...
    @Route(value = "item/:long(" + RouteParameterRegex.LONG + ")", layout = MainView.class)
    @Tag("div")

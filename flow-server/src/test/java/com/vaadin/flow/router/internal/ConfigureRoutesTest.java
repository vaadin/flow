...
public class ConfigureRoutesTest {
    ...
    public void mutableConfiguration_canSetRouteTarget() {
        ...
        Assert.assertTrue("Configuration should have \"\" route registered", mutable.hasTemplate(""));
    ...
    public void mutableConfigurationClear_removesRegisteredRoutes() {
        ...
        Assert.assertNull(mutable.getNavigationRouteTarget("").getRouteTarget());
        Assert.assertNull(mutable.getNavigationRouteTarget("123").getRouteTarget());
    ...
    public void mutableConfigurationClear_preservesErrorRoute() {
        ...
        Assert.assertEquals("ErrorRoute shouldn't be cleared.", BaseError.class,
                mutable.getExceptionHandlerByClass(IndexOutOfBoundsException.class));
    }
    ...
    public void duplicateRootPathRegistration_throwsException() {
        ...
        mutable.setRoute("", BaseTarget.class);
        Assert.assertTrue(mutable.hasRouteTarget(BaseTarget.class));
        Assert.assertEquals(BaseTarget.class, mutable.getNavigationRouteTarget("").getRouteTarget().getTarget());

        exceptionRule.expect(AmbiguousRouteConfigurationException.class);
        exceptionRule.reportMissingExceptionWithMessage("Duplicate routes shouldn't be accepted.");
    ...
    public void duplicateParameterPathRegistration_throwsException() {
        ...
        Assert.assertEquals(ParamTarget.class, mutable.getNavigationRouteTarget("123").getRouteTarget().getTarget());

        exceptionRule.expect(AmbiguousRouteConfigurationException.class);
        exceptionRule.reportMissingExceptionWithMessage("Duplicate parameter routes shouldn't be accepted.");
        exceptionRule.expectMessage(
    ...
    public void mutableConfiguration_canSetErrorTargets() {
        ...
        Assert.assertFalse("Exception targets should be available", mutable.getExceptionHandlers().isEmpty());
        Assert.assertEquals("Given exception returned unexpected handler class", BaseError.class,
                mutable.getExceptionHandlerByClass(IndexOutOfBoundsException.class));
    }
    ...
    public void populatedMutableConfiguration_clearRemovesAllContent() {
        ...
        Assert.assertFalse("Configuration should have routes.", mutable.getRoutes().isEmpty());
        Assert.assertFalse("Configuration should have exceptions.", mutable.getExceptionHandlers().isEmpty());

        mutable.clear();

        Assert.assertTrue("After clear all routes should have been removed.", mutable.getRoutes().isEmpty());
        Assert.assertTrue("After clear all targetRoutes should have been removed. ",
                mutable.getTargetRoutes().isEmpty());
        Assert.assertFalse("After clear  exception targets should still be available.",

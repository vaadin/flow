...
public class NodeFeatureTest {
    ...
    private static Map<Class<? extends NodeFeature>, Integer> buildExpectedIdMap() {
        ...
        expectedIds.put(ElementPropertyMap.class, NodeFeatures.ELEMENT_PROPERTIES);
        expectedIds.put(ElementAttributeMap.class, NodeFeatures.ELEMENT_ATTRIBUTES);
        expectedIds.put(ElementChildrenList.class, NodeFeatures.ELEMENT_CHILDREN);
        expectedIds.put(ElementListenerMap.class, NodeFeatures.ELEMENT_LISTENERS);
        expectedIds.put(PushConfigurationMap.class, NodeFeatures.UI_PUSHCONFIGURATION);
        expectedIds.put(PushConfigurationParametersMap.class, NodeFeatures.UI_PUSHCONFIGURATION_PARAMETERS);
        expectedIds.put(TextNodeMap.class, NodeFeatures.TEXT_NODE);
        expectedIds.put(PollConfigurationMap.class, NodeFeatures.POLL_CONFIGURATION);
        expectedIds.put(ReconnectDialogConfigurationMap.class, NodeFeatures.RECONNECT_DIALOG_CONFIGURATION);
        expectedIds.put(LoadingIndicatorConfigurationMap.class, NodeFeatures.LOADING_INDICATOR_CONFIGURATION);
        expectedIds.put(ElementClassList.class, NodeFeatures.CLASS_LIST);
        expectedIds.put(ElementStylePropertyMap.class, NodeFeatures.ELEMENT_STYLE_PROPERTIES);
        ...
        expectedIds.put(PolymerServerEventHandlers.class, NodeFeatures.POLYMER_SERVER_EVENT_HANDLERS);
        expectedIds.put(PolymerEventListenerMap.class, NodeFeatures.POLYMER_EVENT_LISTENERS);
        expectedIds.put(ClientCallableHandlers.class, NodeFeatures.CLIENT_DELEGATE_HANDLERS);
        ...
        expectedIds.put(AttachExistingElementFeature.class, NodeFeatures.ATTACH_EXISTING_ELEMENT);
        expectedIds.put(BasicTypeValue.class, NodeFeatures.BASIC_TYPE_VALUE);
        expectedIds.put(VirtualChildrenList.class, NodeFeatures.VIRTUAL_CHILDREN);
        expectedIds.put(ReturnChannelMap.class, NodeFeatures.RETURN_CHANNEL_MAP);
    ...
    public void testGetIdValues() {
        ...
        Assert.assertEquals("The number of expected features is not up to date", expectedIds.size(),
                NodeFeatureRegistry.nodeFeatures.size());

        expectedIds.forEach((type, expectedId) -> {
            Assert.assertEquals("Unexpected id for " + type.getName(), expectedId.intValue(),
    ...
    public void testGetById() {
        ...
        expectedIds.forEach((expectedType, id) -> {
            Assert.assertEquals("Unexpected type for id " + id, expectedType, NodeFeatureRegistry.getFeature(id));
        });
    }

    @Test
    public void priorityOrder() {
        List<Class<? extends NodeFeature>> priorityOrder = buildExpectedIdMap().keySet().stream()
                .sorted(NodeFeatureRegistry.PRIORITY_COMPARATOR).collect(Collectors.toList());

        List<Class<? extends NodeFeature>> expectedOrder = Arrays.asList(
                /* Primary features */
                ElementData.class, TextNodeMap.class, ModelList.class, BasicTypeValue.class,
                ...
                ElementClassList.class, ElementAttributeMap.class, ElementListenerMap.class, VirtualChildrenList.class,
                ...
                ElementStylePropertyMap.class, ShadowRootData.class, ShadowRootHost.class,
                AttachExistingElementFeature.class, InertData.class,

                /* Only used for the root node */
                PushConfigurationMap.class, PushConfigurationParametersMap.class,
                LoadingIndicatorConfigurationMap.class, PollConfigurationMap.class,
        ...
        for (int i = 0; i < priorityOrder.size(); i++) {
            if (priorityOrder.get(i) != expectedOrder.get(i)) {
                Assert.fail("Invalid priority ordering at index " + i + ". Expected "
                        + expectedOrder.get(i).getSimpleName() + " but got " + priorityOrder.get(i).getSimpleName());
            }
        }
    }

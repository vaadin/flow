...
public class PushConfigurationMapTest extends AbstractNodeFeatureTest<PushConfigurationMap> {
    ...
    public void transportWebsocket() {
        ...
        Assert.assertEquals(Transport.WEBSOCKET.getIdentifier(), ns.getParameter("transport"));
        Assert.assertFalse(ns.contains(PushConfigurationMap.ALWAYS_USE_XHR_TO_SERVER));
    ...
    public void transportLongPolling() {
        ...
        Assert.assertEquals(Transport.LONG_POLLING.getIdentifier(), ns.getParameter("transport"));
        Assert.assertFalse(ns.contains(PushConfigurationMap.ALWAYS_USE_XHR_TO_SERVER));
    ...
    public void transportLongWebsocketXHR() {
        ...
        Assert.assertEquals(Transport.WEBSOCKET.getIdentifier(), ns.getParameter("transport"));
        Assert.assertTrue((Boolean) ns.get(PushConfigurationMap.ALWAYS_USE_XHR_TO_SERVER));
    ...
    public void fallbackTransportLongPolling() {
        ...
        Assert.assertEquals(Transport.LONG_POLLING.getIdentifier(), ns.getParameter("fallbackTransport"));
    ...
    public void fallbackTransportWebsocket() {
        ...
        Assert.assertEquals(Transport.WEBSOCKET.getIdentifier(), ns.getParameter("fallbackTransport"));
    ...
    public void parameterNames() {
        ...
        Assert.assertArrayEquals(new String[] { "foo" }, ns.getParameterNames().toArray());

        ns.setTransport(Transport.WEBSOCKET);
        ns.setFallbackTransport(Transport.LONG_POLLING);

        String[] expected = new String[] { "foo", "transport", "fallbackTransport" };

...
public class WebComponentConfigurationFactoryTest {
    ...
    public void create_constructsValidConfiguration() {
        WebComponentConfiguration<? extends Component> config1 = factory.create(
                new DefaultWebComponentExporterFactory<WebComponentExporterTest.MyComponent>(MyComponentExporter.class)
                        .create());

        WebComponentConfiguration<? extends Component> config2 = factory.create(new MyComponentExporter());

        Assert.assertNotNull("create() from class should have been successful", config1);
        Assert.assertNotNull("create() from instance should have been " + "successful", config2);
    }
    ...
    public void create_configuration_hashCode() {
        WebComponentConfiguration<WebComponentExporterTest.MyComponent> myComponentConfig = factory.create(
                new DefaultWebComponentExporterFactory<WebComponentExporterTest.MyComponent>(MyComponentExporter.class)
                        .create());
        WebComponentConfiguration<MyComponent> similarConfig1 = factory
                .create(new DefaultWebComponentExporterFactory<>(SimilarExporter1.class).create());
        WebComponentConfiguration<MyComponent> similarConfig2 = factory
                .create(new DefaultWebComponentExporterFactory<>(SimilarExporter2.class).create());
        WebComponentConfiguration<MyComponent> similarConfig3 = factory
                .create(new DefaultWebComponentExporterFactory<>(SimilarExporter3.class).create());

        Assert.assertNotEquals("Configurations with different tags should have " + "not have same hashCodes",
                myComponentConfig.hashCode(), similarConfig1.hashCode());

        Assert.assertNotEquals(
                "Configurations with same tag, but different " + "properties should not have same hashCodes",
    ...
    public void create_configuration_equals() {
        WebComponentConfiguration<WebComponentExporterTest.MyComponent> myComponentConfig = factory
                .create(new DefaultWebComponentExporterFactory<>(MyComponentExporter.class).create());
        WebComponentConfiguration<MyComponent> similarConfig1 = factory
                .create(new DefaultWebComponentExporterFactory<>(SimilarExporter1.class).create());
        WebComponentConfiguration<MyComponent> similarConfig2 = factory
                .create(new DefaultWebComponentExporterFactory<>(SimilarExporter2.class).create());
        ...
        Assert.assertNotEquals("Configurations with different tags should " + "not be equal", myComponentConfig,
                similarConfig1);

        Assert.assertNotEquals("Configurations with same tag, but different " + "properties should not be equal",
                similarConfig1, similarConfig2);

        // even though the classes are different, they define the same
        // embeddable web component
        Assert.assertEquals(
                "Configurations with same tag and same properties " + "but different defaults should be equal",
    ...
    public static class MyComponentExporter extends WebComponentExporter<WebComponentExporterTest.MyComponent> {
        ...
        @Override
        public void configureInstance(WebComponent<WebComponentExporterTest.MyComponent> webComponent,

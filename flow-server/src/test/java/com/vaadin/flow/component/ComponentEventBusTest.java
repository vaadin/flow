...
public class ComponentEventBusTest {
    ...
    private ObjectNode createStateNodeIdData(String key, int value) {
        return createData(JsonConstants.MAP_STATE_NODE_EVENT_DATA + key, value);
    }

    private ObjectNode createStateNodeIdData(String key, int value, String key2,
            int value2) {
        return createData(JsonConstants.MAP_STATE_NODE_EVENT_DATA + key, value,
                JsonConstants.MAP_STATE_NODE_EVENT_DATA + key2, value2);
    }

    private ObjectNode createData(String key, Object value) {
        ObjectNode data = JacksonUtils.createObjectNode();
        data.set(key, JacksonCodec.encodeWithoutTypeInfo(value));
        return data;
    }

    private ObjectNode createData(String key, Object value, String key2,
            Object value2) {
        ObjectNode data = JacksonUtils.createObjectNode();
        data.set(key, JacksonCodec.encodeWithoutTypeInfo(value));
    ...
    public void domEvent_removeListener() {
        ...
        ObjectNode eventData = createData("event.someData", 42,
                "event.moreData", 1);

...
public class DomEventTest {
    ...
    @DomEvent(value = "event", debounce = @DebounceSettings(100))
    public static class DebounceTimeoutEvent extends ComponentEvent<Component> {
        public DebounceTimeoutEvent(Component source, boolean fromClient) {
            super(source, fromClient);
        }
    }

    @Test
    public void debounceTimeout() {
        assertSettings(DebounceTimeoutEvent.class,
                ElementListenerMap.ALWAYS_TRUE_FILTER, 100,
                DebouncePhase.TRAILING);
    }

    @DomEvent(value = "event", debounce = @DebounceSettings(value = 200, phases = {
    ...
    @DomEvent(value = "event", filter = "filter(event)", debounce = @DebounceSettings(300))

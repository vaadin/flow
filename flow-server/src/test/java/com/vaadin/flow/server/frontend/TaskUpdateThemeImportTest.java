...
public class TaskUpdateThemeImportTest {
    ...
    private static final String CUSTOM_THEME_PATH = String.join("/", APPLICATION_THEME_ROOT, CUSTOM_THEME_NAME);
    ...
    public void setUp() throws IOException {
        ...
        Options options = new Options(Mockito.mock(Lookup.class), npmFolder).withFrontendDirectory(frontendDirectory);
    ...
    public void taskExecuted_customThemeWithNonExistingThemeFolder_throwsException() {
        ...
        TaskUpdateThemeImport taskUpdateThemeImportWithNonExistentThemeFolder = new TaskUpdateThemeImport(customTheme,
                options);

        ExecutionFailedException e = Assert.assertThrows(ExecutionFailedException.class,
                taskUpdateThemeImportWithNonExistentThemeFolder::execute);

        Assert.assertTrue(e.getMessage()
                .contains(String.format("Discovered @Theme annotation with theme name '%s', "
                        + "but could not find the theme directory in the "
                        + "project or available as a jar dependency.", CUSTOM_THEME_NAME)));
    }
    ...
    public void taskExecuted_customThemeWithThemeFolderInFrontend_ensuresThemeGeneratedJsCreatedSuccessfully()
        ...
        Assert.assertTrue(
                String.format("%s directory should be created at '%s%s/%s' but failed.", CUSTOM_THEME_NAME,
                        DEFAULT_FRONTEND_DIR, APPLICATION_THEME_ROOT, CUSTOM_THEME_NAME),
    ...
    public void taskExecuted_customThemeWithThemeFolderInMetaInf_ensuresThemeGeneratedJsCreatedSuccessfully()
        ...
        File correctMetaInfResourcesDirectory = new File(projectRoot, APPLICATION_META_INF_RESOURCES);
        File themesDir = new File(correctMetaInfResourcesDirectory, APPLICATION_THEME_ROOT);
        File aCustomThemeDir = new File(themesDir, CUSTOM_THEME_NAME);
        ...
        Assert.assertTrue(
                String.format("%s directory should be created at '%s/%s/%s' but failed.", CUSTOM_THEME_NAME,
                        APPLICATION_META_INF_RESOURCES, APPLICATION_THEME_ROOT, CUSTOM_THEME_NAME),
                customThemeDirCreatedSuccessfully);
    ...
    public void taskExecuted_customThemeWithThemeFolderInStatic_ensuresThemeGeneratedJsCreatedSuccessfully()
        ...
        File correctStaticResourcesDirectory = new File(projectRoot, APPLICATION_STATIC_RESOURCES);
        File themesDir = new File(correctStaticResourcesDirectory, APPLICATION_THEME_ROOT);
        File aCustomThemeDir = new File(themesDir, CUSTOM_THEME_NAME);
        ...
        Assert.assertTrue(
                String.format("%s directory should be created at '%s/%s/%s' but failed.", CUSTOM_THEME_NAME,
                        APPLICATION_STATIC_RESOURCES, APPLICATION_THEME_ROOT, CUSTOM_THEME_NAME),
                customThemeDirCreatedSuccessfully);
    ...
    public void taskExecuted_customThemeWithThemeFolderInClasspath_ensuresThemeGeneratedJsCreatedSuccessfully()
        ...
        File jarResourcesFolder = new File(new File(frontendDirectory, FrontendUtils.GENERATED),
        ...
        Assert.assertTrue(
                String.format("%s directory should be created at '%s' but failed.", CUSTOM_THEME_NAME, aCustomThemeDir),
                customThemeDirCreatedSuccessfully);
    ...
    public void runTaskWithTheme_createsThemeFile_afterRunWithoutTheme_removesThemeFile() throws Exception {
        ...
        assertNoThemeGeneratedDefinitionFilesExist("After removal of theme %s should be removed");
    }

    private void assertNoThemeGeneratedDefinitionFilesExist(String errorMessage) {
        Assert.assertFalse(String.format(errorMessage, "\"theme.js\""), themeImportFile.exists());

        Assert.assertFalse(String.format(errorMessage, "\theme.d.ts\""), themeImportTsFile.exists());
    }

    private void assertThemeGeneratedDefinitionFilesExist(String errorMessage) {
        Assert.assertTrue(String.format(errorMessage, "\"theme.js\""), themeImportFile.exists());

        Assert.assertTrue(String.format(errorMessage, "\theme.d.ts\""), themeImportTsFile.exists());
    }
    ...
    public void taskExecuted_customThemeFolderExistsInBothFrontendAndInClasspath_throwsException() {
        ...
        String classPathThemePath = DEFAULT_FRONTEND_DIR + FrontendUtils.GENERATED + FrontendUtils.JAR_RESOURCES_FOLDER
                + "/" + CUSTOM_THEME_PATH;
        ...
        Assert.assertTrue(e.getMessage()
                .contains(String.format(
                        "Theme '%s' should not exist inside a " + "jar and in the project at the same time.",
                        CUSTOM_THEME_NAME)));
    }
    ...
    public void taskExecuted_customThemeFolderExistsInBothStaticAndMetaInfResources_throwsException() {
        ...
        Assert.assertTrue(e.getMessage().contains(String.format(
                "Discovered Theme folder for theme '%s' " + "in more than one place in the project. Please "

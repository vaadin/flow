...
public class WebComponentGeneratorTest {
    ...
    public void assertGeneratedReplacementMapContainsExpectedEntries(boolean generateUi) {
        ...
        Map<String, String> replacementsMap = WebComponentGenerator.getReplacementsMap("my-component",
                new WebComponentExporter.WebComponentConfigurationFactory().create(exporter).getPropertyDataSet(),
                "/foo", generateUi, null);
        ...
        Assert.assertTrue("Missing 'AttributeChange'", replacementsMap.containsKey("AttributeChange"));
        Assert.assertTrue("Missing 'PropertyDefaults'", replacementsMap.containsKey("PropertyDefaults"));
        Assert.assertTrue("Missing 'PropertyMethods'", replacementsMap.containsKey("PropertyMethods"));
        Assert.assertTrue("Missing 'PropertyValues'", replacementsMap.containsKey("PropertyValues"));
        ...
        MatcherAssert.assertThat(attributeChange, containsString(String.format("if (attribute === 'message') {%n"
                + "  this['message'] = this._deserializeValue(value, " + "String);")));
        MatcherAssert.assertThat(attributeChange, containsString(String.format("if (attribute === 'integer-value') {%n"
                + "  this['integer-value'] = this._deserializeValue" + "(value, Number);")));
        MatcherAssert.assertThat(attributeChange,
                containsString(String.format("if (attribute === 'camel-case-value') {%n"
                        + "  this['camelCaseValue'] = this._deserializeValue" + "(value, Number);")));
        MatcherAssert.assertThat(attributeChange, containsString(String.format("if (attribute === 'response') {%n"
                + "  this['response'] = this._deserializeValue(value," + " String);")));

        String propertyMethods = replacementsMap.get("PropertyMethods");
        MatcherAssert.assertThat(propertyMethods, containsString("get ['message']() {"));
        MatcherAssert.assertThat(propertyMethods, containsString("set ['message'](value) {"));
        MatcherAssert.assertThat(propertyMethods, containsString("this._sync('message',"));
        MatcherAssert.assertThat(propertyMethods, containsString("set ['integer-value'](value) {"));
        MatcherAssert.assertThat(propertyMethods, containsString("this._sync('integer-value',"));
        MatcherAssert.assertThat(propertyMethods, containsString("set ['response'](value) {"));
        MatcherAssert.assertThat(propertyMethods, containsString("this._sync('response',"));

        String propertyValues = replacementsMap.get("PropertyValues");
        MatcherAssert.assertThat(propertyValues, containsString("'message': this['message']"));
        MatcherAssert.assertThat(propertyValues, containsString("'integer-value': this['integer-value']"));
        MatcherAssert.assertThat(propertyValues, containsString("'response': this['response']"));

        String propertyDefaults = replacementsMap.get("PropertyDefaults");
        MatcherAssert.assertThat(propertyDefaults, containsString("this['_message'] = ''"));
        MatcherAssert.assertThat(propertyDefaults, containsString("this['_integer-value'] = 0"));
        MatcherAssert.assertThat(propertyDefaults, containsString("this['_response'] = 'hello'"));
    }

    @Test
    public void providesJSModulesInNpmMode() {
        String module = WebComponentGenerator.generateModule(
                new DefaultWebComponentExporterFactory<MyComponent>(MyComponentExporter.class), "", null);
        // make sure that the test works on windows machines:
        module = module.replace("\r", "");
        MatcherAssert.assertThat(module, startsWith("class Tag extends HTMLElement {"));
        ...
        MatcherAssert.assertThat(module, containsString("customElements.define('tag', Tag);\n"));
    }

    @Test
    public void providedJSModuleContainsCorrectThemeReplacements() {
        String module = WebComponentGenerator
                .generateModule(new DefaultWebComponentExporterFactory<>(MyComponentExporter.class), "", "my-theme");
        // make sure that the test works on windows machines:
        module = module.replace("\r", "");
    ...
    public static class MyComponentExporter extends WebComponentExporter<MyComponent> {
        ...
        public MyComponentExporter() {
            ...
            addProperty("integer-value", 0).onChange(MyComponent::setIntegerValue);

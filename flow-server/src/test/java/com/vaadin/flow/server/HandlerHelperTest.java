...
public class HandlerHelperTest {
    ...
    private HttpServletRequest createRequest(String pathInfo, RequestType type) {
        return createRequest(pathInfo, type == null ? null : type.getIdentifier());
    }
    ...
    public void isFrameworkInternalRequest_validType_nullPathInfo() {
        ...
        Assert.assertTrue(HandlerHelper.isFrameworkInternalRequest("/*", request));
        Assert.assertFalse(HandlerHelper.isFrameworkInternalRequest("/foo/*", request));
        Assert.assertFalse(HandlerHelper.isFrameworkInternalRequest("/foo", request));
    }
    ...
    public void isFrameworkInternalRequest_validType_withPath() {
        ...
        Assert.assertFalse(HandlerHelper.isFrameworkInternalRequest("/foo", request));
        Assert.assertFalse(HandlerHelper.isFrameworkInternalRequest("/foo/*", request));
        Assert.assertTrue(HandlerHelper.isFrameworkInternalRequest("/hello", request));
        Assert.assertTrue(HandlerHelper.isFrameworkInternalRequest("/hello/*", request));
    }
    ...
    public void isFrameworkInternalRequest_validType_withServletMappingAndPath() {
        ...
        Assert.assertFalse(HandlerHelper.isFrameworkInternalRequest("/", request));
        Assert.assertFalse(HandlerHelper.isFrameworkInternalRequest("", request));
        Assert.assertTrue(HandlerHelper.isFrameworkInternalRequest("/servlet", request));
    }
    ...
    public void isFrameworkInternalRequest_noType_withServletMappingAndPath() {
        ...
        Assert.assertFalse(HandlerHelper.isFrameworkInternalRequest("/", request));
        Assert.assertFalse(HandlerHelper.isFrameworkInternalRequest("/servlet", request));
    }
    ...
    public void isFrameworkInternalRequest_vaadinRequest_servletRoot_noType() {
        ...
        Assert.assertFalse(BootstrapHandler.isFrameworkInternalRequest(request));
    }

    @Test
    public void isFrameworkInternalRequest_vaadinRequest_pathInsideServlet() {
        VaadinRequest request = createVaadinRequest("/foo", "/*", RequestType.INIT);
    ...
    public void isFrameworkInternalRequest_vaadinRequest_nonRootServlet() {
        VaadinRequest request = createVaadinRequest("", "/myservlet/", RequestType.INIT);
    ...
    public void isFrameworkInternalRequest_vaadinRequest_nonRootServlet_pathInsideServlet() {
        VaadinRequest request = createVaadinRequest("/hello", "/myservlet", null);
    ...
    public void isFrameworkInternalRequest_uploadUrl() {
        VaadinServletRequest request = createVaadinRequest(
                "VAADIN/dynamic/resource/1/e83d6b6d-2b75-4960-8922-5431f4a23e49/upload", "", null);

        Assert.assertTrue(HandlerHelper.isFrameworkInternalRequest("/*", request.getHttpServletRequest()));
    }

    @Test
    public void isFrameworkInternalRequest_hillaPushUrl() {
        VaadinServletRequest request = createVaadinRequest("HILLA/push", "", null);
    ...
    public void isFrameworkInternalRequest_flowPushUrl() {
        VaadinServletRequest request = createVaadinRequest("VAADIN/push", "", RequestType.PUSH);
    ...
    public void isFrameworkInternalRequest_vaadinServletMapping_flowPushUrl() {
        VaadinServletRequest request = createVaadinRequest("/VAADIN/push", "/ui", RequestType.PUSH);

        Assert.assertTrue(HandlerHelper.isFrameworkInternalRequest("/ui/*", request.getHttpServletRequest()));
    }

    @Test
    public void isFrameworkInternalRequest_fakeUploadUrl() {
        VaadinServletRequest request = createVaadinRequest("VAADIN/dynamic/resource/../../../upload", "", null);

        Assert.assertFalse(HandlerHelper.isFrameworkInternalRequest("/*", request.getHttpServletRequest()));
    }

    @Test
    public void isFrameworkInternalRequest_staticFileUrl() {
        VaadinServletRequest request = createVaadinRequest("VAADIN/static/file.png", "", null);
    ...
    public void getPathIfInsideServlet_default_servlet() {
        ...
        Assert.assertEquals(Optional.of(""), HandlerHelper.getPathIfInsideServlet(servletMapping, ""));
        Assert.assertEquals(Optional.of("/"), HandlerHelper.getPathIfInsideServlet(servletMapping, "/"));
        Assert.assertEquals(Optional.of("foo"), HandlerHelper.getPathIfInsideServlet(servletMapping, "foo"));
        Assert.assertEquals(Optional.of("/foo"), HandlerHelper.getPathIfInsideServlet(servletMapping, "/foo"));
    }
    ...
    public void getPathIfInsideServlet_root_only_servlet() {
        String servletMapping = "";
        Assert.assertEquals(Optional.of(""), HandlerHelper.getPathIfInsideServlet(servletMapping, ""));
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, "/"));
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, "foo"));
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, "/foo"));
    }
    ...
    public void getPathIfInsideServlet_sevlet_using_single_path() {
        ...
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, ""));
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, "bar"));
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, "/bar"));
        Assert.assertEquals(Optional.of(""), HandlerHelper.getPathIfInsideServlet(servletMapping, "foo"));
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, "foo/"));
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, "foo/bar"));
    }
    ...
    public void getPathIfInsideServlet_sevlet_with_context_path() {
        ...
        Assert.assertEquals(Optional.of(""), HandlerHelper.getPathIfInsideServlet(servletMapping, "foo/"));
        Assert.assertEquals(Optional.of(""), HandlerHelper.getPathIfInsideServlet(servletMapping, "/foo"));
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, "/foos"));
        Assert.assertEquals(Optional.empty(), HandlerHelper.getPathIfInsideServlet(servletMapping, "/foos/bar"));
        Assert.assertEquals(Optional.of(""), HandlerHelper.getPathIfInsideServlet(servletMapping, "/foo/"));
        Assert.assertEquals(Optional.of("bar"), HandlerHelper.getPathIfInsideServlet(servletMapping, "/foo/bar"));
    }

    private VaadinServletRequest createVaadinRequest(String requestPath, String servletPath, RequestType type) {
        ...
        return new VaadinServletRequest(servletRequest, Mockito.mock(VaadinServletService.class));
    }

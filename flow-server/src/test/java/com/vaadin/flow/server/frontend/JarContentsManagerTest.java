...
public class JarContentsManagerTest {
    ...
    public void containsPath_nonExistingPath() {
        ...
        assertFalse(String.format("Test jar '%s' should not contain path '%s'", testJar, nonExistingPath),
    ...
    public void containsPath_existingFile() {
        ...
        assertTrue(String.format("Test jar '%s' should contain path '%s'", testJar, existingPath),
                jarContentsManager.containsPath(testJar, existingPath));
    }

    /*
     * Test for issue: flow fails to serve static resources from latest webjars #6241
    ...
    public void containsPath_missingDirectoryStructure_scansForMatch() {
        ...
        assertTrue(String.format("Test jar '%s' should contain path '%s'", testJar, existingPathLocal),
    ...
    public void findFiles_existingFiles() {
        ...
        List<String> bowerJsons = jarContentsManager.findFiles(TestUtils.getTestJar(resourceName), "", searchName);

        assertEquals(String.format("Expect '%s' WebJar to contain two '%s' files", resourceName, searchName), 2,
                bowerJsons.size());
    ...
    public void findFiles_existingFiles_baseDirectoryMatters() {
        ...
        List<String> bowerJson = jarContentsManager.findFiles(TestUtils.getTestJar(resourceName), testPath, searchName);

        assertEquals(String.format("Expect '%s' WebJar to contain one '%s' file in directory '%s'", resourceName,
                searchName, testPath), 1, bowerJson.size());
    }
    ...
    public void copyFilesFromJar_notAJarFile() throws IOException {
        ...
        expectedException.expect(UncheckedIOException.class);
        expectedException.expectMessage(String.format("Failed to extract files from jarFile '%s' to directory '%s'",
    ...
    public void copyFilesFromJar_noBasePath_noExclusions() {
        assertThat("Do not expect any files in temporary directory before the test",
                TestUtils.listFilesRecursively(testDirectory.getRoot()).size(), is(0));
        ...
        assertThat("Temporary directory should have files after jar copied",
                TestUtils.listFilesRecursively(testDirectory.getRoot()).size(), is(not(0)));
    }

    @Test
    public void copyFilesFromJar_noBasePath_excludeEverything() {
        jarContentsManager.copyFilesFromJarTrimmingBasePath(testJar, null, testDirectory.getRoot(), "*");
        assertThat("Do not expect any files with filter that excludes everything",
                TestUtils.listFilesRecursively(testDirectory.getRoot()).size(), is(0));
    }
    ...
    public void copyFilesFromJar_withBasePath_noExclusions() throws IOException {
        ...
        assertThat(String.format("Expect jar '%s' to contain files with base path '%s'", testJar, basePath),
                resultingPaths.size(), is(not(0)));
        assertTrue("Resulting paths should not contain base path = " + basePath,
                resultingPaths.stream().noneMatch(path -> path.contains(basePath)));
    }
    ...
    public void copyFilesFromJar_exclusionsWork() throws IOException {
        ...
        assertTrue("Filtered paths' count should be less than non filtered paths' count",
                filteredPaths.size() < notFilteredPaths.size());
        assertTrue("Not filtered paths should contain xml files",
                notFilteredPaths.stream().anyMatch(path -> path.endsWith(".xml")));
    ...
    public void copyFilesFromJar_basePathAppendedWithTrailingSlash() throws IOException {
        ...
        assertEquals("Base path without trailing slash should be treated the same as base path with one",
    ...
    public void copyFilesFromJar_copiedFromBasePathResultsAreContainedInAllPaths() throws IOException {
        ...
        List<String> filteredPathsPrefixedByBasePath = filteredPaths.stream().map(path -> basePath + path)
    ...
    public void copyFilesFromJar_casePreserved() {
        ...
        assertEquals(String.format(
                "Number of files in jar '%s' in jar directory '%s' and number of copied files should match.", testJar,
                jarDirectory), originalFiles.size(), copiedFiles.size());

        copiedFiles
                .forEach(copiedFile -> assertTrue(
                        String.format("Failed to find copied file '%s' in files '%s' from jar '%s'", copiedFile,
                                originalFiles, testJar),
    ...
    public void copyFilesFromJar_doNotUpdateFileIfContentIsTheSame() {
        ...
        File testJar = TestUtils.getTestJar("paper-button-2.0.0.jar");
        File jsonFile = copyFilesFromJar(outputDirectory, jarDirectory, testJar);
    ...
    public void copyFilesFromJar_updateFileIfContentIsNotTheSame() throws IOException {
        ...
        String originalContent = FileUtils.readLines(jsonFile, StandardCharsets.UTF_8).stream()
        ...
        Assert.assertNotEquals(content,
                FileUtils.readLines(jsonFile, StandardCharsets.UTF_8).stream().collect(Collectors.joining("")));
    ...
    private List<String> listFilesInJar(File jar, String jarDirectory) {
        try (JarFile jarFile = new JarFile(jar, false)) {
            return jarFile.stream().filter(file -> !file.isDirectory())
                    .filter(file -> file.getName().startsWith(jarDirectory)).map(ZipEntry::getName)
                    .collect(Collectors.toList());
        } catch (IOException e) {

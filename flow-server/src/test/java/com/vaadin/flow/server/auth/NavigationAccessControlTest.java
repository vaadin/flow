...
public class NavigationAccessControlTest {
    ...
    public void setUp() throws Exception {
        ...
        accessControl = new NavigationAccessControl(List.of(checker1, checker2, checker3));
    }
    ...
    public void beforeEnter_principalAndRoleCheckerProvidedToCheckers() {
        ...
        ArgumentCaptor<NavigationContext> captor = ArgumentCaptor.forClass(NavigationContext.class);
        Mockito.when(checker1.check(captor.capture())).then(i -> i.getArgument(0, NavigationContext.class).allow());

        checkAccess(true, false);

        NavigationContext navigationContext = captor.getValue();
        Assert.assertNotNull("Expected Principal to be provided to checkers, but was null",
                navigationContext.getPrincipal());
        Assert.assertTrue("Wrong role checker provided to checkers", navigationContext.hasRole("user"));
    ...
    public void beforeEnter_anonymous_allCheckersDenyAccess_rerouteToNotFound() {
        ...
        Assert.assertEquals(String.join(System.lineSeparator(), accessDeniedReason(checker1),
                accessDeniedReason(checker2), accessDeniedReason(checker3)), result.getRerouteErrorMessage());
    }
    ...
    public void beforeEnter_anonymous_allCheckersNeutral_rerouteToNotFound() {
        ...
        Assert.assertFalse(result.wasTargetViewRendered());
        Assert.assertEquals(NotFoundException.class, result.getRerouteError());
        Assert.assertEquals("Access denied because navigation checkers did not take any decision.",
                result.getRerouteErrorMessage());
    }
    ...
    public void beforeEnter_developmentMode_anonymous_mixedCheckersConsensus_exceptionThrown() {
        ...
        Assert.assertTrue("Expected exception because of mixed consensus, but reason was " + failure.getMessage(),
                failure.getMessage().startsWith("Mixed consensus from navigation checkers"));
        Assert.assertTrue("Expected exception message to contain deny reasons" + failure.getMessage(),
        ...
        mockCheckerResult(checker2, AccessCheckDecision.ALLOW);
        mockCheckerResult(checker3, AccessCheckDecision.NEUTRAL);
        failure = Assert.assertThrows(IllegalStateException.class, () -> checkAccess(false, false));
        Assert.assertTrue("Expected exception because of mixed consensus, but reason was " + failure.getMessage(),
                failure.getMessage().startsWith("Mixed consensus from navigation checkers"));
        Assert.assertTrue("Expected exception message to contain deny reasons" + failure.getMessage(),
    ...
    public void beforeEnter_errorHandlingViewReroute_allCheckersNeutral_allowNavigation() {
        ...
        mockCheckerResult(checker2, AccessCheckDecision.NEUTRAL);
        mockCheckerResult(checker3, AccessCheckDecision.NEUTRAL);
        TestNavigationResult result = checkAccess(RouteAccessDeniedError.class, false, false, true);
        Assert.assertTrue(result.wasTargetViewRendered());
    ...
    public void setLoginViewClassCannotBeCalledAfterSettingClass() {
        accessControl.setLoginView(TestLoginView.class);
        accessControl.setLoginView(AccessControlTestClasses.NoAnnotationView.class);
    }
    ...
    public void beforeEnter_loginView_accessToLoginViewAlwaysAllowed() {
        ...
        mockCheckerResult(checker3, AccessCheckDecision.DENY);
        accessControl.setLoginView(TestLoginView.class);
        TestNavigationResult result = checkAccess(TestLoginView.class, false, true, false);
        Assert.assertTrue("Expecting navigation to login view to be allowed, but was not",
    ...
    public void beforeEnter_redirectUrlStoredForAnonymousUsers() {
        ...
        TestNavigationResult result = checkAccess(PermitAllView.class, false, false, true);

        String routePath = RouteUtil.getRoutePath(new MockVaadinContext(), PermitAllView.class);
        Assert.assertFalse(result.wasTargetViewRendered());
        Assert.assertEquals(AccessAnnotationCheckerTest.REQUEST_URL + routePath,
                result.sessionAttributes.get(NavigationAccessControl.SESSION_STORED_REDIRECT_ABSOLUTE));
        Assert.assertEquals(routePath, result.sessionAttributes.get(NavigationAccessControl.SESSION_STORED_REDIRECT));
    ...
    public void beforeEnter_redirectUrlNotStoredForLoggedInUsers() {
        ...
        TestNavigationResult result = checkAccess(PermitAllView.class, true, false, true);

        Assert.assertFalse(result.wasTargetViewRendered());
        Assert.assertNull(result.sessionAttributes.get(NavigationAccessControl.SESSION_STORED_REDIRECT));
        Assert.assertNull(result.sessionAttributes.get(NavigationAccessControl.SESSION_STORED_REDIRECT_ABSOLUTE));
    ...
    public void beforeEnter_disabledNavigationControl_alwaysPasses_rejectsWhenEnabled() {
        ...
        TestNavigationResult result = checkAccess(PermitAllView.class, false, false, false);
        Assert.assertTrue("Expected navigation to target view to be allowed when access control is disabled",
                result.wasTargetViewRendered());
        ...
        Assert.assertFalse("Expected navigation to target view to be denied when access control is enabled",
                result.wasTargetViewRendered());
    ...
    public void beforeEnter_noCheckersConfigured_alwaysPasses() {
        ...
        TestNavigationResult result = checkAccess(AnonymousAllowedView.class, false, false, false);
        Assert.assertTrue(result.wasTargetViewRendered());
    }

    private TestNavigationResult checkAccess(boolean authenticated, boolean productionMode) {
        return checkAccess(AnonymousAllowedView.class, authenticated, productionMode, true);
    }

    private TestNavigationResult checkAccess(Class<?> navigationTarget, boolean authenticated, boolean productionMode,
            boolean expectCheckersUsed) {
        TestNavigationResult result = setupRequest(navigationTarget, authenticated ? User.NORMAL_USER : null,
                productionMode);

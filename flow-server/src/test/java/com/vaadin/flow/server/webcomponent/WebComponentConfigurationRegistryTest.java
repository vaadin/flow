...
public class WebComponentConfigurationRegistryTest {
    ...
    public void assertRegistryIsSingleton() {
        Assert.assertSame("WebComponentConfigurationRegistry instance should be singleton", registry,
    ...
    public void setConfigurations_allCanBeFoundInRegistry() {
        Assert.assertTrue("Registry should have accepted the webComponents",
                registry.setConfigurations(createConfigurations(MyComponentExporter.class, UserBoxExporter.class)));

        Assert.assertEquals("Expected two targets to be registered", 2, registry.getConfigurations().size());

        Assert.assertEquals("Tag 'my-component' should have returned " + "'WebComponentBuilder' matching MyComponent",
                MyComponent.class, registry.getConfiguration(MY_COMPONENT_TAG).get().getComponentClass());
        Assert.assertEquals("Tag 'user-box' should have returned 'WebComponentBuilder' " + "matching UserBox",
                UserBox.class, registry.getConfiguration(USER_BOX_TAG).get().getComponentClass());
    }
    ...
    public void setConfigurations_getConfigurationsCallDoesNotChangeSetProtection() {
        ...
        WebComponentConfiguration<? extends Component> conf1 = registry.getConfiguration("my-component").get();

        Assert.assertNotNull(conf1);

        Assert.assertFalse(registry.setConfigurations(createConfigurations(UserBoxExporter.class)));

        WebComponentConfiguration<? extends Component> conf2 = registry.getConfiguration("my-component").get();
    ...
    public void getWebComponentConfigurationsForComponent() {
        registry.setConfigurations(
                createConfigurations(MyComponentExporter.class, MyComponentExporter2.class, UserBoxExporter.class));

        Set<WebComponentConfiguration<MyComponent>> set = registry.getConfigurationsByComponentType(MyComponent.class);

        Assert.assertEquals("Set should contain two configurations", 2, set.size());

        Assert.assertTrue("Both configurations should have component class " + "MyComponent.class", set.stream()
                .map(WebComponentConfiguration::getComponentClass).allMatch(clazz -> clazz.equals(MyComponent.class)));
    }
    ...
    public void setConfigurationsTwice_onlyFirstSetIsAccepted() {
        ...
        Set<WebComponentConfiguration<? extends Component>> configs2nd = createConfigurations(UserBoxExporter.class);

        Assert.assertTrue("Registry should have accepted the configurations", registry.setConfigurations(configs1st));

        Assert.assertFalse("Registry should not accept a second set of configurations.",
                registry.setConfigurations(configs2nd));

        Assert.assertEquals("Builders from the first Set should have been added", MyComponent.class,
                registry.getConfiguration("my" + "-component").get().getComponentClass());

        Assert.assertFalse("Components from the second Set should not have been added",
    ...
    public void hasConfigurations() {
        ...
        Assert.assertTrue("Should have configurations, when 3 were set", registry.hasConfigurations());
    }

    @Test
    public void hasConfigurations_noConfigurations() {
        Assert.assertFalse("New registry should have no configurations", registry.hasConfigurations());

        registry.setConfigurations(Collections.emptySet());

        Assert.assertFalse("Should not have configurations, when empty set is" + " given",
    ...
    public void setSameRouteValueFromDifferentThreads_ConcurrencyTest()
        ...
        List<Callable<AtomicBoolean>> callables = IntStream.range(0, THREADS).mapToObj(i -> {
            Callable<AtomicBoolean> callable = () -> {
                // Add random sleep for better possibility to run at
                // same time
                Thread.sleep(new Random().nextInt(200));
                return new AtomicBoolean(registry.setConfigurations(createConfigurations(MyComponentExporter.class)));
            };
            return callable;
        }).collect(Collectors.toList());

        List<Future<AtomicBoolean>> futures = executorService.invokeAll(callables);

        executorService.shutdown();

        Assert.assertEquals("Expected a result for all threads", THREADS, futures.size());
        ...
        Assert.assertEquals("Expected all except one thread to return false", THREADS - 1,

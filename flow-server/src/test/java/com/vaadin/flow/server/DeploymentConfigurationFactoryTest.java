...
public class DeploymentConfigurationFactoryTest {
        ...
        public void servletWithEnclosingUI_hasItsNameInConfig() throws Exception {
                ...
                DeploymentConfiguration config = new DeploymentConfigurationFactory()
                                .createDeploymentConfiguration(servlet, createVaadinConfigMock(servletConfigParams,
        ...
        public void servletWithNoEnclosingUI_hasDefaultUiInConfig() throws Exception {
                ...
                DeploymentConfiguration config = new DeploymentConfigurationFactory().createDeploymentConfiguration(
                                servlet, createVaadinConfigMock(servletConfigParams, emptyMap()));
        ...
        public void servletConfigParametersOverrideServletContextParameters() throws Exception {
                ...
                DeploymentConfiguration config = new DeploymentConfigurationFactory().createDeploymentConfiguration(
                                servlet, createVaadinConfigMock(servletConfigParams, servletContextParams));
        ...
        @Test
        public void shouldNotThrow_noTokenFile_correctWebPackConfigExists() throws Exception {
                Map<String, String> map = new HashMap<>();
                map.put(FrontendUtils.PROJECT_BASEDIR, temporaryFolder.getRoot().getAbsolutePath());

                File webPack = new File(temporaryFolder.getRoot().getAbsolutePath(), FrontendUtils.WEBPACK_CONFIG);
                FileUtils.writeLines(webPack, Arrays.asList("./webpack.generated.js"));

                new DeploymentConfigurationFactory().createDeploymentConfiguration(VaadinServlet.class,
        ...
        public void should_readConfigurationFromTokenFile() throws Exception {
                ...
                DeploymentConfiguration config = createConfig(
                                Collections.singletonMap(PARAM_TOKEN_FILE, tokenFile.getPath()));
        ...
        public void shouldThrow_tokenFileContainsNonExistingNpmFolderInDevMode() throws Exception {
                ...
                FileUtils.writeLines(tokenFile,
                                Arrays.asList("{", "\"productionMode\": false,", "\"npmFolder\": \"npm\",",
        ...
        public void createInitParameters_valuesAreTakenFromservletConfigAndTokenFile_valuesFromTokenFileOverridenByServletConfig()
                ...
                Set<String> stringParams = new HashSet<>(Arrays.asList(InitParameters.UI_PARAMETER,
                                ...
                                InitParameters.COMPILED_WEB_COMPONENTS_PATH, InitParameters.BUILD_FOLDER));
                ...
                for (int i = 0; i < initParamFields.length; i++) {
                        ...
                        if (!stringParams.contains(paramName)) {
                                ...
                                if (paramName.equals(InitParameters.SERVLET_PARAMETER_PRODUCTION_MODE)) {
                                        Assert.assertEquals(InitParameters.SERVLET_PARAMETER_PRODUCTION_MODE
        ...
        public void createInitParameters_tokenFileIsSetViaContext_externalStatsUrlIsReadFromTokenFile_predefinedProperties()
                ...
                Assert.assertEquals(Boolean.FALSE.toString(),
                                parameters.get(InitParameters.SERVLET_PARAMETER_ENABLE_DEV_SERVER));
        }
        ...
        private VaadinConfig mockTokenFileViaContextParam(String content) throws IOException {
                ...
                Mockito.when(config.getConfigParameterNames()).thenReturn(
                                Collections.enumeration(Collections.singleton(FrontendUtils.PARAM_TOKEN_FILE)));
        ...
        public void externalStatsFileTrue_predefinedValuesAreNotOverridden() throws Exception {
                ...
                FileUtils.writeLines(tokenFile, Arrays.asList("{", "\"enableDevServer\": true,",
        ...
        public void createInitParameters_fallbackChunkIsCreatedViaAppConfig_fallbackChunkObjectIsInInitParams()
                ...
                Properties properties = new DeploymentConfigurationFactory().createInitParameters(Object.class,
        ...
        public void createInitParameters_servletConfigDefinesTokenFile_fallbackChunkObjectIsInInitParams()
                ...
                Files.write(tokenFile.toPath(), Collections.singletonList("{ 'chunks': { " + "'fallback': {"
                                + "            'jsModules': ['foo', 'bar'],"
        ...
        public void createInitParameters_initParamtersAreSet_tokenDevModePropertiesAreNotSet() throws Exception {
                ...
                config.getInitParameters().setProperty(InitParameters.SERVLET_PARAMETER_ENABLE_PNPM,
                                Boolean.FALSE.toString());
                config.getInitParameters().setProperty(InitParameters.REQUIRE_HOME_NODE_EXECUTABLE,
                                Boolean.FALSE.toString());
                ...
                Assert.assertEquals(Boolean.FALSE.toString(), config.getInitParameters()
                                .getProperty(InitParameters.SERVLET_PARAMETER_DEVMODE_OPTIMIZE_BUNDLE));
        }
        ...
        private VaadinConfig createVaadinConfigMock(Map<String, String> servletConfigParameters,
                ...
                return new VaadinServletConfig(
                                createServletConfigMock(servletConfigParameters, servletContextParameters));
        }
        ...
        private ApplicationConfiguration mockApplicationConfiguration() {
                ...
                expect(configuration.getStringProperty(EasyMock.anyString(), EasyMock.anyString())).andReturn(null)
                                .anyTimes();
                expect(configuration.isXsrfProtectionEnabled()).andReturn(false).anyTimes();

                expect(configuration.getPropertyNames()).andReturn(Collections.emptyEnumeration()).anyTimes();

                fallbackChunk = mock(FallbackChunk.class);

                expect(configuration.getFallbackChunk()).andReturn(fallbackChunk).anyTimes();
        ...
        private ServletConfig createServletConfigMock(Map<String, String> servletConfigParameters,
                ...
                expect(contextMock.getAttribute(ApplicationConfiguration.class.getName())).andReturn(appConfiguration)
                                .anyTimes();

                expect(contextMock.getInitParameterNames())
                                .andAnswer(() -> Collections.enumeration(servletContextParameters.keySet())).anyTimes();
                ...
                expect(contextMock.getInitParameter(capture(initParameterNameCapture)))
                                .andAnswer(() -> servletContextParameters.get(initParameterNameCapture.getValue()))
                                .anyTimes();
                ...
                expect(provider.getApplicationResources(VAADIN_SERVLET_RESOURCES + TOKEN_FILE))
                                .andAnswer(() -> Collections.emptyList()).anyTimes();

                replay(provider);

                expect(contextMock.getAttribute(Lookup.class.getName())).andAnswer(() -> lookup).anyTimes();

                Capture<String> resourceCapture = EasyMock.newCapture();
                expect(contextMock.getResource(capture(resourceCapture))).andReturn(null).anyTimes();
                replay(contextMock);

...
public class SessionRouteRegistryTest {
    ...
    private SessionRouteRegistry getRegistry(VaadinSession session) {
        try {
            session.lock();
            return (SessionRouteRegistry) SessionRouteRegistry.getSessionRegistry(session);
        } finally {
            session.unlock();
        }
    }
    ...
    public void addSameClassForMultipleRoutes_removalOfRouteClassClearsRegisttry() {
        ...
        Assert.assertTrue("Registry didn't contain navigation targets even though some were registered",
                !registry.getRegisteredRoutes().isEmpty());

        registry.removeRoute(MyRoute.class);

        Assert.assertFalse("Registry should be empty as only one class was registered",
                !registry.getRegisteredRoutes().isEmpty());
    }
    ...
    public void addMultipleClassesToSameRoute_removeClassLeavesRoute() {
        ...
        Assert.assertEquals("No parameters route class was expected for only path String.", MyRoute.class,
                registry.getNavigationTarget("home").get());
        Assert.assertEquals("No parameters route class was expected for empty segments.", MyRoute.class,
                registry.getNavigationTarget("home", Collections.emptyList()).get());
        Assert.assertEquals("Expected HasRouteParameters class for request with segments.", Parameter.class,
                registry.getNavigationTarget("home", Arrays.asList("param")).get());

        registry.removeRoute(MyRoute.class);

        Assert.assertTrue("Registry is empty even though we should have one route available",
                !registry.getRegisteredRoutes().isEmpty());
        Assert.assertFalse("MyRoute should have been removed from the registry.",
                registry.getTargetUrl(MyRoute.class).isPresent());
        Assert.assertTrue("Parameter class should have been available from the registry",
                registry.getTargetUrl(Parameter.class, HasUrlParameterFormat.getParameters("foo")).isPresent());
        Assert.assertTrue("Parameter class should have been available from the registry",
                registry.getTemplate(Parameter.class).isPresent());
        Assert.assertEquals("Parameter route should have been available.", Parameter.class,
                registry.getNavigationTarget("home", Arrays.asList("param")).get());
    }
    ...
    public void sessionRegistryOverridesParentRegistryForGetTargetUrl_globalRouteStillAccessible() {
        ...
        Assert.assertEquals("Expected session registry route to be returned", "alternate",
    ...
    public void sessionRegistryOverridesParentRegistryWithOwnClass_globalRouteReturnedAfterClassRemoval() {
        ...
        Assert.assertEquals("Route 'MyRoute' should return Secondary as registered to SessionRegistry.",
                Secondary.class, sessionRegistry.getNavigationTarget("MyRoute").get());

        sessionRegistry.removeRoute(Secondary.class);

        Assert.assertEquals("Route 'MyRoute' should return MyRoute as registered to GlobalRegistry.", MyRoute.class,
    ...
    public void registerRouteWithAliases_routeAliasesRegisteredAsExpected() {
        ...
        Assert.assertTrue("Main route was not registered for given Class.",
                sessionRegistry.getNavigationTarget("MyRoute").isPresent());
        Assert.assertTrue("RouteAlias 'info' was not registered for given Class.",
                sessionRegistry.getNavigationTarget("info").isPresent());
        Assert.assertTrue("RouteAlias 'version' was not registered for given Class.",
    ...
    public void routesWithParentLayouts_parentLayoutReturnsAsExpected() {
        ...
        Assert.assertFalse("'MyRoute' should have a single parent",
                sessionRegistry.getNavigationRouteTarget("MyRoute").getRouteTarget().getParentLayouts().isEmpty());
        Assert.assertTrue("'info' should have no parents.",
                sessionRegistry.getNavigationRouteTarget("info").getRouteTarget().getParentLayouts().isEmpty());
        Assert.assertEquals("'version' should return two parents", 2,
                sessionRegistry.getNavigationRouteTarget("version").getRouteTarget().getParentLayouts().size());
    }
    ...
    public void registeredParentLayouts_changingListDoesntChangeRegistration() {
        ...
        Assert.assertEquals("'version' should return two parents even when original list is changed", 2,
                registry.getNavigationRouteTarget("version").getRouteTarget().getParentLayouts().size());
    }
    ...
    public void registeredParentLayouts_returnedListInSameOrder() {
        ...
        Assert.assertArrayEquals("Registry should return parent layouts in the same order as set.",
                parentChain.toArray(),
                registry.getNavigationRouteTarget("version").getRouteTarget().getParentLayouts().toArray());
    }
    ...
    public void routeRegisteredOnMultiplePaths_removalOfDefaultPathUpdatesDefaultPath() {
        ...
        Assert.assertEquals("MyRoute", sessionRegistry.getTargetUrl(MyRouteWithAliases.class).get());

        sessionRegistry.removeRoute("MyRoute");

        Assert.assertFalse("Route 'MyRoute' was still available even though it should have been removed.",
                sessionRegistry.getNavigationTarget("MyRoute").isPresent());
        Assert.assertTrue("Route 'info' has been removed eve though it should still be available",
                sessionRegistry.getNavigationTarget("info").isPresent());
        Assert.assertTrue("Route 'version' has been removed eve though it should still be available",
                sessionRegistry.getNavigationTarget("version").isPresent());

        Assert.assertTrue("Route was not found from the registry anymore even though it should be available.",
                sessionRegistry.getTargetUrl(MyRouteWithAliases.class).isPresent());

        // Either or is expected as the new default as first match is picked
        // from the map
        Assert.assertTrue("Route didn't return a url matching either of the expected aliases.", Arrays
                .asList("info", "version").contains(sessionRegistry.getTargetUrl(MyRouteWithAliases.class).get()));
    }
    ...
    public void manuallyRegisteredAliases_RouteDataIsReturnedCorrectly() {
        ...
        List<RouteData> registeredRoutes = sessionRegistry.getRegisteredRoutes();
        ...
        Assert.assertEquals("Two 'RouteAlias'es should be registered in the collected route data.", 2,
        ...
        Assert.assertTrue("Registry should still contain the alias routes", !registeredRoutes.isEmpty());
    ...
    public void registeredRouteWithAliasGlobally_sessionRegistryReturnsFromGlobal() {
        ...
        Assert.assertTrue("Path for main route 'MyRoute' returned empty",
                sessionRegistry.getNavigationTarget("MyRoute").isPresent());
        Assert.assertTrue("RouteAlias 'info' returned empty.", sessionRegistry.getNavigationTarget("info").isPresent());
    ...
    public void registeredRouteWithAliasGlobally_sessionRegistryOverridesMainUrl() {
        ...
        Assert.assertTrue("Registry didn't contain routes.", !sessionRegistry.getRegisteredRoutes().isEmpty());

        Assert.assertTrue("Path for main route 'MyRoute' returned empty",
                sessionRegistry.getNavigationTarget("MyRoute").isPresent());
        Assert.assertEquals("Navigation target for route 'MyRoute' was not the expected one.", Secondary.class,
                sessionRegistry.getNavigationTarget("MyRoute").get());
    ...
    public void setSameRouteValueFromDifferentThreads_ConcurrencyTest()
        ...
        List<Callable<Result>> callables = IntStream.range(0, THREADS).mapToObj(i -> {
        ...
        Assert.assertEquals("Expected 4 route already exists exceptions due to route target validation", THREADS - 1,
                exceptions.size());
        ...
        Optional<Class<? extends Component>> myRoute = getRegistry(session).getNavigationTarget("MyRoute");
        Assert.assertTrue("MyRoute was missing from the session scope registry.", myRoute.isPresent());
    ...
    public void updateRoutesFromMultipleThreads_allRoutesAreRegistered()
        ...
        callables.add(() -> {
            try {
                getRegistry(session).setRoute("home", MyRoute.class, Collections.emptyList());
            } catch (Exception e) {
                return new Result(e.getMessage());
            }
            return new Result(null);
        ...
        callables.add(() -> {
            try {
                getRegistry(session).setRoute("info", MyRoute.class, Collections.emptyList());
            } catch (Exception e) {
                return new Result(e.getMessage());
            }
            return new Result(null);
        ...
        callables.add(() -> {
            try {
                getRegistry(session).setRoute("palace", MyRoute.class, Collections.emptyList());
            } catch (Exception e) {
                return new Result(e.getMessage());
            }
            return new Result(null);
    ...
    public void updateAndRemoveFromMultipleThreads_endResultAsExpected()
        ...
        callables.add(() -> {
            try {
                getRegistry(session).setRoute("modular", MyRoute.class, Collections.emptyList());
            } catch (Exception e) {
                return new Result(e.getMessage());
            }
            return new Result(null);
    ...
    public void settingSessionRouteRegistryOfAnotherSession_getRegistryFails() {
        ...
        Assert.assertNotEquals("Another session should receive another session", registry, anotherRegistry);
    ...
    public void routeChangeListener_correctChangesAreReturned() {
        ...
        Assert.assertEquals("One route should have gotten removed", 1, removed.size());
    ...
    public void routeChangeListener_blockChangesAreGivenCorrectlyInEvent() {
        ...
        Assert.assertEquals("One MyRoute should have been removed", MyRoute.class,
    ...
    public void maskedPathsInParent_eventContainsOnlyChangesVisibleForSession() {
        ...
        Assert.assertTrue("No event for masked path should have been received.", events.isEmpty());
        ...
        Assert.assertEquals("Addition of non masked path should have fired an event.", 1, events.size());
        Assert.assertEquals("Source should have been ApplicationRouteRegistry", registry, events.get(0).getSource());
        Assert.assertEquals("One route should have been added", 1, events.get(0).getAddedRoutes().size());
        Assert.assertEquals("No routes should have been removed", 0, events.get(0).getRemovedRoutes().size());
    }
    ...
    public void serialize_deserialize_parentRegistryIsANewOne() throws Throwable {
        ...
        TestSessionRouteRegistry registry = new TestSessionRouteRegistry(session);

        TestSessionRouteRegistry deserialized = serializeAndDeserialize(registry);

        VaadinService service = new TestService();
        RouteRegistry newAppRegistry = service.getRouteRegistry();

        Mockito.when(newAppRegistry.getNavigationTarget("foo", Collections.emptyList()))
        ...
        Assert.assertEquals(Optional.empty(), registry.getNavigationTarget("foo", Collections.emptyList()));
        // The deserialized one (after refreshing transients) contains "foo"
        // navigation target
        Assert.assertEquals(Optional.of(HtmlContainer.class),
                deserialized.getNavigationTarget("foo", Collections.emptyList()));
    ...
    public void sessionScopeContainsTemplateRoute_applicationRegistryExactMatchIsReturned() {
        registry.setRoute(":first/:second", Templated.class, Collections.emptyList());
        registry.setRoute("other/view", NonTemplated.class, Collections.emptyList());

        SessionRouteRegistry sessionRegistry = getRegistry(session);
        Assert.assertEquals("ApplicationRegisty Templated should be found.", Templated.class,
                sessionRegistry.getNavigationTarget("oh/my").get());
        Assert.assertEquals("ApplicationRegistry NonTemplated should be found", NonTemplated.class,
                sessionRegistry.getNavigationTarget("other/view").get());

        sessionRegistry.setRoute(":one/:two", Secondary.class, Collections.emptyList());

        Assert.assertEquals("SessionRegistry should override ApplicationRegistry Templated", Secondary.class,
                sessionRegistry.getNavigationTarget("oh/my").get());

        Assert.assertEquals(
                "ApplicationRegistry exact match should be returned instead of SessionRegistry wildcard match",
                NonTemplated.class, sessionRegistry.getNavigationTarget("other/view").get());
        ...
        Assert.assertEquals(
                ...
                NonTemplated.class, sessionRegistry.getNavigationTarget("other/view").get());
        Assert.assertEquals("SessionRegistry best match with least wildcards should be returned", MyRoute.class,
    ...
    public void sessionScopeContainsTemplateRoute_applicationRegistryBetterMatchIsReturned() {
        registry.setRoute("other/view/parent", NonTemplated.class, Collections.emptyList());
        registry.setRoute("other/alias/:extra?", Templated.class, Collections.emptyList());
        ...
        Assert.assertEquals("MyRoute should be selected as the matching parts are equal", MyRoute.class,
                sessionRegistry.getNavigationTarget("other/view/offset").get());
        Assert.assertEquals("Exact macth in ApplicationRegistry should be selected", NonTemplated.class,
                sessionRegistry.getNavigationTarget("other/view/parent").get());
        Assert.assertEquals("Closer macth in ApplicationRegistry should be selected", Templated.class,
    ...
    private static class TestService extends VaadinServletService {
        ...
        private RouteRegistry appRegistry = Mockito.mock(ApplicationRouteRegistry.class);

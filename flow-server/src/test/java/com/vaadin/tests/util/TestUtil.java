...
public class TestUtil {
    public static void assertArrays(Object[] actualObjects, Object[] expectedObjects) {
        Assert.assertEquals("Actual contains a different number of values than was expected", expectedObjects.length,
    ...
    public static void assertIterableEquals(Iterable<?> iterable1, Iterable<?> iterable2) {
        ...
        if (i2.hasNext()) {
            Assert.fail("The second iterable contains more items than the first. The object " + i2.next()
                    + " has no match in the first iterable.");
        }
    }

    /**
     * Checks whether a weak reference is garbage collected. This methods also tries to force collection of the
     * reference by doing a few iterations of {@link System#gc()}.
     *
     * @param ref
     *            the weak reference to check
     * @return <code>true</code> if the reference has been collected, <code>false</code> if the reference is still
     *         reachable
    ...
    public static void createStubFileInFrontend(File projectRootFolder, String stubFileName) throws IOException {
        try (InputStream indexStream = IndexHtmlRequestHandlerTest.class.getClassLoader()
            ...
            File indexHtml = new File(new File(projectRootFolder, DEFAULT_FRONTEND_DIR), stubFileName);
    ...
    public static void createStatsJsonStub(File projectRootFolder) throws IOException {
        String content = "{\"npmModules\": {}, " + "\"entryScripts\": [\"foo.js\"], " + "\"packageJsonHash\": \"42\","
                + "\"indexHtmlGenerated\": []}";
    ...
    public static void createStylesCssStubInBundle(File projectRootFolder, String themeName, String content)
        ...
        createStubFile(projectRootFolder,
                "target/" + Constants.DEV_BUNDLE_LOCATION + "/assets/themes/" + themeName + "/styles.css", content);
    }
    ...
    public static void createThemeJs(File projectRootFolder) throws IOException {
        ...
        createStubFile(projectRootFolder, FrontendUtils.DEFAULT_FRONTEND_DIR + "generated/" + THEME_IMPORTS_NAME,
                content);
    }

    public static void createStyleCssStubInFrontend(File projectRootFolder, String themeName, String content)
            throws IOException {
        createStubFile(projectRootFolder, FrontendUtils.DEFAULT_FRONTEND_DIR + "themes/" + themeName + "/styles.css",
                content);
    }

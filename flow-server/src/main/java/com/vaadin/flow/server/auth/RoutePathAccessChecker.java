 ...
 * <p>
 * The check is performed by a pluggable {@link AccessPathChecker} on the actual event location path, without
 * considering if it is the route main path or an alias. Therefore, the provided {@link AccessPathChecker} should be
 * configured to handle both route main paths and aliases.
 * <p>
 * An instance of this class should be provided to a {@link NavigationAccessControl} added as a
 * {@link com.vaadin.flow.router.BeforeEnterListener} to the {@link com.vaadin.flow.component.UI} of interest.
 *
...
public class RoutePathAccessChecker implements NavigationAccessChecker {
    ...
    public AccessCheckResult check(NavigationContext context) {
        ...
        boolean hasPathAccess = accessPathChecker.hasAccess(path, context.getPrincipal(), context::hasRole);
        LOGGER.debug("Access to view '{}' with path '{}' is {}", context.getNavigationTarget().getName(), path,

...
public class DevModeInitializer implements ServletContainerInitializer,
    ...
    private static final Set<String> APPLICABLE_CLASS_NAMES = Collections
        .unmodifiableSet(DevModeClassFinder.calculateApplicableClassNames());
    ...
    private void ensureImplementation(Class<?> clazz) {
      if (!DevModeClassFinder.APPLICABLE_CLASS_NAMES.contains(clazz.getName())) {
  ...
  private static final Pattern DIR_REGEX_FRONTEND_DEFAULT = Pattern.compile(
      "^(?:file:)?(.+)" + Constants.RESOURCES_FRONTEND_DEFAULT + "/?$"); // allow trailing slash

  private static final Pattern DIR_REGEX_COMPATIBILITY_FRONTEND_DEFAULT = Pattern
      .compile("^(?:file:)?(.+)"
          + Constants.COMPATIBILITY_RESOURCES_FRONTEND_DEFAULT + "/?$"); // allow trailing slash
  ...
  public static void initDevModeHandler(Set<Class<?>> classes,
    ...
    DevModeInitializer.log().info("Starting dev-mode updaters in {} folder.",
        builder.npmFolder);
    ...
    Set<File> frontendLocations = DevModeInitializer.getJarFilesFromClassloader(
  ...
  protected static Set<File> getJarFilesFromClassloader(
    ...
    jarFiles.addAll(DevModeInitializer.getFrontendLocationsFromClassloader(classLoader,
  ...
  private static Set<File> getFrontendLocationsFromClassloader(
    ...
    try {
      ...
      while (en.hasMoreElements()) {
        ...
        Matcher jarWildfly11Matcher = DevModeInitializer.VFS_FILE_REGEX_WILDFLY11.matcher(url.toString());
        if (jarWildfly11Matcher.find()){
          
          jarFiles.add(getWildfly11FrontendFolder(resourcesFolder, url));
        } else {
          String path = URLDecoder.decode(url.getPath(),
              StandardCharsets.UTF_8.name());
          
          Matcher jarMatcher = DevModeInitializer.JAR_FILE_REGEX.matcher(path);
          Matcher dirMatcher = DevModeInitializer.DIR_REGEX_FRONTEND_DEFAULT.matcher(path);
          Matcher dirCompatibilityMatcher =
              DevModeInitializer.DIR_REGEX_COMPATIBILITY_FRONTEND_DEFAULT.matcher(path);
          if (jarMatcher.find()) {
  ...
  private static File getWildfly11FrontendFolder(String resourcesFolder, URL url) {
    try {
      ...
      rootDirAbsPath = rootDirAbsPath.substring(0, rootDirAbsPath.length() - resourcesFolder.length() - 1);
      return new File(rootDirAbsPath);
    } catch (IOException e) {
      throw new UncheckedIOException(e);
    } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException exc) {
      throw new RuntimeException(exc); // TODO: possibly use a less generic unchecked exception
    }
  }
}

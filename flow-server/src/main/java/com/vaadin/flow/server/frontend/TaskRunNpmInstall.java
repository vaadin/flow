...
public class TaskRunNpmInstall implements FallibleCommand {
    ...
    private JsonObject generateVersionsFromPackageJson() throws IOException {
        ...
        versionsJson.put(DEPENDENCIES, Json.createObject());
        versionsJson.put(DEV_DEPENDENCIES, Json.createObject());
        ...
        if (dependencies != null) {
            for (String key : dependencies.keys()) {
                versionsJson.getObject(DEPENDENCIES)
                        .put(key, dependencies.getString(key));
            }
        }
        ...
        if (devDependencies != null) {
            for (String key : devDependencies.keys()) {
                versionsJson.getObject(DEV_DEPENDENCIES)
                        .put(key, devDependencies.getString(key));
            }
        }

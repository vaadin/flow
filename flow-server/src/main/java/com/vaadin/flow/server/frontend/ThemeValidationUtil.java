...
public class ThemeValidationUtil {
    ...
    public static boolean themeConfigurationChanged(Options options, JsonObject statsJson,
            FrontendDependenciesScanner frontendDependencies) {
        Map<String, JsonObject> themeJsonContents = new HashMap<>();

        if (options.getJarFiles() != null) {
            options.getJarFiles().stream().filter(File::exists).filter(file -> !file.isDirectory())
                    .forEach(jarFile -> getPackagedThemeJsonContents(jarFile, themeJsonContents));
        }

        Optional<String> maybeThemeName = Optional.ofNullable(frontendDependencies.getThemeDefinition())
                .map(ThemeDefinition::getName).filter(name -> !name.isBlank());
        Optional<JsonObject> projectThemeJson = maybeThemeName
                .flatMap(themeName -> ThemeUtils.getThemeJson(themeName, options.getFrontendDirectory()));
        ...
        if (projectThemeJson.isPresent()) {
            ...
            if (statsThemeJson.hasKey(projectThemeName)) {
                ...
                getLogger().info("Found newly added configuration for project theme '{}' in 'theme.json'.",
                        projectThemeName);
                return true;
            }

            collectThemeJsonContentsInFrontend(options, themeJsonContents, key, projectThemeJson.get());
        }

        for (Map.Entry<String, JsonObject> themeContent : themeJsonContents.entrySet()) {
            if (hasNewAssetsOrImports(statsThemeJson, themeContent)) {
                getLogger().info("Found new configuration for theme '{}' in 'theme.json'.", themeContent.getKey());
                return true;
            } else if (statsThemeJson.hasKey(themeContent.getKey())) {
                List<String> missedKeys = new ArrayList<>();
                JsonObject content = Json.parse(statsThemeJson.getString(themeContent.getKey()));
                if (!objectIncludesEntry(content, themeContent.getValue(), missedKeys)) {
                    getLogger().info(
    ...
    /**
     * Checks if theme has legacy Shadow DOM stylesheets in {@literal <theme>/components} folder and if their content
     * has changed.
     *
     * @param options
     *            Flow plugin options
     ...
     * @return {@literal true} if the theme has legacy Shadow DOM stylesheets, and they are not included on the
     *         application bundle, otherwise {@literal false}.
     */
    public static boolean themeShadowDOMStylesheetsChanged(Options options, JsonObject statsJson,
            FrontendDependenciesScanner frontendDependencies) {
        File frontendDirectory = options.getFrontendDirectory();
        // Scan the theme hierarchy and collect all <theme>/components folders
        Set<Path> themeComponentsDirs = Optional.ofNullable(frontendDependencies.getThemeDefinition())
                .map(ThemeDefinition::getName).filter(name -> !name.isBlank()).map(themeName -> {
                    Map<String, JsonObject> themeJsonContents = new HashMap<>();
                    ThemeUtils.getThemeJson(themeName, frontendDirectory)
                            .ifPresent(themeJson -> collectThemeJsonContentsInFrontend(options, themeJsonContents,
                                    themeName, themeJson));
                    return themeJsonContents.keySet().stream()
                            .map(name -> ThemeUtils.getThemeFolder(frontendDirectory, name))
                            .map(dir -> new File(dir, "components")).filter(File::exists).map(File::toPath)
        ...
        if (themeComponentsDirs != null) {
            ...
            if (statsJson.hasKey(FRONTEND_HASHES_KEY)) {
                ...
                Stream.of(json.keys())
                        ...
                        .filter(path -> themeComponentsDirs.stream()
                                .anyMatch(dir -> frontendDirectory.toPath().resolve(path).startsWith(dir)))
                        .forEach(key -> hashesWithNoComponentCssMatches.put(key, json.getString(key)));
            }

            List<String> themeComponentsCssFiles = new ArrayList<>();
            for (Path dir : themeComponentsDirs) {
                FileUtils.listFiles(dir.toFile(), new String[] { "css" }, true).stream()
                        .filter(themeFile -> isFrontendResourceChangedOrMissingInBundle(hashesWithNoComponentCssMatches,
                                frontendDirectory, themeFile))
                        .map(f -> frontendDirectory.toPath().relativize(f.toPath()).toString().replaceAll("\\\\", "/"))
                        .collect(Collectors.toCollection(() -> themeComponentsCssFiles));
            }
            ...
            if (!hashesWithNoComponentCssMatches.isEmpty()) {
                BundleValidationUtil.logChangedFiles(new ArrayList<>(hashesWithNoComponentCssMatches.keySet()),
    ...
    private static void collectThemeJsonContentsInFrontend(Options options, Map<String, JsonObject> themeJsonContents,
            String themeName, JsonObject themeJson) {
        ...
        if (parentThemeInFrontend.isPresent()) {
            ...
            Optional<JsonObject> parentThemeJson = ThemeUtils.getThemeJson(parentThemeName,
    ...
    static boolean objectIncludesEntry(JsonValue jsonFromBundle, JsonValue projectJson, Collection<String> missedKeys) {
        ...
        if (bundleJsonType == JsonType.NULL) {
            ...
            return compareArrays(missedKeys, jsonArrayFromBundle, jsonArrayFromProject);
            ...
            return compareObjects(missedKeys, jsonObjectFromBundle, projectJsonObject);
        } else {
    ...
    private static void getPackagedThemeJsonContents(File jarFileToLookup,
        ...
        if (jarContentsManager.containsPath(jarFileToLookup, Constants.RESOURCES_THEME_JAR_DEFAULT)) {
            List<String> themeJsons = jarContentsManager.findFiles(jarFileToLookup,
    ...
    private static boolean compareObjects(Collection<String> missedKeys, JsonObject jsonObjectFromBundle,
        ...
        for (String projectEntryKey : projectJsonObject.keys()) {
            ...
            if (projectEntry.getType() == JsonType.STRING && "parent".equals(projectEntryKey)) {
                continue;
            }
            ...
            for (String bundleEntryKey : jsonObjectFromBundle.keys()) {
                ...
                if (bundleEntry.getType() == projectEntry.getType()
                        && objectIncludesEntry(bundleEntry, projectEntry, missedKeys)) {
                    entryFound = true;
                    break;
                }
            }
    ...
    private static boolean compareArrays(Collection<String> missedKeys, JsonArray jsonArrayFromBundle,
            JsonArray jsonArrayFromProject) {
    ...
    private static boolean checkMissedKeys(Collection<String> missedKeys, JsonArray arrayIterating,
            JsonArray arrayComparing) {
        boolean allEntriesFound = true;
        ...
        for (int arrayComparingIndex = 0; arrayComparingIndex < arrayComparing.length(); arrayComparingIndex++) {
            ...
            for (int arrayIteratingIndex = 0; arrayIteratingIndex < arrayIterating.length(); arrayIteratingIndex++) {
                ...
                if (arrayIteratingEntry.getType() == arrayComparingEntry.getType()
                        && objectIncludesEntry(arrayIteratingEntry, arrayComparingEntry, missedKeys)) {
                    entryFound = true;
                    break;
                }
            }
    ...
    private static boolean isFrontendResourceChangedOrMissingInBundle(Map<String, String> bundledHashes,
            File frontendFolder, File frontendResource) {
        String relativePath = frontendFolder.toPath().relativize(frontendResource.toPath()).toString()
                .replaceAll("\\\\", "/");
        boolean presentInBundle = bundledHashes.containsKey(relativePath);
        if (presentInBundle) {
            final String contentHash;
            try {
                contentHash = BundleValidationUtil
                        .calculateHash(FileUtils.readFileToString(frontendResource, StandardCharsets.UTF_8));
            } catch (IOException e) {

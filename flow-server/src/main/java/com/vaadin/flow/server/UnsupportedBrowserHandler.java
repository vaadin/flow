...
/**
 * A {@link RequestHandler} that presents an informative page that the browser in use will not work.
 *
 * @since 1.0
 */
public class UnsupportedBrowserHandler implements RequestHandler {
    ...
    public boolean handleRequest(VaadinSession session, VaadinRequest request, VaadinResponse response)
            throws IOException {
        if (!HandlerHelper.isRequestType(request, RequestType.BROWSER_TOO_OLD)) {
            return false;
        }
        ...
        if (lookup != null) {
            ...
            if (resourceProvider != null) {
                final URL applicationBrowserTooOldPage = resourceProvider.getApplicationResource(BROWSER_TOO_OLD_HTML);
                if (applicationBrowserTooOldPage != null) {
                    IOUtils.copy(applicationBrowserTooOldPage, response.getOutputStream());
                    return true;
                }
            }
        }

        IOUtils.copy(getClass().getResourceAsStream(BROWSER_TOO_OLD_HTML), response.getOutputStream());
        return true;
    }

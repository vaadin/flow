...
/**
 * RPC handler for events triggered through <code>element.$server</code> or simply <code>$server</code> in template
 * event handlers.
 * <p>
...
public class PublishedServerEventHandlerRpcHandler extends AbstractRpcInvocationHandler {
    ...
    public Optional<Runnable> handleNode(StateNode node, JsonObject invocationJson) {
        ...
        if (invocationJson.hasKey(JsonConstants.RPC_TEMPLATE_EVENT_PROMISE)) {
            promiseId = (int) invocationJson.getNumber(JsonConstants.RPC_TEMPLATE_EVENT_PROMISE);
        } else {
            promiseId = -1;
        }
        ...
        if (!component.isPresent()) {
            throw new IllegalStateException("Unable to handle RPC template event JSON message: "
        ...
        if (!execute) {
            ...
            if (clientDelegate.hasHandler(methodName)) {
                execute = DisabledUpdateMode.ALWAYS.equals(clientDelegate.getDisabledUpdateMode(methodName));
            }
            if (eventHandlers.hasHandler(methodName)) {
                execute = execute || DisabledUpdateMode.ALWAYS.equals(eventHandlers.getDisabledUpdateMode(methodName));
            }
        }

        if (execute) {
            invokeMethod(component.get(), component.get().getClass(), methodName, (JsonArray) args, promiseId,
                    node.isInert());
        }
    ...
    static void invokeMethod(Component instance, Class<?> clazz, String methodName, JsonArray args, int promiseId) {
    ...
    static void invokeMethod(Component instance, Class<?> clazz, String methodName, JsonArray args, int promiseId,
        ...
        if (method.isPresent()) {
            ...
            invokeMethod(compositeContent, compositeContent.getClass(), methodName, args, promiseId, inert);
        } else {
    ...
    private static Optional<Method> findMethod(Component instance, Class<?> clazz, String methodName) {
        List<Method> methods = Stream.of(clazz.getDeclaredMethods())
                .filter(method -> methodName.equals(method.getName())).filter(method -> hasMethodAnnotation(method))
    ...
    private static void invokeMethod(Component instance, Method method, JsonArray args, int promiseId, boolean inert) {
        ...
        if (promiseId == -1) {
            ...
            try {
                Serializable returnValue = (Serializable) invokeMethod(instance, method, args);

                instance.getElement().executeJs(
                        "this.$server['" + JsonConstants.RPC_PROMISE_CALLBACK_NAME + "']($0, true, $1)",
                        Integer.valueOf(promiseId), returnValue);
            } catch (RuntimeException e) {
                instance.getElement().executeJs(
                        "this.$server['" + JsonConstants.RPC_PROMISE_CALLBACK_NAME + "']($0, false)",
    ...
    private static Object invokeMethod(Component instance, Method method, JsonArray args) {
        try {
            ...
            LoggerFactory.getLogger(PublishedServerEventHandlerRpcHandler.class.getName()).debug(null, e);
        ...
        if (method.isVarArgs()) {
            if (clientValuesCount >= methodArgs - 1) {
                ...
                String msg = String.format(
                        ...
                        argsFromClient.length(), method.getName(), method.getDeclaringClass().getName(),
            ...
            if (methodArgs == clientValuesCount) {
                ...
                String msg = String.format(
                        "The number of received values (%d) is not equal "
                                + "to the number of arguments (%d) in the method '%s' " + "declared in '%s'",
                        argsFromClient.length(), method.getParameterCount(), method.getName(),
    ...
    private static JsonArray unwrapVarArgs(JsonArray argsFromClient, Method method) {
        ...
        if (argsFromClient.length() == paramCount) {
            if (argsFromClient.get(paramCount - 1).getType().equals(JsonType.ARRAY)) {
    ...
    private static Object decodeArg(Component instance, Method method, Class<?> type, int index, JsonValue argValue) {
        ...
        if (type.isPrimitive() && argValue.getType() == JsonType.NULL) {
            ...
            if (instance != null) {
                ...
                DeprecatedPolymerPublishedEventHandler handler = context.getAttribute(Lookup.class)
                ...
                if (handler != null && handler.isTemplateModelValue(instance, argValue, convertedType)) {
                    return handler.getTemplateItem(instance, (JsonObject) argValue,
            ...
            String msg = String.format(
                    "Class '%s' has the method '%s' " + "whose parameter %d refers to unsupported type '%s'",
                    method.getDeclaringClass().getName(), method.getName(), index, type.getName());
            throw new IllegalArgumentException(msg);
        }
    ...
    private static Object decodeArray(Method method, Class<?> type, int index, JsonValue argValue) {
        if (argValue.getType() != JsonType.ARRAY) {
            String msg = String.format(
                    "Class '%s' has the method '%s' " + "whose parameter %d refers to the array type '%s' "
        ...
        for (int i = 0; i < array.length(); i++) {
            Array.set(result, i, decodeArg(null, method, componentType, index, array.get(i)));
        }
        return result;
    }
    ...
    private static Logger getLogger() {
        return LoggerFactory.getLogger(PublishedServerEventHandlerRpcHandler.class.getName());
    }
}

 ...
 * <p>
 * <b>WARNING:</b> This class is intended for internal usage only. May be renamed or removed in a future release.
 *
 * @author Vaadin Ltd
 *
 */
public class FrontendTools {
     ...
     * {@link #SUPPORTED_NODE_VERSION}, i.e.
    ...
    public static final String NPM_BIN_PATH = FrontendUtils.isWindows() ? "node/node_modules/npm/bin/"
    ...
    private static final String PNPM_NOT_FOUND = MSG_PREFIX
            ...
            + "%nor exclude 'pnpm.global' from the configuration or set it to false." + MSG_SUFFIX;
    private static final String BUN_NOT_FOUND = MSG_PREFIX
            ...
            + "%nInstall bun by following the instruction at https://bun.sh " + MSG_SUFFIX;
    ...
    private static final String BAD_VERSION = MSG_PREFIX
            ...
            + "%n  - by following the https://nodejs.org/en/download/ guide to install it globally" + "%s"
    ...
    static final FrontendVersion SUPPORTED_NODE_VERSION = new FrontendVersion(SUPPORTED_NODE_MAJOR_VERSION,
            SUPPORTED_NODE_MINOR_VERSION);

    private static final FrontendVersion SUPPORTED_NPM_VERSION = new FrontendVersion(SUPPORTED_NPM_MAJOR_VERSION,
    ...
    private static final FrontendVersion SUPPORTED_PNPM_VERSION = new FrontendVersion(SUPPORTED_PNPM_MAJOR_VERSION,
            SUPPORTED_PNPM_MINOR_VERSION);
    private static final FrontendVersion SUPPORTED_BUN_VERSION = new FrontendVersion(1, 0, 6); // Bun 1.0.6 is the first
                                                                                               // version with
                                                                                               // "overrides" support

    private enum BuildTool {
        NPM("npm", "npm-cli.js"), NPX("npx", "npx-cli.js"), PNPM("pnpm", null), BUN("bun", null);
        ...
        String getScript() {
            if (script == null) {
                throw new RuntimeException(String.format("'%s' build tool doesn't have a CLI script", name));
            }
            return script;
        }
    }
    ...
    /**
     * Creates an instance of the class using the {@code baseDir} as a base directory to locate the tools and the
     * directory returned by the {@code alternativeDirGetter} as a directory to install tools if they are not found and
     * use it as an alternative tools location.
     * <p>
     ...
     * @deprecated use {@link FrontendTools#FrontendTools(FrontendToolsSettings)} instead, as it simplifies configuring
     *             the frontend tools and gives the default values to configuration parameters.
     */
    @Deprecated
    public FrontendTools(String baseDir, Supplier<String> alternativeDirGetter, boolean forceAlternativeNode) {
        this(baseDir, alternativeDirGetter, DEFAULT_NODE_VERSION, URI.create(Platform.guess().getNodeDownloadRoot()),
     ...
     *            <code>"v16.0.0"</code>. Use {@value #DEFAULT_NODE_VERSION} by default.
     * @param nodeDownloadRoot
     *            Download Node.js from this URL. Handy in heavily firewalled corporate environments where the Node.js
     *            download can be provided from an intranet mirror. Use
     *            {@link NodeInstaller#DEFAULT_NODEJS_DOWNLOAD_ROOT} by default.
     * @deprecated use {@link FrontendTools#FrontendTools(FrontendToolsSettings)} instead, as it simplifies configuring
     *             the frontend tools and gives the default values to configuration parameters.
     */
    @Deprecated
    public FrontendTools(String baseDir, Supplier<String> alternativeDirGetter, String nodeVersion,
     ...
     *            force usage of node executable from alternative directory
     * @param useGlobalPnpm
     *            use globally installed pnpm instead of the default one (see {@link #DEFAULT_PNPM_VERSION})
     * @deprecated use {@link FrontendTools#FrontendTools(FrontendToolsSettings)} instead, as it simplifies configuring
     *             the frontend tools and gives the default values to configuration parameters.
     */
    @Deprecated
    public FrontendTools(String baseDir, Supplier<String> alternativeDirGetter, String nodeVersion,
            URI nodeDownloadRoot, boolean forceAlternativeNode, boolean useGlobalPnpm) {
    ...
    @Deprecated
    public FrontendTools(String baseDir, Supplier<String> alternativeDirGetter, String nodeVersion,
            URI nodeDownloadRoot, boolean forceAlternativeNode, boolean useGlobalPnpm, boolean autoUpdate) {
        this(baseDir, alternativeDirGetter, nodeVersion, nodeDownloadRoot,
                "true".equalsIgnoreCase(System.getProperty(FrontendUtils.PARAM_IGNORE_VERSION_CHECKS)),
                forceAlternativeNode, useGlobalPnpm, autoUpdate);
    }

    FrontendTools(String baseDir, Supplier<String> alternativeDirGetter, String nodeVersion, URI nodeDownloadRoot,
            boolean ignoreVersionChecks, boolean forceAlternativeNode, boolean useGlobalPnpm, boolean autoUpdate) {
    ...
    private static FrontendToolsSettings createSettings(ApplicationConfiguration applicationConfiguration,
            File projectRoot) {
        boolean useHomeNodeExec = applicationConfiguration
                .getBooleanProperty(InitParameters.REQUIRE_HOME_NODE_EXECUTABLE, false);
        boolean nodeAutoUpdate = applicationConfiguration.getBooleanProperty(InitParameters.NODE_AUTO_UPDATE, false);
        boolean useGlobalPnpm = applicationConfiguration
                .getBooleanProperty(InitParameters.SERVLET_PARAMETER_GLOBAL_PNPM, false);
        final String nodeVersion = applicationConfiguration.getStringProperty(NODE_VERSION,
                FrontendTools.DEFAULT_NODE_VERSION);
        final String nodeDownloadRoot = applicationConfiguration.getStringProperty(NODE_DOWNLOAD_ROOT,
                Platform.guess().getNodeDownloadRoot());

        FrontendToolsSettings settings = new FrontendToolsSettings(projectRoot.getAbsolutePath(),
    ...
    public String getNodeExecutable() {
        ...
        if (file == null && !forceAlternativeNode) {
            file = frontendToolsLocator.tryLocateTool(nodeCommands.getFirst()).orElse(null);
        }
        file = rejectUnsupportedNodeVersion(file);
        if (file == null) {
            file = updateAlternateIfNeeded(getExecutable(getAlternativeDir(), nodeCommands.getSecond()));
        }
        if (file == null && alternativeDirGetter != null) {
            getLogger().info("Couldn't find {}. Installing Node and npm to {}.", nodeCommands.getFirst(),
                    getAlternativeDir());
     ...
     * <p>
     * Also update is {@code auto.update} flag set and installed version is older than the current default version.
     *
     * @param file
     *            node executable
    ...
    private File updateAlternateIfNeeded(File file) {
        ...
        try {
            ...
            final FrontendVersion installedNodeVersion = FrontendUtils.getVersion("node", versionCommand);

            boolean installDefault = false;
            final FrontendVersion defaultVersion = new FrontendVersion(nodeVersion);
            if (installedNodeVersion.isOlderThan(SUPPORTED_NODE_VERSION)) {
                getLogger().info("Updating unsupported node version {} to {}", installedNodeVersion.getFullVersion(),
                        defaultVersion.getFullVersion());
                installDefault = true;
            } else if (autoUpdate && installedNodeVersion.isOlderThan(defaultVersion)) {
                getLogger().info("Updating current installed node version from {} to {}",
                        installedNodeVersion.getFullVersion(), defaultVersion.getFullVersion());
                installDefault = true;
            }
            if (installDefault) {
                file = new File(installNode(nodeVersion, nodeDownloadRoot));
     ...
     *
     * Returns the input executable if version is supported, otherwise {@literal null}.
     *
        ...
        try {
            ...
            if (installedNodeVersion.isOlderThan(SUPPORTED_NODE_VERSION)) {
                getLogger().info(
                        ...
                        installedNodeVersion.getFullVersion(), SUPPORTED_NODE_VERSION.getFullVersion(),
     ...
     *
     * <p>
     * The difference between {@link #getNodeExecutable()} and this method in a search algorithm:
     * {@link #getNodeExecutable()} first searches executable in the base/alternative directory and fallbacks to the
     * globally installed if it's not found there. The {@link #forceAlternativeNodeExecutable()} doesn't search for
     * globally installed executable. It tries to find it in the installation directory and if it's not found it
     * downloads and installs it there.
     *
    ...
    public String forceAlternativeNodeExecutable() {
        ...
        if (file.exists()) {
            ...
            getLogger().info("Node not found in {}. Installing node {}.", dir, nodeVersion);
     ...
     *
     * @return the list of all commands in sequence that need to be executed to have npm running
     */
     ...
     *
     * @return the list of all commands in sequence that need to be executed to have pnpm running
     */
     ...
     *
     * @return the list of all commands in sequence that need to be executed to have bun running
     */
    ...
    /**
     * Validate that the found node and npm versions are new enough. Throws an exception with a descriptive message if a
     * version is too old.
     */
    public void validateNodeAndNpmVersion() {
        ...
        try {
            ...
            getLogger().debug("Using node {} located at {}", foundNodeVersion.getFullVersion(),
            ...
            if (foundNodeVersionAndExe != null) {
                getLogger().info("Validated node from '{}'", foundNodeVersionAndExe.getSecond());
            }
            throw ise;
        }
        ...
        try {
            ...
            getLogger().debug("Using npm {} located at {}", foundNpmVersion.getFullVersion(),
    ...
    private Pair<FrontendVersion, String> getNodeVersionAndExecutable() throws UnknownVersionException {
     ...
     * @param downloadRoot
     *            optional download root for downloading node. May be a filesystem file or a URL see
    ...
    protected String installNode(String nodeVersion, URI downloadRoot) {
        NodeInstaller nodeInstaller = new NodeInstaller(new File(getAlternativeDir()), getProxies())
        ...
        return new File(nodeInstaller.getInstallDirectory(), getNodeCommands().getFirst()).toString();
    }
    ...
    /**
     * Checks whether the currently installed npm version accepts/properly processes the path to a given folder.
     * <p>
     * For example, the older versions of npm don't accept whitespaces in folders path.
     *
     * @param folder
     *            the folder to check.
     * @return <code>true</code>, if the current version of npm accepts the given folder path, <code>false</code> if it
     *         causes issues.
     */
    boolean folderIsAcceptableByNpm(File folder) {
        Objects.requireNonNull(folder);
        boolean hidden = folder.isHidden() || folder.getPath().contains(File.separator + ".");
        ...
        if (FrontendUtils.isWindows() && folder.getAbsolutePath().matches(".*[\\s+].*")) {
            try {
                ...
                return foundNpmVersion.isEqualOrNewer(WHITESPACE_ACCEPTING_NPM_VERSION);
            } catch (UnknownVersionException e) {
                getLogger().warn("Error checking if npm accepts path '{}'", folder, e);
            }
        }
        return true;
    }
    ...
    /**
     * Gives a file object representing path to the cache directory of currently installed npm.
     *
     ...
     */
    File getNpmCacheDir() throws CommandExecutionException, IllegalStateException {
        ...
        if (output.isEmpty()) {
            throw new IllegalStateException(
                    String.format("Command '%s' returned an empty path", String.join(" ", npmCacheCommand)));
        }
    ...
    public FrontendVersion getNpmVersion() throws UnknownVersionException {
        List<String> npmVersionCommand = new ArrayList<>(getNpmExecutable(false));
        npmVersionCommand.add("--version"); // NOSONAR
        return FrontendUtils.getVersion("npm", npmVersionCommand);
    }

    /**
     * Returns flags required to pass to Node for Webpack to function. Determine whether webpack requires Node.js to be
     * started with the --openssl-legacy-provider parameter. This is a webpack 4 workaround of the issue
     * https://github.com/webpack/webpack/issues/14532 See: https://github.com/vaadin/flow/issues/12649
     *
     * @return the flags
     */
    public Map<String, String> getWebpackNodeEnvironment() {
        Map<String, String> environment = new HashMap<>();
        ProcessBuilder processBuilder = new ProcessBuilder().command(getNodeExecutable(), "-p",
    ...
    private List<String> getNpmExecutable(boolean removePnpmLock) {
        List<String> returnCommand = new ArrayList<>(getNpmCliToolExecutable(BuildTool.NPM));
    ...
    private List<String> getNpmCliToolExecutable(BuildTool cliTool, String... flags) {
        ...
        List<String> returnCommand = getNpmScriptCommand(baseDir, cliTool.getScript());
        ...
        if (returnCommand.isEmpty() && forceAlternativeNode) {
            ...
            returnCommand = getNpmScriptCommand(getAlternativeDir(), cliTool.getScript());
            alternativeDirChecked = true;
        }
        if (returnCommand.isEmpty()) {
            // Otherwise look for regular `npm`/`npx` global search path
            Optional<String> command = frontendToolsLocator.tryLocateTool(cliTool.getCommand())
                    .map(File::getAbsolutePath);
            if (command.isPresent()) {
                returnCommand = Collections.singletonList(command.get());
                if (!alternativeDirChecked && cliTool.equals(BuildTool.NPM)) {
                    try {
                        List<String> npmVersionCommand = new ArrayList<>(returnCommand);
                        npmVersionCommand.add("--version"); // NOSONAR
                        final FrontendVersion npmVersion = FrontendUtils.getVersion("npm", npmVersionCommand);
    ...
    List<String> getSuitablePnpm() {
        ...
        if (useGlobalPnpm) {
            ...
            pnpmCommand = frontendToolsLocator.tryLocateTool(BuildTool.PNPM.getCommand()).map(File::getAbsolutePath)
                    .map(Collections::singletonList)
                    .orElseThrow(() -> new IllegalStateException(String.format(PNPM_NOT_FOUND)));
            pnpmCommand = Stream.of(pnpmCommand).filter(this::validatePnpmVersion).findFirst()
            ...
            pnpmCommand = getNpmCliToolExecutable(BuildTool.NPX, "--yes", "--quiet", "pnpm");
    ...
    List<String> getSuitableBun() {
        ...
        bunCommand = frontendToolsLocator.tryLocateTool(BuildTool.BUN.getCommand()).map(File::getAbsolutePath)
                .map(Collections::singletonList)
                .orElseThrow(() -> new IllegalStateException(String.format(BUN_NOT_FOUND)));
        bunCommand = Stream.of(bunCommand).filter(this::validateBunVersion).findFirst()
                .orElseThrow(() -> new IllegalStateException(
    ...
    private boolean validatePnpmVersion(List<String> pnpmCommand) {
        ...
        try {
            ...
            if (!versionAccepted) {
                getLogger().warn("pnpm '{}' is version {} which is not supported (expected >={})", commandLine,
                        pnpmVersion.getFullVersion(), SUPPORTED_PNPM_VERSION.getFullVersion());
            }
    ...
    private boolean validateBunVersion(List<String> bunCommand) {
        ...
        try {
            ...
            boolean versionAccepted = ignoreVersionChecks || versionNewEnough;
            if (!versionAccepted) {
                getLogger().warn("bun '{}' is version {} which is not supported (expected >={})", commandLine,
                        bunVersion.getFullVersion(), SUPPORTED_BUN_VERSION.getFullVersion());
            }
            return versionAccepted;
        } catch (UnknownVersionException e) {
            getLogger().warn("version check '{}' failed", commandLine, e);
     ...
     *
     * The return value can be used when executing node commands, as the first part of a process builder command.
     *

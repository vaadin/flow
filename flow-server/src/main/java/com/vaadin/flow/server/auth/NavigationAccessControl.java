...
/**
 * A {@link BeforeEnterListener} implementation that contains logic to perform access checks before entering a view.
 * <p>
 * </p>
 *
 * Access rules are defined by providing one or more {@link NavigationAccessChecker} instances, that are responsible for
 * deciding if a navigation should be allowed or not. The final navigation grant decision is taken by a pluggable
 * {@link AccessCheckDecisionResolver} component, based on the results of all access checkers.
 * <p>
 * </p>
 * If access is allowed, the navigation continues to the target component. Otherwise, for not authenticated requests, a
 * redirect is performed to the login page (if configured) or to the {@literal not found} error view.
 * <p>
 * </p>
 * In development mode, the access check failure reason is forwarded to the {@literal not found} error view, for
 * debugging purpose. In addition, an exception will be thrown if the decision resolver determines the navigation should
 * be rejected because of misconfigurations.
 * <p>
 * </p>
 * In production mode, for security reasons, the failure message is never exposed and rejection is treated as a normal
 * deny, without any exception being thrown.
 * <p>
 * </p>
 * Before redirecting to the login page, the route path and its absolute URL are stored in the HTTP session, to allow
 * the authentication logic to access the requested resource ( {@link #SESSION_STORED_REDIRECT},
 * {@link #SESSION_STORED_REDIRECT_ABSOLUTE} ).
 * <p>
 * </p>
 * The default constructor create an instance pre-configured with {@link AnnotatedViewAccessChecker}.
 * <p>
 * </p>
...
public class NavigationAccessControl implements BeforeEnterListener {
     ...
     */
    public static final String SESSION_STORED_REDIRECT_ABSOLUTE = NavigationAccessControl.class.getName()
            + ".redirectAbsolute";
     ...
     */
    public NavigationAccessControl(Collection<NavigationAccessChecker> checkerList,
     ...
     *
     * @param enabled
     *            {@literal true} to enable access control, {@literal false} to disable the checks and allow navigation
     *            to all routes.
     */
     ...
     *
     * @return {@literal true} if the access control is enabled, otherwise {@literal false}.
     */
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        NavigationContext context = new NavigationContext(event, getPrincipal(request), getRolesChecker(request));
        AccessCheckResult result = checkAccess(context, isProductionMode(event));
        if (result.decision() != AccessCheckDecision.ALLOW) {
            if (context.getPrincipal() == null) {
                ...
                event.rerouteToError(
                        getAccessDeniedException(AccessAnnotationChecker.securityTarget(event.getNavigationTarget())),
                        result.reason());
            }
        }
    }
    ...
    public AccessCheckResult checkAccess(NavigationContext context, boolean productionMode) {
        ...
        if (!enabled) {
            getLogger().trace("Navigation to view {} allowed because navigation access control is disable",
                    navigationTarget.getName());
            return context.allow();
        }

        if (checkerList.isEmpty()) {
            getLogger().debug("Navigation to view {} allowed because there are no navigation checkers configured",
                    navigationTarget.getName());
            return context.allow();
        }
        ...
        if (loginView != null && navigationTarget == loginView) {
            ...
            return context.allow();
        } else if (loginUrl != null && PathUtil.trimPath(loginUrl).equals(context.getLocation().getPath())) {
            getLogger().debug("Allowing access for login URL {}", loginUrl);
            return AccessCheckResult.allow();
        }

        List<AccessCheckResult> results = checkerList.stream().map(checker -> checker.check(context)).toList();
        AccessCheckResult decision = decisionResolver.resolve(results, context);
        getLogger().debug("Decision against {} checker results: {}", results.size(), decision);
     ...
     *
     * The returned URL contains a protocol, server name, port number, and server path, but it does not include query
     * string parameters.
     *
     * Returns an empty string if the URL cannot be extracted from the current request.
     *
     * @param vaadinRequest
     *            current request
    ...
    private void storeRedirectURL(BeforeEnterEvent event, VaadinRequest request) {
        WrappedSession session = request != null ? request.getWrappedSession() : null;
    ...
    public boolean hasAccessChecker(Class<? extends NavigationAccessChecker> type) {
        return checkerList.stream().anyMatch(checker -> type.isAssignableFrom(checker.getClass()));
    }
    ...
    public NavigationContext createNavigationContext(Class<?> navigationTarget, String path,
        ...
        return new NavigationContext(vaadinService.getRouter(), navigationTarget, new Location(path),
                RouteParameters.empty(), vaadinRequest.getUserPrincipal(), getRolesChecker(vaadinRequest), false);
    }
}

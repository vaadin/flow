...
public class FrontendUtils {
    ...
    private static File getExecutable(String cmd, String defaultLocation) {
        File file = null;
        try {
            file = defaultLocation == null ? frontendToolsLocator.tryLocateTool(cmd).get()
                    : Optional.of(defaultLocation == null ? null : new File(getBaseDir(), defaultLocation))
                            .filter(frontendToolsLocator::verifyTool)
                            .orElseGet(() -> frontendToolsLocator.tryLocateTool(cmd).get());

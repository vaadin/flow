...
public class FrontendUtils {
    ...
    private static InputStream getFileFromSnowpack(String filePath)
        ...
        if ("index.html".equalsIgnoreCase(filePath)) {
             ...
             * index.html template by HtmlWebpackPlugin. The location of the
             * index.html template file is configured independently, and
             * therefore it's possible to keep it in /frontend yet still serve
             * via /index.html (not /frontend/index.html).
             * 
             * Snowpack does not have a separate config option to specify a
             * custom index.html file path, and it's resolved according to the
             * same rules as everything else, effectively having the URL
             * 'VAADIN/index.html' (which is resolved to frontend/index.html
             * according to snowpack dev server mount point configuration)
             */

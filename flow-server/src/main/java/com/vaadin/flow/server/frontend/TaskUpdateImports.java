...
public class TaskUpdateImports extends NodeUpdater {
    ...
    private final String IMPORT = "import '%s';";
    ...
    private boolean importedFileExists(String jsImport) {
        return  // NOSONAR
                // file is in /frontend
                new File(frontendDirectory, jsImport).isFile()
                // file is in /node_modules/ ?
                || new File(nodeModulesFolder, jsImport).isFile()
                // file is in /node_modules/@vaadin/flow-frontend ?
                || new File(new File(nodeModulesFolder, FLOW_NPM_PACKAGE_NAME), jsImport).isFile()
                // file is generated by flow ?
                || new File(generatedFolder, generatedResourcePathIntoRelativePath(jsImport)).isFile()
                // has a package.json file /node_modules/package-name/package.json ?
                || new File(nodeModulesFolder, jsImport + "/" + PACKAGE_JSON ).isFile();
    }

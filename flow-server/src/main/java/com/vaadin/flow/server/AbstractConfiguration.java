...
public interface AbstractConfiguration extends Serializable {
    ...
    default boolean frontendHotdeploy() {
        ...
        return getBooleanProperty(InitParameters.FRONTEND_HOTDEPLOY, FrontendUtils.isHillaUsed(getFrontendFolder()));
    }

    default File getFrontendFolder() {
        String frontendFolderPath = getStringProperty(FrontendUtils.PARAM_FRONTEND_DIR,
        ...
        return FrontendUtils.getLegacyFrontendFolderIfExists(getProjectFolder(), frontend);
    }

    /**
     * Gets the mode the application is running in.
     *
     * @return custom production bundle, pre-compiled production bundle, development using livereload or development
     *         using bundle
     **/
    default Mode getMode() {
        if (isProductionMode()) {
            return BundleUtils.isPreCompiledProductionBundle() ? Mode.PRODUCTION_PRECOMPILED_BUNDLE
    ...
    /**
     * Get if the dev server should be reused on each reload. True by default, set it to false in tests so as dev server
    ...
    default boolean isPnpmEnabled() {
        return getBooleanProperty(InitParameters.SERVLET_PARAMETER_ENABLE_PNPM, Constants.ENABLE_PNPM_DEFAULT);
    }
    ...
    default boolean isBunEnabled() {
        return getBooleanProperty(InitParameters.SERVLET_PARAMETER_ENABLE_BUN, Constants.ENABLE_BUN_DEFAULT);
    }
     ...
     *
     * @return {@code true} if globally installed pnpm is used, {@code false} if the default one is used.
     */
    default boolean isGlobalPnpm() {
        return getBooleanProperty(InitParameters.SERVLET_PARAMETER_GLOBAL_PNPM, Constants.GLOBAL_PNPM_DEFAULT);
    }
    ...
    default boolean isXsrfProtectionEnabled() {
        return !getBooleanProperty(SERVLET_PARAMETER_DISABLE_XSRF_PROTECTION, false);
    }

    /**
     * Return the defined build folder for the used build system.
     * <p>
     * Default value is <code>target</code> used by maven and the gradle plugin will set it to <code>build</code>.
     *
     ...
     * Only available in development mode.
     *
     * @return the folder where resources are stored, typically {@code src/main/resources}.
     */
    default File getJavaResourceFolder() {
        File folder = new File(getStringProperty(Constants.JAVA_RESOURCE_FOLDER_TOKEN, "src/main/resources"));
     ...
     * Only available in development mode.
     *
     * @return the folder where source files are stored, typically {@code src/main/java}.
     */
    default File getJavaSourceFolder() {
        File folder = new File(getStringProperty(Constants.CONNECT_JAVA_SOURCE_FOLDER_TOKEN, "src/main/java"));
        if (!folder.isAbsolute()) {
            folder = new File(getProjectFolder(), folder.getPath());
        }
        return folder.getAbsoluteFile();

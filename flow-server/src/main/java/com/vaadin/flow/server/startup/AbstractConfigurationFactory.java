...
public class AbstractConfigurationFactory implements Serializable {
    ...
    protected Map<String, String> getConfigParametersUsingTokenData(JsonObject buildInfo) {
        ...
        if (buildInfo.hasKey(EXTERNAL_STATS_FILE_TOKEN) || buildInfo.hasKey(EXTERNAL_STATS_URL_TOKEN)) {
            ...
            if (buildInfo.hasKey(EXTERNAL_STATS_URL_TOKEN)) {
                params.put(EXTERNAL_STATS_URL, buildInfo.getString(EXTERNAL_STATS_URL_TOKEN));
            }
            // NO OTHER CONFIGURATION:
            return params;
        }
        if (buildInfo.hasKey(SERVLET_PARAMETER_INITIAL_UIDL)) {
            params.put(SERVLET_PARAMETER_INITIAL_UIDL,
                    String.valueOf(buildInfo.getBoolean(SERVLET_PARAMETER_INITIAL_UIDL)));
            // Need to be sure that we remove the system property,
            // because it has priority in the configuration getter
            System.clearProperty(VAADIN_PREFIX + SERVLET_PARAMETER_INITIAL_UIDL);
        }
        ...
        if (buildInfo.hasKey(NODE_DOWNLOAD_ROOT)) {
            params.put(NODE_DOWNLOAD_ROOT, buildInfo.getString(NODE_DOWNLOAD_ROOT));
        }

        if (buildInfo.hasKey(FRONTEND_TOKEN)) {
            params.put(FrontendUtils.PARAM_FRONTEND_DIR, buildInfo.getString(FRONTEND_TOKEN));
        ...
        if (buildInfo.hasKey(FRONTEND_HOTDEPLOY)) {
            params.put(FRONTEND_HOTDEPLOY, String.valueOf(buildInfo.getBoolean(FRONTEND_HOTDEPLOY)));
        } else if (buildInfo.hasKey(SERVLET_PARAMETER_ENABLE_DEV_SERVER)) {
            params.put(FRONTEND_HOTDEPLOY, String.valueOf(buildInfo.getBoolean(SERVLET_PARAMETER_ENABLE_DEV_SERVER)));
        }
        ...
        if (buildInfo.hasKey(CONNECT_JAVA_SOURCE_FOLDER_TOKEN)) {
            params.put(CONNECT_JAVA_SOURCE_FOLDER_TOKEN, buildInfo.getString(CONNECT_JAVA_SOURCE_FOLDER_TOKEN));
        }
        if (buildInfo.hasKey(Constants.JAVA_RESOURCE_FOLDER_TOKEN)) {
            params.put(Constants.JAVA_RESOURCE_FOLDER_TOKEN, buildInfo.getString(Constants.JAVA_RESOURCE_FOLDER_TOKEN));
        }
        if (buildInfo.hasKey(CONNECT_OPEN_API_FILE_TOKEN)) {
            params.put(CONNECT_OPEN_API_FILE_TOKEN, buildInfo.getString(CONNECT_OPEN_API_FILE_TOKEN));
        }
        if (buildInfo.hasKey(CONNECT_APPLICATION_PROPERTIES_TOKEN)) {
            params.put(CONNECT_APPLICATION_PROPERTIES_TOKEN, buildInfo.getString(CONNECT_APPLICATION_PROPERTIES_TOKEN));
        }
        if (buildInfo.hasKey(PROJECT_FRONTEND_GENERATED_DIR_TOKEN)) {
            params.put(PROJECT_FRONTEND_GENERATED_DIR_TOKEN, buildInfo.getString(PROJECT_FRONTEND_GENERATED_DIR_TOKEN));
        }
        ...
        if (buildInfo.hasKey(REACT_ENABLE)) {
            params.put(REACT_ENABLE, String.valueOf(buildInfo.getBoolean(REACT_ENABLE)));
        }
        if (buildInfo.hasKey(APPLICATION_IDENTIFIER)) {
            params.put(APPLICATION_IDENTIFIER, buildInfo.getString(APPLICATION_IDENTIFIER));
        }
        if (buildInfo.hasKey(DAU_TOKEN)) {
            params.put(DAU_TOKEN, String.valueOf(buildInfo.getBoolean(DAU_TOKEN)));
        }
        if (buildInfo.hasKey(PREMIUM_FEATURES)) {
            params.put(PREMIUM_FEATURES, String.valueOf(buildInfo.getBoolean(PREMIUM_FEATURES)));
        }
    ...
    protected void setDevModePropertiesUsingTokenData(Map<String, String> params, JsonObject buildInfo) {
        ...
        if (params.get(InitParameters.SERVLET_PARAMETER_ENABLE_BUN) == null
                && buildInfo.hasKey(InitParameters.SERVLET_PARAMETER_ENABLE_BUN)) {
            params.put(InitParameters.SERVLET_PARAMETER_ENABLE_BUN,
                    String.valueOf(buildInfo.getBoolean(InitParameters.SERVLET_PARAMETER_ENABLE_BUN)));
        }
    ...
    protected String getTokenFileContent(Function<String, String> locationProvider) {
        String location = locationProvider.apply(FrontendUtils.PARAM_TOKEN_FILE);
        String json = null;
    ...
    protected void verifyFolderExists(Map<String, String> params, String folder) {
        Boolean productionMode = Boolean.parseBoolean(params.getOrDefault(SERVLET_PARAMETER_PRODUCTION_MODE, "false"));

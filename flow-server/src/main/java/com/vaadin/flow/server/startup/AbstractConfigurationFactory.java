...
public class AbstractConfigurationFactory implements Serializable {
     ...
     * behind by an older Vaadin version, or packaged into a dependency by
     * mistake, carries a Node.js version that nobody has chosen for this
     * project. Honoring it would install an unsupported Node.js and fail the
     * frontend build with a version that is found nowhere in the project, so it
     * is ignored and the current default is used instead. A version configured
     * through the {@code vaadin.node.version} property is not affected, as it
     * does not come from the token file.
     *
     * @param version

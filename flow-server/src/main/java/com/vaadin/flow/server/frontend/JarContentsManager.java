...
public class JarContentsManager {
     ...
     * @return {@code true} if path is contained in the jar, {@code false} otherwise
     * @throws IllegalArgumentException
     *             if jar file specified is not a file or does not exist
     * @throws NullPointerException
     *             if any of the arguments is null
     * @throws UncheckedIOException
     *             if {@link IOException} occurs during file search, for instance, when jar file specified is not a jar
     *             file
     */
    public boolean containsPath(File jar, String filePath) {
        ...
        try (JarFile jarFile = new JarFile(jar, false)) {
            ...
            throw new UncheckedIOException(String.format("Failed to retrieve file '%s' from jar '%s'", filePath, jar),
                    e);
        }
    }

    /**
     * Tries to find a file by its path (case sensitive) in jar file. If found, its contents is returned, {@code null}
     * otherwise.
     *
     * @param jar
     *            jar file to look for file in, not {@code null}
     ...
     *             if any of the arguments is null
     * @throws UncheckedIOException
     *             if {@link IOException} occurs during file retrieval, for instance, when jar file specified is not a
     *             jar file
     */
     ...
     * @param fileName
     *            a string that should required files' paths end with, not {@code null}
     * @return list of files from the directory specified with required file names
     * @throws IllegalArgumentException
     *             if jar file specified is not a file or does not exist
     * @throws NullPointerException
     *             if any of the arguments is null
    ...
    public List<String> findFiles(File jar, String baseDirectoryName, String fileName) {
        ...
        try (JarFile jarFile = new JarFile(jar, false)) {
            return jarFile.stream().filter(entry -> !entry.isDirectory()).map(ZipEntry::getName)
                    ...
                    .filter(path -> path.endsWith(JAR_PATH_SEPARATOR + fileName)).collect(Collectors.toList());
        } catch (IOException e) {
    ...
    private byte[] getJarEntryContents(JarFile jarFile, JarEntry entry) {
        ...
        try (InputStream entryStream = jarFile.getInputStream(entry)) {
            ...
            throw new UncheckedIOException(
                    String.format("Failed to get entry '%s' contents from jar file '%s'", entry, jarFile), e);
        }
    }
     ...
     *            a path relative to jar root, only files from this path will be copied, can be {@code null}, which is
     *            treated as a root of the jar. Files will be copied relative to this path (i.e. only path part after
     *            this path is preserved in output directory)
     * @param outputDirectory
     *            the directory to copy files to, not {@code null}
     * @param wildcardPathExclusions
     *            wildcard exclusions that are used to check each path against before copying
     * @return names of the files that were either copied or already existed in the output directory
     * @throws IllegalArgumentException
     *             if jar file specified is not a file or does not exist or if output directory is not a directory or
     *             does not exist
     * @throws NullPointerException
     *             if jar file or output directory is {@code null}
     * @throws UncheckedIOException
     *             if {@link IOException} occurs during the operation, for instance, when jar file specified is not a
     *             jar file
     */
    public Set<String> copyFilesFromJarTrimmingBasePath(File jar, String jarDirectoryToCopyFrom, File outputDirectory,
            String... wildcardPathExclusions) {
        requireFileExistence(jar);

        if (!Objects.requireNonNull(outputDirectory).isDirectory()) {
            throw new IllegalArgumentException(
                    String.format("Expect '%s' to be an existing directory", outputDirectory));
        }

        String basePath = normalizeJarBasePath(jarDirectoryToCopyFrom);

        try (JarFile jarFile = new JarFile(jar, false)) {
            Set<String> handledFiles = jarFile.stream().filter(file -> !file.isDirectory())
                    .filter(file -> file.getName().toLowerCase(Locale.ENGLISH)
                            .startsWith(basePath.toLowerCase(Locale.ENGLISH)))
                    .filter(file -> isFileIncluded(file, wildcardPathExclusions))
                    .map(jarEntry -> copyJarEntryTrimmingBasePath(jarFile, jarEntry, basePath, outputDirectory))
                    .collect(Collectors.toSet());
            return handledFiles;
        } catch (IOException e) {
            throw new UncheckedIOException(
                    String.format("Failed to extract files from jarFile '%s' to directory '%s'", jar, outputDirectory),
                    e);
        }
    }
     ...
     *            wildcard inclusions that are used to check each path against before copying
     * @return names of the files that were either copied or already existed in the output directory
     * @throws IllegalArgumentException
     *             if jar file specified is not a file or does not exist or if output directory is not a directory or
     *             does not exist
     ...
     *             jar file
     */
    public Set<String> copyIncludedFilesFromJarTrimmingBasePath(File jar, String jarDirectoryToCopyFrom,
            File outputDirectory, String... wildcardPathInclusions) {
        requireFileExistence(jar);
    ...
    private boolean isFileIncluded(ZipEntry file, String... pathExclusions) {
        ...
        return Stream.of(pathExclusions)
                .noneMatch(exclusionRule -> FilenameUtils.wildcardMatch(filePath, exclusionRule));
    }

    private boolean includeFile(ZipEntry file, String... pathInclusions) {
        String filePath = file.getName();
        return Stream.of(pathInclusions)
                .anyMatch(inclusionRule -> FilenameUtils.wildcardMatch(filePath, inclusionRule));
    }

    private String copyJarEntryTrimmingBasePath(JarFile jarFile, ZipEntry jarEntry, String basePath,
            File outputDirectory) {
        String fullPath = jarEntry.getName();
        String relativePath = fullPath.substring(
                fullPath.toLowerCase(Locale.ENGLISH).indexOf(basePath.toLowerCase(Locale.ENGLISH)) + basePath.length());
        File target = new File(outputDirectory, relativePath);
        try {
            if (!target.exists() || !hasSameContent(jarFile.getInputStream(jarEntry), target)) {
                FileUtils.copyInputStreamToFile(jarFile.getInputStream(jarEntry), target);
            }
            return relativePath;
        } catch (IOException e) {
    ...
    private boolean hasSameContent(InputStream jarContent, File existingContent) throws IOException {
        try (InputStream existingContentStream = new FileInputStream(existingContent)) {

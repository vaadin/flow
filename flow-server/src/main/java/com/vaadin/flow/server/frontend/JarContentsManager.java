...
public class JarContentsManager {
     ...
     *
     * @param jar      jar file to look for file in, not {@code null}
     * @param filePath file path relative to jar root, not {@code null}
     * @return {@code true} if path is contained in the jar, {@code false} otherwise
     * @throws IllegalArgumentException if jar file specified is not a file or does not exist
     * @throws NullPointerException     if any of the arguments is null
     * @throws UncheckedIOException     if {@link IOException} occurs during file search, for instance, when jar file specified is not a jar file
     */
     ...
     * If found, its contents is returned, {@code null} otherwise.
     *
     * @param jar      jar file to look for file in, not {@code null}
     * @param filePath file path relative to jar root, not {@code null}
     ...
     * @throws IllegalArgumentException if jar file specified is not a file or does not exist
     * @throws NullPointerException     if any of the arguments is null
     * @throws UncheckedIOException     if {@link IOException} occurs during file retrieval, for instance, when jar file specified is not a jar file
     */
     ...
     *
     * @param jar               jar file to look for file in, not {@code null}
     * @param baseDirectoryName the directory to search in the jar, not {@code null}
     * @param fileName          a string that should required files' paths end with, not {@code null}
     * @return list of files from the directory specified with required file names
     * @throws IllegalArgumentException if jar file specified is not a file or does not exist
     * @throws NullPointerException     if any of the arguments is null
     * @throws UncheckedIOException     if {@link IOException} occurs during jar file search
     */
    public List<String> findFiles(File jar, String baseDirectoryName, String fileName) {
        ...
        try (JarFile jarFile = new JarFile(jar, false)) {
            return jarFile.stream()
                    ...
                    .filter(path -> path.endsWith(JAR_PATH_SEPARATOR + fileName))
     ...
     *
     * @param jar                    jar file to look for files in, not {@code null}
     * @param jarDirectoryToCopyFrom a path relative to jar root, only files from this path will be copied, can be {@code null}, which is treated as a root of the jar.
     *                               Files will be copied relative to this path (i.e. only path part after this path is preserved in output directory)
     * @param outputDirectory        the directory to copy files to, not {@code null}
     * @param wildcardPathExclusions wildcard exclusions that are used to check each path against before copying
     * @throws IllegalArgumentException if jar file specified is not a file or does not exist or if output directory is not a directory or does not exist
     * @throws NullPointerException     if jar file or output directory is {@code null}
    ...
    public void copyFilesFromJarTrimmingBasePath(File jar, String jarDirectoryToCopyFrom, File outputDirectory, String... wildcardPathExclusions) {
        ...
        try (JarFile jarFile = new JarFile(jar, false)) {
            jarFile.stream()
                    ...
                    .forEach(jarEntry -> copyJarEntryTrimmingBasePath(jarFile, jarEntry, basePath, outputDirectory));
        } catch (IOException e) {
     ...
     *                               Files will be copied relative to this path (i.e. only path part after this path is preserved in output directory)
     * @param outputDirectory        the directory to copy files to, not {@code null}
     * @param wildcardPathInclusions wildcard inclusions that are used to check each path against before copying
     * @throws IllegalArgumentException if jar file specified is not a file or does not exist or if output directory is not a directory or does not exist
     * @throws NullPointerException     if jar file or output directory is {@code null}
     * @throws UncheckedIOException     if {@link IOException} occurs during the operation, for instance, when jar file specified is not a jar file
     */
    ...
    private boolean isFileIncluded(ZipEntry file, String... pathExclusions) {
        ...
        return Stream.of(pathExclusions).noneMatch(exclusionRule -> FilenameUtils.wildcardMatch(filePath, exclusionRule));
    }


    private boolean includeFile(ZipEntry file, String... pathInclusions) {
        String filePath = file.getName();
        return Stream.of(pathInclusions).anyMatch(inclusionRule -> FilenameUtils.wildcardMatch(filePath, inclusionRule));
    }
    ...
    private void copyJarEntryTrimmingBasePath(JarFile jarFile, ZipEntry jarEntry, String basePath, File outputDirectory) {
        ...
        try {
            FileUtils.copyInputStreamToFile(jarFile.getInputStream(jarEntry), new File(outputDirectory, relativePath));
        } catch (IOException e) {

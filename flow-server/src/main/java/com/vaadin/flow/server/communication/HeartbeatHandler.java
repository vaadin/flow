...
/**
 * Handles heartbeat requests. Heartbeat requests are periodically sent by the client-side to inform the server that the
 * UI sending the heartbeat is still alive (the browser window is open, the connection is up) even when there are no
 * UIDL requests for a prolonged period of time. UIs that do not receive either heartbeat or UIDL requests are
 * eventually removed from the session and garbage collected.
 * <p>
...
public class HeartbeatHandler extends SynchronizedRequestHandler implements SessionExpiredHandler {
    ...
    /**
     * Handles a heartbeat request for the given session. Reads the GET parameter named
     * {@link ApplicationConstants#UI_ID_PARAMETER} to identify the UI. If the UI is found in the session, sets it
     * {@link UIInternals#getLastHeartbeatTimestamp() heartbeat timestamp} to the current time. Otherwise, writes a HTTP
     * Not Found error to the response.
     */
    @Override

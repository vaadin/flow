 ...
 * <ol>
 * <li>A security annotation (see below) is searched for on that particular method.</li>
 * <li>If a security annotation was not found on the method, checks the class the method is declared in.</li>
 * <li>If no security annotation was found, deny access by default</li>
 * </ol>
 * <p>
 * The security annotations checked and their meaning are:
 * <ul>
 * <li>{@link AnonymousAllowed} - allows access to any logged on or not logged in user. Public access.</li>
 * <li>{@link PermitAll} - allows access to any logged in user but denies access to anonymous users.</li>
 * <li>{@link RolesAllowed} - allows access there is a logged in user that has any of the roles mentioned in the
 * annotation</li>
...
public class AccessAnnotationChecker implements Serializable {
     ...
     * {@link HttpServletRequest#getUserPrincipal()} and {@link HttpServletRequest#isUserInRole(String)} has access to
     * the given method.
     *
     * @param method
     *            the method to check access to
     * @return {@code true} if the user has access to the given method, {@code false} otherwise
     */
    ...
    /**
     *
     * Checks if the user defined by the request (using {@link HttpServletRequest#getUserPrincipal()} and
     * {@link HttpServletRequest#isUserInRole(String)} has access to the given method.
     *
     * @param method
     *            the method to check access to
     * @param request
     ...
     *
     * Checks if the user defined by the request (using {@link HttpServletRequest#getUserPrincipal()} and
     * {@link HttpServletRequest#isUserInRole(String)} has access to the given class.
     *
     * @param cls
     *            the class to check access to
     * @param request
    ...
    /**
     * Checks if the user defined by the given {@link Principal} and role checker has access to the given method.
     *
     * @param method
     *            the method to check access to
     * @param principal
    ...
    /**
     * Checks if the user defined by the given {@link Principal} and role checker has access to the given class.
     *
     * @param cls
     *            the class to check access to
     * @param principal
     ...
     * @return the entity that is responsible for security settings for the method passed
     ...
     * @param cls
     *            the class to check
     * @return the first annotated class in {@code cls}'s hierarchy that annotated with one of the access annotations,
     *         starting from the input {@code cls} class itself, going up in the hierarchy. <em>Note:</em> interfaces in
     *         the {@code cls}'s hierarchy are ignored.
     *         <p>
     *         If no class in the hierarchy was annotated with any of the access annotations, the {@code cls} input
     *         parameter itself would be returned.
     *         <p>
    ...
    private boolean hasAccess(AnnotatedElement annotatedClassOrMethod, Principal principal,
    ...
    private static boolean hasSecurityAnnotation(AnnotatedElement method) {
        return method.isAnnotationPresent(AnonymousAllowed.class) || method.isAnnotationPresent(PermitAll.class)
                || method.isAnnotationPresent(DenyAll.class) || method.isAnnotationPresent(RolesAllowed.class);
    }

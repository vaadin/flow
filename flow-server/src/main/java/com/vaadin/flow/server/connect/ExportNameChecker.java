...
/**
 * A checker responsible for validating the Vaadin Connect export names.
 */
public class ExportNameChecker {
    /**
    ...
    /**
     * Validates the Vaadin Connect export name given.
     *
     * @param exportName
     ...
     */
    public String check(String exportName) {
        if (exportName == null || exportName.isEmpty()) {
            return "Export name cannot be blank";
        }
        if (ECMA_SCRIPT_RESERVED_WORDS.contains(exportName)) {
            return "Export name cannot be equal to JavaScript reserved words";
        }
        if (CONTAINS_WHITESPACE_PATTERN.matcher(exportName).matches()) {
            return "Export name cannot contain any whitespaces";
        }
        return null;
    }
}

 ...
 * {@link com.vaadin.flow.server.communication.AtmospherePushConnection.PushMessage}s to ensure that a message is
 * preserved in the {@link BroadcasterCache} until the client has received it.
 *
 * The filter acts only on {@literal LONG POLLING} transport and expects that the client sends the
 * {@literal X-Vaadin-LastSeenServerSyncId} header with the identifier of the last message seen, every time the
 * connection is established.
 *
 * Messages already seen are discarded, whereas messages not yet sent to the client are added again to the cache to
 * preserve them until client confirms reception by sending the last seen message identifier.
 */
public class LongPollingCacheFilter implements PerRequestBroadcastFilter, Serializable {
    ...
    static void onConnect(AtmosphereResource resource) {
        Integer syncId = Optional.ofNullable(resource.getRequest().getHeader(SEEN_SERVER_SYNC_ID))
    ...
    public BroadcastAction filter(String broadcasterId, AtmosphereResource r, Object originalMessage, Object message) {
        ...
        if (originalMessage instanceof AtmospherePushConnection.PushMessage
                && r.transport() == AtmosphereResource.TRANSPORT.LONG_POLLING && session != null
            ...
            String uuid = r.uuid();
            int lastSeenOnClient = session.getAttribute(SEEN_SERVER_SYNC_ID, Integer.class);
            if (lastSeenOnClient == -1) {
                return new BroadcastAction(BroadcastAction.ACTION.CONTINUE, message);
            } else if (pushMessage.alreadySeen(lastSeenOnClient)) {
                getLogger().trace("Discarding message {} for resource {} as client already seen {}. {}",
                        pushMessage.serverSyncId, uuid, lastSeenOnClient, pushMessage.message);

 ...
 * when there are {@link Export} annotations.
 *
 * @since 3.0
 */
@HandlesTypes({ Export.class })
public class ConnectExportsValidator
    ...
    public void onStartup(Set<Class<?>> classSet, ServletContext servletContext)
        ...
        if (classSet == null) {
            ...
            // a export and dependencies are not added to the project.
            return;
        }

        ClassFinder finder = new DefaultClassFinder(classSet);
        Set<Class<?>> exports = finder.getAnnotatedClasses(Export.class);
        if (!exports.isEmpty()) {
            try {
                finder.loadClass(classToCheck);
            } catch (ClassNotFoundException e) {
                throw new ServletException(
                        "ERROR: Vaadin Connect Exports only work for Spring "
                                + "enabled projects.\n"
                                + "This is not a spring application but there are connect exports in these classes: "
                                + exports.stream().map(clazz -> clazz.getName())
                                        .collect(
                                                Collectors.joining("\n    - ")),
                        e);
            }

...
/**
 * A registration for a return channel. A new return channel can be registered for a state node using
 * {@link ReturnChannelMap#registerChannel(com.vaadin.flow.function.SerializableConsumer)}. The registration can be
 * passed as a parameter to various <code>executeJavaScript</code> methods and will be represented on the client as a
 * function that calls the registered handler.
 * <p>
...
public interface ReturnChannelRegistration extends Registration {
     ...
     *
     * @param arguments
     *            a JSON array containing passed from the client, not <code>null</code>
     */
    void invoke(JsonArray arguments);

    /**
     * Gets the setting for whether this channel will receive updates in case the state node is disabled. By default,
     * updates are allowed only when the state node is enabled.
     *

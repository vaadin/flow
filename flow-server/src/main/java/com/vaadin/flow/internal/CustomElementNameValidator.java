...
/**
 * Validates custom-element name according to definition in
 * <a href= "https://www.w3.org/TR/custom-elements/#prod-potentialcustomelementname">Custom element name</a>
 * <p>
...
public final class CustomElementNameValidator {
    ...
    private static final Set<String> RESERVED_NAMES = Stream.of("annotation-xml", "color-profile", "font-face",
    ...
    public static boolean isCustomElementName(String name) {
        return name != null && checkHtmlTagRules(name) && checkWebComponentRules(name);
    }

    private static boolean checkHtmlTagRules(String name) {
        return !name.isEmpty() && name.equals(name.toLowerCase(Locale.ENGLISH)) && !name.startsWith("-")

...
public class RouteConfiguration implements Serializable {
    ...
    /**
     * Get a {@link RouteConfiguration} that edits the session scope routes. This requires that
    ...
    /**
     * Get a {@link RouteConfiguration} that edits the application scope routes. This requires that
    ...
    /**
     * Get a {@link RouteConfiguration} for editing the given RouteRegistry implementation. This enables editing of
     * registry when the required {@link CurrentInstance} is not yet populated.
     *
     * @param registry
     ...
     * <p>
     * Note! This would be best to request for session scope registry as it will then contain the actual currently
     * visible routes from both the session and application scopes.
     * <p>
    ...
    public boolean isPathAvailable(String path) {
        if (handledRegistry instanceof AbstractRouteRegistry) {
            return ((AbstractRouteRegistry) handledRegistry).getConfiguration().hasTemplate(path);
        }
        return getAvailableRoutes().stream().anyMatch(routeData -> routeData.getTemplate().equals(path) || routeData
                .getRouteAliases().stream().anyMatch(routeAliasData -> routeAliasData.getTemplate().equals(path)));
    }
    ...
    /**
     * Gets the registered route class for a given path. Returns an empty optional if no navigation target corresponds
     * to the given path.
     *
     * @param path
    ...
    public Optional<Class<? extends Component>> getRoute(String path, List<String> segments) {
        return handledRegistry.getNavigationTarget(PathUtil.getPath(path, segments));
    }
     ...
     * <p>
     * Using this method makes the registry changes made inside the command atomic for the registry as no one else can
     * change the state during the duration of the command.
     * <p>
     * Any other configuration thread for the same registry will be blocked until all the update locks have been
     * released.
     * <p>
     * Note! During an update other threads will get the pre-update state of the registry until the update has fully
     * completed.
     *
     * @param command
    ...
    /**
     * Giving a navigation target here will handle the {@link Route} annotation to get the path and also register any
     * {@link RouteAlias} that may be on the class.
     *
    ...
    public void setAnnotatedRoute(Class<? extends Component> navigationTarget) {
        if (!navigationTarget.isAnnotationPresent(Route.class)) {
            String message = String.format("Given navigationTarget %s is missing the '@Route' annotation.",
                    navigationTarget.getName());
            throw new InvalidRouteConfigurationException(message);
        }
        String route = RouteUtil.getRoutePath(handledRegistry.getContext(), navigationTarget);
        handledRegistry.setRoute(route, navigationTarget,
                RouteUtil.getParentLayouts(handledRegistry, navigationTarget, route));
        ...
        for (RouteAlias alias : navigationTarget.getAnnotationsByType(RouteAlias.class)) {
            ...
            handledRegistry.setRoute(path, navigationTarget,
                    RouteUtil.getParentLayouts(handledRegistry, navigationTarget, path));
        }
    }

    /**
     * Register a navigation target on the specified path. Any {@link ParentLayout} annotation on class will be used to
     * populate layout chain, but {@link Route} and {@link RouteAlias} will not be taken into consideration.
     *
     * @param path
    ...
    public void setParentAnnotatedRoute(String path, Class<? extends Component> navigationTarget) {
        handledRegistry.setRoute(path, navigationTarget, RouteUtil.getParentLayoutsForNonRouteTarget(navigationTarget));
    }

    /**
     * Register a navigation target with specified path and with no parent layouts.
     * <p>
     * Note! Any {@link ParentLayout}, {@link Route} or {@link RouteAlias} will be ignored in route handling.
     *
     * @param path
     *            path to register navigation target to
     * @param navigationTarget
    ...
    public void setRoute(String path, Class<? extends Component> navigationTarget,
        ...
        handledRegistry.setRoute(path, navigationTarget, Arrays.asList(parentChain));
    }

    /**
     * Remove the given navigation target route registration. Path where the navigation target was may still be usable,
     * e.g. we remove target with url param and there is left a non param target, but will not return the removed
     * target.
     * <p>
    ...
    /**
     * Remove all registrations for given path. This means that any navigation target registered on the given path will
     * be removed. But if a removed navigationTarget for the path exists it is then stored with a new main path so it
     * can still get a resolved url.
     * <p>
     ...
     * <p>
     * Note! If another path exists for the removed navigation target it will get a new main path so it can still get a
     * resolved url. The restored path will be the first found match for all paths that are registered.
     *
     * @param path
     ...
     * <p>
     * Note! If the navigation target has a url parameter that is required then this method will throw an
     ...
     * @param navigationTarget
     *            navigation target to get url for
     * @return optional url base without route parameters or empty if there is no registered route for
     *         {@code navigationTarget}, not {@code null}
     */
     ...
     * <p>
     * Note! Given parameter is checked for correct class type. This means that if the navigation target defined
     * parameter is of type {@code Boolean} then calling getUrl with a {@code String} will fail.
     *
     * @param navigationTarget
     *            navigation target to get url for
     * @param parameter
    ...
    public <T, C extends Component & HasUrlParameter<T>> String getUrl(Class<? extends C> navigationTarget,
            T parameter) {
        return getUrl(navigationTarget, HasUrlParameterFormat.getParameters(parameter));
    }

    /**
     * Get the url string for given navigation target with the parameters in the url.
     * <p>
     * Note! Given parameters are checked for correct class type. This means that if the navigation target defined
     * parameter is of type {@code Boolean} then calling getUrl with a {@code String} will fail.
     *
     * @param navigationTarget
     *            navigation target to get url for
    ...
    public <T, C extends Component & HasUrlParameter<T>> String getUrl(Class<? extends C> navigationTarget,
            List<T> parameters) {
        return getUrl(navigationTarget, HasUrlParameterFormat.getParameters(parameters));
    }
     ...
     * @param parameters
     *            route parameters.
     * @return the url which navigates to given navigationTarget using given parameters.
    ...
    public String getUrl(Class<? extends Component> navigationTarget, RouteParameters parameters) {
        ...
        Optional<String> targetUrl = handledRegistry.getTargetUrl(navigationTarget, parameters);
        if (!targetUrl.isPresent()) {
            throw new NotFoundException(
                    String.format("No route found for the given navigation target '%s' and parameters '%s'",
    ...
    @SafeVarargs
    private final boolean isAnnotatedParameter(Class<? extends Component> navigationTarget,
            Class<? extends Annotation>... parameterAnnotations) {
        for (Class<? extends Annotation> annotation : parameterAnnotations) {
            if (ParameterDeserializer.isAnnotatedParameter(navigationTarget, annotation)) {
                return true;
            }
        }
        return false;
     ...
     * checking depends on the active {@link VaadinService} and {@link VaadinRequest}.
     * <p>
    ...
    public List<RouteData> getRegisteredAccessibleMenuRoutes() {
        UI ui = UI.getCurrent();
        if (ui != null) {
            List<BeforeEnterListener> accessControls = ui.getInternals().getListeners(BeforeEnterHandler.class).stream()
                    .filter(BeforeEnterListener.class::isInstance).map(BeforeEnterListener.class::cast).toList();
     ...
     * @return list of accessible menu routes available for handled registry
     */
    public List<RouteData> getRegisteredAccessibleMenuRoutes(Collection<BeforeEnterListener> accessControls) {
        return getHandledRegistry().getRegisteredAccessibleMenuRoutes(VaadinRequest.getCurrent(), accessControls);
    }
}

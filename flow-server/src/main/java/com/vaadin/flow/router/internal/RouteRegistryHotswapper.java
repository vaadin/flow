 ...
 * <p>
 * </p>
 * This class is meant to be used in combination wit Flow {@link com.vaadin.flow.hotswap.Hotswapper} to immediately
 * update routes registries when classes have been added or modified.
 * <p>
 * </p>
...
public class RouteRegistryHotswapper implements VaadinHotswapper {
     ...
     * <ul>
     * <li>{@link Route} annotation removed: the previous route is removed from the registry</li>
    ...
    public boolean onClassLoadEvent(VaadinService vaadinService, Set<Class<?>> classes, boolean redefined) {
        ...
        if (hasComponentClasses(addedClasses, modifiedClasses, removedClasses)) {
            ...
            if (configuration.isReactEnabled() && Stream.of(addedClasses, modifiedClasses, removedClasses)
                    .flatMap(Set::stream).anyMatch(layouts::contains)) {

                Options options = new Options(vaadinService.getContext().getAttribute(Lookup.class), null,
                        configuration.getProjectFolder()).withFrontendDirectory(configuration.getFrontendFolder());
                TaskGenerateReactFiles.writeLayouts(options, ((AbstractRouteRegistry) appRegistry).getLayouts());
            }
        }
        return false;
    }
    ...
    @Override
    public boolean onClassLoadEvent(VaadinSession session, Set<Class<?>> classes, boolean redefined) {
        Set<Class<?>> addedClasses = redefined ? Set.of() : classes;
        Set<Class<?>> modifiedClasses = redefined ? classes : Set.of();
        Set<Class<?>> removedClasses = Set.of();
        if (session.getAttribute(SessionRouteRegistry.class) != null
                && hasComponentClasses(addedClasses, modifiedClasses, removedClasses)) {
            RouteUtil.updateRouteRegistry(SessionRouteRegistry.getSessionRegistry(session), Set.of(), modifiedClasses,
                    removedClasses);
        }
        return false;
    }
    ...
    private boolean hasComponentClasses(Set<Class<?>>... classes) {
        return Stream.of(classes).filter(classSet -> classSet != null && !classSet.isEmpty()).flatMap(Set::stream)

...
/**
 * A Component is a higher level abstraction of an {@link Element} or a hierarchy of {@link Element}s.
 * <p>
 * A component must have exactly one root element which is created based on the {@link Tag} annotation of the sub class
 * (or in special cases set using the constructor {@link #Component(Element)} or using
 * {@link #setElement(Component, Element)} before the element is attached to a parent). The root element cannot be
 * changed once it has been set.
 *
 * @author Vaadin Ltd
 * @since 1.0
 */
public abstract class Component implements HasStyle, AttachNotifier, DetachNotifier {

    /**
     * Encapsulates data required for mapping a new component instance to an existing element.
     */
    ...
    /**
     * Contains information about the element which should be used the next time a component class is instantiated.
     */
    ...
    /**
     * Creates a component instance with an element created based on the {@link Tag} annotation of the sub class.
     * <p>
     * If this is invoked through {@link #from(Element, Class)} or {@link Element#as(Class)}, uses the element defined
     * in those methods instead of creating a new element.
     */
    protected Component() {
        ComponentTracker.trackCreate(this);
        Optional<String> tagNameAnnotation = AnnotationReader.getAnnotationFor(getClass(), Tag.class).map(Tag::value);
        if (!tagNameAnnotation.isPresent()) {
            throw new IllegalStateException(getClass().getSimpleName() + " (or a super class) must be annotated with @"
                    + Tag.class.getName() + " if the default constructor is used.");
        }
     ...
     * <p>
     * For nearly all cases you want to pass an element reference but it is possible to pass {@code null} to this
     * method. If you pass {@code null} you must ensure that the element is initialized using
     * {@link #setElement(Component, Element)} before {@link #getElement()} is used.
     *
     * @param element
    ...
    protected Component(Element element) {
        ...
        if (elementToMapTo.get() != null) {
            ...
            templateMapped = this.element != null && this.element.isVirtualChild();
        } else {
            if (element != null) {
    ...
    private void addSynchronizedProperty(ComponentMetaData.SynchronizedPropertyInfo info) {
        ...
        info.getEventNames().forEach(eventType -> {
            if (eventType == null) {
                throw new IllegalArgumentException(
                        getClass().getName() + ": event type must not be null for @Synchronize annotation");
            }
            element.addPropertyChangeListener(info.getProperty(), eventType, NOOP_PROPERTY_LISTENER)
    ...
    private void mapToElement(String tagName) {
        ...
        if (tagName != null && !tagName.equalsIgnoreCase(elementTag)) {
            throw new IllegalArgumentException("A component specified to use a " + tagName
                    + " element cannot use an element with tag name " + elementTag);
        }
     ...
     * <p>
     * <b>Note!</b> Element API is designed for building components at a lower abstraction level than normal Vaadin UI
     * development. If you see a direct call to this method in your applications UI code, you should consider that as a
     * sign that you are probably doing something wrong and you should instead use other methods from your component,
     * e.g. when getting children, parent or ancestor component or adding listeners. This method is breaking the
     * Component's abstraction layer and its implementations provided. You should only call this method and use the
     * Element API when creating or extending components (e.g. setting the attributes and properties, adding DOM
     * listeners, execute JavaScript code), or when you otherwise need to break through the abstraction layer. If it is
     * a hack or a workaround, it is also better to hide that into an extension, helper class, separate add-on module or
     * at least into a private method documenting the usage.
     * <p>
     * Each component must have exactly one root element. When the component is attached to a parent component, this
     * element is attached to the parent component's element hierarchy.
     *
     ...
     * <p>
     * Each component must have a root element and it must be set before the component is attached to a parent. The root
     * element of a component cannot be changed once it has been set.
     * <p>
     * Typically you do not want to call this method but define the element through {@link #Component(Element)} instead.
     *
     * @param element
     *            the root element of the component
     * @param mapElementToComponent
     *            <code>true</code> to map the element to the component in addition to mapping the component to the
     *            element, <code>false</code> to only map the component to the element
     */
     ...
     *
     * @return an optional parent component, or an empty optional if the component is not attached to a parent
     */
     ...
     * <p>
     * The default implementation finds child components by traversing each child {@link Element} tree.
     * <p>
     * If the component is injected to a PolymerTemplate using the <code>@Id</code> annotation the getChildren method
     * will only return children added from the server side and will not return any children declared in the template
     * file.
     *
     * @see Id
     *
    ...
    protected Collection<?> getListeners(Class<? extends ComponentEvent> eventType) {
        return eventBus != null ? eventBus.getListeners(eventType) : Collections.emptyList();
    }
     ...
     *
     * @return an optional UI component, or an empty optional if this component is not attached to a UI
     */
    public Optional<UI> getUI() {
        ...
        if (parent.isPresent()) {
            ...
            parent = ComponentUtil.findParentComponent(((ShadowRoot) getElement().getParentNode()).getHost());
            return parent.flatMap(Component::getUI);
        }
     ...
     *
     * @param id
     *            the id to set, or <code>""</code> to remove any previously set id
     */
     ...
     * <p>
     * This method is invoked before the {@link AttachEvent} is fired for the component.
     * </p>
     ...
     * <p>
     * This method is invoked before the {@link DetachEvent} is fired for the component.
     * <p>
     ...
     * <p>
     * When {@link UI#close()} is called, the UI and the components are not detached immediately; the UI cleanup is
     * performed at the end of the current request which also detaches the UI and its components.
     *
     ...
     * <p>
     * This method attaches the component instance to the element so that {@link Element#getComponent()} returns the
     * component instance. This means that {@link #getParent()}, {@link #getChildren()} and other methods which rely on
     * {@link Element} -&gt; {@link Component} mappings will work correctly.
     * <p>
     * Note that only one {@link Component} can be mapped to any given {@link Element}.
     *
     ...
     * <p>
     * By default this sets or removes the 'disabled' attribute from the element. This can be overridden to have custom
     * handling.
     *
     * @param enabled
    ...
    /**
     * Gets whether this component was attached as part of a template (by being mapped by an {@link Id} annotation), or
     * if it was created directly.
     *
     * @return <code>true</code> when it was mapped inside a template, <code>false</code> otherwise
     */
     ...
     * <p>
     * The method never returns a null. If there is no {@link I18NProvider} available or no translation for the
     ...
     * @return translation for key if found (implementation should not return null)
     */
    public String getTranslation(String key, Object... params) {
        final Optional<I18NProvider> i18NProvider = LocaleUtil.getI18NProvider();
        return i18NProvider.map(i18n -> i18n.getTranslation(key, LocaleUtil.getLocale(() -> i18NProvider), params))
                .orElseGet(() -> "!{" + key + "}!");
    }
     ...
     * @deprecated Use {@link #getTranslation(Locale, String, Object...)} instead
     */
    @Deprecated
    ...
    public String getTranslation(Locale locale, String key, Object... params) {
        return LocaleUtil.getI18NProvider().map(i18n -> i18n.getTranslation(key, locale, params))
                .orElseGet(() -> "!{" + key + "}!");
    }
     ...
     * <p>
     * It returns the {@link UI} locale if it has been set. If there is no {@link UI} locale available then it tries to
     * use the first locale provided by the {@link I18NProvider}. If there is no any provided locale then the default
     * locale is used.
     *
     ...
     * @return The first ancestor that can be assigned to the given class. Null if no ancestor with the correct type
     *         could be found.
     * @param <T>

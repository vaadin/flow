...
public final class BundleParser {
    ...
    private static JsonObject getSourceObject(JsonObject module, String name) {
        JsonObject object = null;
        if (module.hasKey(MODULES) && module.get(MODULES).getType().equals(ARRAY)) {
            object = getSourceObjectFromArray(module.getArray(MODULES), name);
        }
        if (object == null && module.hasKey(CHUNKS) && module.get(CHUNKS).getType().equals(ARRAY)) {
            object = getSourceObjectFromArray(module.getArray(CHUNKS), name);
        }
        if (object == null && module.hasKey(NAME) && module.getString(NAME).endsWith(name) && module.hasKey(SOURCE)
                && module.get(SOURCE).getType().equals(STRING) && !module.getString(SOURCE).isEmpty()) {
            object = module;
        }
        return object;
    }

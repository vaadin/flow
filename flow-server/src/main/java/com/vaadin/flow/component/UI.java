 ...
 * {@link VaadinServlet}, the UI mapped to that servlet is opened. The selection is based on the <code>UI</code> init
 * parameter.
 * <p>
 * After a UI has been created by the application, it is initialized using {@link #init(VaadinRequest)}.
 *
...
public class UI extends Component implements PollNotifier, HasComponents, RouterLayout {
    ...
    /**
     * The id of this UI, used to find the server side instance of the UI form which a request originates. A negative
     * value indicates that the UI id has not yet been assigned by the Application.
     *
    ...
    /*
     * Despite section 6 of RFC 4122, this particular use of UUID *is* adequate for security capabilities. Type 4 UUIDs
     * contain 122 bits of random data, and UUID.randomUUID() is defined to use a cryptographically secure random
     * generator.
     */
    ...
    /**
     * Create a new empty UI with a custom {@link UIInternalUpdater} implementation.
     *
     ...
     *
     * <p>
     * Getting a null value is often a problem in constructors of regular components and in the initializers of custom
     * composite components. A standard workaround is to use {@link VaadinSession#getCurrent()} to retrieve the
     * application instance that the current request relates to. Another way is to move the problematic initialization
     * to {@link #onAttach(AttachEvent)}, as described in the documentation of the method.
     * </p>
     *
    ...
    /**
     * Gets the id of the UI, used to identify this UI within its application when processing requests. The UI id should
     * be present in every request to the server that originates from this UI.
     * {@link VaadinService#findUI(VaadinRequest)} uses this id to find the route to which the request belongs.
     * <p>
     * This method is not intended to be overridden. If it is overridden, care should be taken since this method might
     * be called in situations where {@link UI#getCurrent()} does not return this UI.
     *
    ...
    /**
     * Internal initialization method, should not be overridden. This method is not declared as final because that would
     * break compatibility with e.g. CDI.
     *
     * @param request
    ...
    public void doInit(VaadinRequest request, int uiId) {
        doInit(request, uiId, getSession().getService().getMainDivId(getSession(), request));
    }
    ...
    public void doInit(VaadinRequest request, int uiId, String appId) {
        if (this.uiId != -1) {
            String message = "This UI instance is already initialized (as UI id " + this.uiId
        ...
        if (this.isNavigationSupported()) {
            ...
            getElement().getStateProvider().appendVirtualChild(getElement().getNode(), wrapperElement,
                    NodeProperties.INJECT_BY_ID, appId);

            getEventBus().addListener(BrowserLeaveNavigationEvent.class, this::leaveNavigation);
            getEventBus().addListener(BrowserNavigateEvent.class, this::browserNavigate);
            getEventBus().addListener(BrowserRefreshEvent.class, this::browserRefresh);
    ...
    /**
     * Initializes this UI. This method is intended to be overridden by subclasses to build the view if {@link Router}
     * is not used. The method can also be used to configure non-component functionality. Performing the initialization
     * in a constructor is not suggested as the state of the UI is not properly set up when the constructor is invoked.
     * <p>
     * The provided {@link VaadinRequest} can be used to get information about the request that caused this UI to be
     * created.
     *
     * @param request
    ...
    /**
     * Sets the thread local for the current UI. This method is used by the framework to set the current application
     * whenever a new request is processed and it is cleared when the request has been processed.
     * <p>
     * The application developer can also use this method to define the current UI outside the normal request handling,
     * e.g. when initiating custom background threads.
     * <p>
    ...
    /**
     * Gets the currently used UI. The current UI is automatically defined when processing requests to the server. In
     * other cases, (e.g. from background threads), the current UI is not automatically defined.
     * <p>
     * The UI is stored using a weak reference to avoid leaking memory in case it is not explicitly cleared.
     *
    ...
    /**
     * Marks this UI to be {@link #onDetach(DetachEvent) detached} from the session at the end of the current request,
     * or the next request if there is no current request (if called from a background thread, for instance.)
     * <p>
     * The UI is detached after the response is sent, so in the current request it can still update the client side
     * normally. However, after the response any new requests from the client side to this UI will cause an error, so
     * usually the client should be asked, for instance, to reload the page (serving a fresh UI instance), to close the
     * page, or to navigate somewhere else.
     * <p>
     * Note that this method is strictly for users to explicitly signal the framework that the UI should be detached.
     * Overriding it is not a reliable way to catch UIs that are to be detached. Instead, {@code #onDetach(DetachEvent)}
     ...
     * {@link #init(VaadinRequest) init} method is called.
     *
     */
    @Override
     ...
     * </ul>
     * <p>
     * Note that when a UI is detached, any changes made in the {@code detach} methods of any children that would be
     ...
     * <p>
     * It is generally recommended to use {@link #access(Command)} instead of this method for accessing a session from a
     * different thread as {@link #access(Command)} can be used while holding the lock of another session. To avoid
     * causing deadlocks, this methods throws an exception if it is detected than another session is also locked by the
     * current thread.
     * <p>
     * This method behaves differently than {@link #access(Command)} in some situations:
     * <ul>
     * <li>If the current thread is currently holding the lock of the session, {@link #accessSynchronously(Command)}
     * runs the task right away whereas {@link #access(Command)} defers the task to a later point in time.</li>
     * <li>If some other thread is currently holding the lock for the session, {@link #accessSynchronously(Command)}
     * blocks while waiting for the lock to be available whereas {@link #access(Command)} defers the task to a later
     * point in time.</li>
     * </ul>
     *
     *
     ...
     *             if the UI is not attached to a session (and locking can therefore not be done)
    ...
    /*
     * Yes, we are mixing legacy Command with newer SerializableRunnable. This is done for this internal method since it
     * helps preserve old APIs as-is while allowing new APIs to use newer conventions.
     */
     ...
     * <p>
     * The given command is executed while holding the session lock to ensure exclusive access to this UI. If the
     * session is not locked, the lock will be acquired and the command is run right away. If the session is currently
     * locked, the command will be run before that lock is released.
     * </p>
     * <p>
     * RPC handlers for components inside this UI do not need to use this method as the session is automatically locked
     * by the framework during RPC handling.
     * </p>
     * <p>
     * Please note that the command might be invoked on a different thread or later on the current thread, which means
     * that custom thread locals might not have the expected values when the command is executed.
     * {@link UI#getCurrent()}, {@link VaadinSession#getCurrent()} and {@link VaadinService#getCurrent()} are set
     * according to this UI before executing the command. Other standard CurrentInstance values such as
     ...
     * helps preserve old APIs as-is while allowing new APIs to use newer conventions.
     */
    private Future<Void> access(Command command, SerializableRunnable detachHandler) {
        VaadinSession session = getSession();
            ...
            public void handleError(Exception exception) {
                try {
                    if (command instanceof ErrorHandlingCommand) {
                        ...
                        final Map<Class<?>, CurrentInstance> map = CurrentInstance.setCurrent(UI.this);
                        try {
                            getSession().getErrorHandler().error(new ErrorEvent(exception));
                        } finally {
                            CurrentInstance.restoreInstances(map);
                        }
                    } else {
                        /*
                         * The session has expired after `ui.access` was called. It makes no sense to pollute the logs
                         * with a UIDetachedException at this point.
                         */
                        if (exception instanceof ExecutionException
                                && exception.getCause() instanceof UIDetachedException) {
    ...
    /**
     * Wraps the given access task as a runnable that runs the given task with this UI locked. The wrapped task may be
     * run synchronously or asynchronously. If the UI is detached when the returned runnable is run, the provided detach
     * handler is run instead. If the provided detach handler is <code>null</code>, the returned runnable may throw an
     * {@link UIDetachedException}.
     * <p>
     * This method can be used to create a callback that can be passed to an external notifier that isn't aware of the
     * synchronization needed to update a UI instance.
     *
     * @param accessTask
     *            the task that updates this UI, not <code>null</code>
     * @param detachHandler
     *            the callback that will be invoked if the UI is detached, or <code>null</code> as described above
     * @return a runnable that will run either the access task or the detach handler, possibly asynchronously
     */
    ...
    /**
     * Wraps the given access task as a consumer that passes a value to the given task with this UI locked. The wrapped
     * task may be run synchronously or asynchronously. If the UI is detached when the returned consumer is run, the
     * provided detach handler is run instead. If the provided detach handler is <code>null</code>, the returned
     * runnable may throw an {@link UIDetachedException}.
     * <p>
     * This method can be used to create a callback that can be passed to an external notifier that isn't aware of the
     * synchronization needed to update a UI instance.
     *
     ...
     * @param detachHandler
     *            the callback that will be invoked if the UI is detached, or <code>null</code> as described above
     * @return a consumer that will run either the access task or the detach handler, possibly asynchronously
     */
    ...
    /**
     * Sets the interval with which the UI should poll the server to see if there are any changes. Polling is disabled
     * by default.
     * <p>
     * Note that it is possible to enable push and polling at the same time but it should not be done to avoid excessive
     * server traffic.
     * </p>
     * <p>
     * Add-on developers should note that this method is only meant for the application developer. An add-on should not
     * set the poll interval directly, rather instruct the user to set it.
     * </p>
     *
    ...
    public void setPollInterval(int intervalInMillis) {
        getNode().getFeature(PollConfigurationMap.class).setPollInterval(intervalInMillis);
    }

    /**
     * Returns the interval with which the UI polls the server.
     *
     * @return The interval (in ms) with which the UI polls the server or -1 if polling is disabled
     */
    public int getPollInterval() {
        return getNode().getFeature(PollConfigurationMap.class).getPollInterval();
    }
     ...
     * <p>
     * If push is enabled, but the push connection is not currently open, the push will be done when the connection is
     * established.
     * <p>
     * As with all UI methods, the session must be locked when calling this method. It is also recommended that
     * {@link UI#getCurrent()} is set up to return this UI since writing the response may invoke logic in any attached
     * component or extension. The recommended way of fulfilling these conditions is to use {@link #access(Command)}.
     *
     * @throws IllegalStateException
     *             if push is disabled.
     * @throws UIDetachedException
    ...
    public void push() {
        ...
        /*
         * Purge the pending access queue as it might mark a connector as dirty when the push would otherwise be ignored
         * because there are no changes to push.
         */
     ...
     * <p>
     * Note that you cannot change push parameters on the fly, you need to configure the push channel at the same time
     * (in the same request) it is enabled.
     *
    ...
    /**
     * Gets the locale for this UI. The default locale is based on the session's locale, which is in turn determined in
     * different ways depending on whether a {@link I18NProvider} is available.
     * <p>
     * If a i18n provider is available, the locale is determined by selecting the locale from
     * {@link I18NProvider#getProvidedLocales()} that best matches the user agent preferences (i.e. the
     * <code>Accept-Language</code> header). If an exact match is found, then that locale is used. Otherwise, the
     * matching logic looks for the first provided locale that uses the same language regardless of the country. If no
     * other match is found, then the first item from {@link I18NProvider#getProvidedLocales()} is used.
     * <p>
     * If no i18n provider is available, then the {@link Locale#getDefault() default JVM locale} is used as the default
     * locale.
     *
     ...
     * <p>
     * Note that {@link VaadinSession#setLocale(Locale)} will set the locale for all UI instances in that session, and
     * might thus override any custom locale previous set for a specific UI.
     *
     * @param locale
     ...
     * <p>
     * If you need the direction to update automatically upon {@link Locale} change, make the main layout implement
     * {@link com.vaadin.flow.i18n.LocaleChangeObserver} and call this method from the
     * {@link com.vaadin.flow.i18n.LocaleChangeObserver#localeChange(LocaleChangeEvent)} implementation.
     *
     * @param direction
     ...
     * <p>
     * If the view change actually happens (e.g. the view itself doesn't cancel the navigation), all navigation
     * listeners are notified and a reference of the new view is returned for additional configuration.
     *
     *
     * @param navigationTarget
     *            navigation target to navigate to
     * @throws IllegalArgumentException
     *             if navigationTarget is a {@link HasUrlParameter} with a mandatory parameter.
     * @throws NotFoundException
     *             in case there is no route defined for the given navigationTarget.
    ...
    /**
     * Updates this UI to show the view corresponding to the given navigation target with the specified parameter. The
     * parameter needs to be the same as defined in the route target HasUrlParameter.
     * <p>
     * Besides the navigation to the {@code location} this method also updates the browser location (and page history).
     * <p>
     * Note! A {@code null} parameter will be handled the same as navigate(navigationTarget) and will throw an exception
     * if HasUrlParameter is not @OptionalParameter or @WildcardParameter.
     * <p>
     * If the view change actually happens (e.g. the view itself doesn't cancel the navigation), all navigation
     * listeners are notified and a reference of the new view is returned for additional configuration.
     *
     ...
     * @return the view instance, if navigation actually happened
     * @throws IllegalArgumentException
     *             if a {@code null} parameter is given while navigationTarget's parameter is not annotated
     *             with @OptionalParameter or @WildcardParameter.
     * @throws NotFoundException
    ...
    public <T, C extends Component & HasUrlParameter<T>> Optional<C> navigate(Class<? extends C> navigationTarget,
            T parameter) {
        navigate(navigationTarget, HasUrlParameterFormat.getParameters(parameter));
        return (Optional<C>) findCurrentNavigationTarget(navigationTarget);
    }

    /**
     * Updates this UI to show the view corresponding to the given navigation target with the specified parameters. The
     * parameters needs to comply with the ones defined in one of the {@link com.vaadin.flow.router.Route} or
     * {@link com.vaadin.flow.router.RouteAlias} annotating the navigationTarget and with any
     * {@link com.vaadin.flow.router.RoutePrefix} annotating the parent layouts of the navigationTarget.
     * <p>
     * Besides the navigation to the {@code location} this method also updates the browser location (and page history).
     * <p>
     * If the view change actually happens (e.g. the view itself doesn't cancel the navigation), all navigation
     ...
     * @return the view instance, if navigation actually happened
     * @throws IllegalArgumentException
     *             if navigationTarget is a {@link HasUrlParameter} with a mandatory parameter, but parameters argument
     *             doesn't provide {@link HasUrlParameterFormat#PARAMETER_NAME} parameter.
     * @throws NotFoundException
     *             in case there is no route defined for the given navigationTarget matching the parameters.
     */
    ...
    @SuppressWarnings("unchecked")
    public <T, C extends Component & HasUrlParameter<T>> Optional<C> navigate(Class<? extends C> navigationTarget,
            T parameter, QueryParameters queryParameters) {
        ...
        getInternals().getRouter().navigate(this, new Location(url, queryParameters), NavigationTrigger.UI_NAVIGATE);
        return (Optional<C>) findCurrentNavigationTarget(navigationTarget);
    }

    /**
     * Updates this UI to show the view corresponding to the given navigation target with the specified parameters. The
     * route parameters needs to comply with the ones defined in one of the {@link com.vaadin.flow.router.Route} or
     * {@link com.vaadin.flow.router.RouteAlias} annotating the navigationTarget and with any
     * {@link com.vaadin.flow.router.RoutePrefix} annotating the parent layouts of the navigationTarget.
     * <p>
     * Besides the navigation to the {@code location} this method also updates the browser location (and page history).
    ...
    public <C extends Component> Optional<C> navigate(Class<? extends C> navigationTarget,
            RouteParameters routeParameter, QueryParameters queryParameters) {
        RouteConfiguration configuration = RouteConfiguration.forRegistry(getInternals().getRouter().getRegistry());
    ...
    /**
     * Updates this UI to show the view corresponding to the given location. The location must be a relative path
    ...
    /**
     * Updates this UI to show the view corresponding to the given location and query parameters. The location must be a
     * relative path without any ".." segments.
     * <p>
     * Besides the navigation to the {@code location} this method also updates the browser location (and page history).
     *
     * @see #navigate(String)
     ...
     *            the location to navigate to, not {@code null}
     * @param queryParameters
     *            query parameters that are used for navigation, not {@code null}
     * @throws NullPointerException
    ...
    public void navigate(String locationString, QueryParameters queryParameters) {
        ...
        if (navigationInProgress || (getInternals().hasLastHandledLocation()
                && sameLocation(getInternals().getLastHandledLocation(), location))) {
            return;
        }
    ...
    /**
     * Re-navigates to the current route. Also re-instantiates the route target component, and optionally all layouts in
     * the route chain.
     *
     ...
     * <p>
     * Also, the current route might not be initialized if this method is called while still building the view chain,
     * for example in the constructor of layouts. Thus, consider postponing the usage of this method to for example
     ...
     * <p>
     * If the {@link Component} related to the task is not attached to the document by the time the task is evaluated,
     * the execution is postponed to before the next response.
     * <p>
     * The task receives a {@link ExecutionContext} as parameter, which contains information about the component state
     * before the response.
     *
     * @param component
     *            the Component relevant for the execution. Can not be <code>null</code>
     *
     * @param execution
    ...
    public ExecutionRegistration beforeClientResponse(Component component,
            SerializableConsumer<ExecutionContext> execution) throws IllegalArgumentException {
        ...
        return internals.getStateTree().beforeClientResponse(component.getElement().getNode(), execution);
    }
     ...
     * <p>
     * By the time the components are going to be attached, their state is already calculated, consider this when using
     * the listener.
     * <p>
    ...
    /**
     * Registers a global shortcut tied to the {@code UI} which executes the given {@link Command} when invoked.
     * <p>
     * Returns {@link ShortcutRegistration} which can be used to fluently configure the shortcut. The shortcut will be
     * present until {@link ShortcutRegistration#remove()} is called.
     *
     * @param command
     *            code to execute when the shortcut is invoked. Cannot be null
     * @param key
     *            primary {@link Key} used to trigger the shortcut. Cannot be null
     * @param keyModifiers
     *            {@link KeyModifier KeyModifiers} which also need to be pressed for the shortcut to trigger
     * @return {@link ShortcutRegistration} for configuring the shortcut and removing
     * @see #addShortcutListener(ShortcutEventListener, Key, KeyModifier...) for registering a listener which receives a
     *      {@link ShortcutEvent}
    ...
    public ShortcutRegistration addShortcutListener(Command command, Key key, KeyModifier... keyModifiers) {
        if (command == null) {
            throw new IllegalArgumentException(String.format(Shortcuts.NULL, "command"));
        }
        if (key == null) {
            throw new IllegalArgumentException(String.format(Shortcuts.NULL, "key"));
        }
        return new ShortcutRegistration(this, () -> new Component[] { this }, event -> command.execute(), key)
                .withModifiers(keyModifiers);
    }

    /**
     * Registers a global shortcut tied to the {@code UI} which executes the given {@link ComponentEventListener} when
     * invoked.
     * <p>
     * Returns {@link ShortcutRegistration} which can be used to fluently configure the shortcut. The shortcut will be
     * present until {@link ShortcutRegistration#remove()} is called.
     *
     * @param listener
     *            listener to execute when the shortcut is invoked. Receives a {@link ShortcutEvent}. Cannot be null
     * @param key
     ...
     * @see Shortcuts for a more generic way to add a shortcut
     */
    public ShortcutRegistration addShortcutListener(ShortcutEventListener listener, Key key,
            KeyModifier... keyModifiers) {
        if (listener == null) {
            throw new IllegalArgumentException(String.format(Shortcuts.NULL, "listener"));
        }
        if (key == null) {
            throw new IllegalArgumentException(String.format(Shortcuts.NULL, "key"));
        }
     ...
     * <p>
     * Heartbeat requests are periodically sent by the client-side to inform the server that the UI sending the
     * heartbeat is still alive (the browser window is open, the connection is up) even when there are no UIDL requests
     * for a prolonged period of time. UIs that do not receive either heartbeat or UIDL requests are eventually removed
     * from the session and garbage collected.
     *
     * @param listener
     ...
     * <p>
     * <em>NOTE: the generic drag and drop feature for Flow is available in another artifact, {@code flow-dnd} for
     * now.</em>
     *
    ...
    /**
     * Gets the CSRF token (synchronizer token pattern) that is used to protect against Cross Site Request Forgery
     * attacks.
     *
    ...
    /**
     * Adds the given component as a modal child to the UI, making the UI and all other (existing) components added to
     * the UI impossible for the user to interact with. This is useful for modal dialogs which should make the UI in the
     * background inert. Note that this only prevents user interaction, but doesn't show a modality curtain or change
     * the visible state of the components in the UI - that should be handled by the component separately. Thus this is
     * purely a server side feature.
     * <p>
     * When the modal component is removed the UI and its other children are no longer inert, unless there was another
     * component added as modal before.
     *
     *
     * @param component
    ...
    /**
     * Makes the child component modal or modeless. The component needs to be a child of this UI. By default all child
     ...
     */
    /*
     * TODO decide and document whether resize listener still works for UI even when it is inert.
     */
    public void setChildComponentModal(Component childComponent, boolean modal) {
        ...
        if (ui.isPresent() && !ui.get().equals(this)) {
            throw new IllegalStateException("Given component is not a child in this UI. "
                    + "Add it first as a child of the UI so it is " + "attached or just use addModal(component).");
        }
        if (modal) {
     ...
     * <p>
     * This is meant to be used with components that are not added as part of a layout, like dialog, so that they are
     * interactive when a modal component opens up an overlay component.
     *
     * @param component
    ...
    public void addToModalComponent(Component component) {
        if (hasModalComponent()) {
            final Component activeModalComponent = getInternals().getActiveModalComponent();
    ...
    public Stream<Component> getChildren() {
        ...
        wrapperElement.getChildren()
                .forEach(childElement -> ComponentUtil.findComponents(childElement, childComponents::add));
    ...
    public static class BrowserLeaveNavigationEvent extends ComponentEvent<UI> {
         ...
         */
        public BrowserLeaveNavigationEvent(UI source, boolean fromClient, @EventData("route") String route,
    ...
    public static class BrowserNavigateEvent extends ComponentEvent<UI> {
         ...
         *
         */
        public BrowserNavigateEvent(UI source, boolean fromClient, @EventData("route") String route,
                @EventData("query") String query, @EventData("appShellTitle") String appShellTitle,
                @EventData("historyState") JsonValue historyState, @EventData("trigger") String trigger) {
            super(source, true);
            this.route = route;
            this.query = query;
     ...
     * {@code fullRefresh} event flag is active.
     *
    ...
    public static class BrowserRefreshEvent extends ComponentEvent<UI> {
         ...
         *            NOTE: for technical reason the argument must be added to the constructor, but this event the value
         *            is always true.
         * @param refreshRouteChain
         *            {@code true} to refresh all layouts in the route chain, {@code false} to only refresh the route
         *            instance
    ...
    /**
     * Connect a client with the server side UI. This method is invoked each time client router navigates to a server
     * route.
     *
     * @param flowRoutePath
     *            flow route path that should be attached to the client element
     ...
     *            navigation trigger
     *
     * @deprecated(forRemoval=true) method is not enabled for client side anymore and connectClient is triggered by DOM
     *                              event, to be removed in next major 25
     */
    @Deprecated
    public void connectClient(String flowRoutePath, String flowRouteQuery, String appShellTitle, JsonValue historyState,
            String trigger) {
        browserNavigate(new BrowserNavigateEvent(this, false, flowRoutePath, flowRouteQuery, appShellTitle,
                historyState, trigger));
    }
    ...
    public void browserNavigate(BrowserNavigateEvent event) {
        ...
        final Location location = new Location(trimmedRoute, QueryParameters.fromString(event.query));
        ...
        if (firstNavigation) {
            ...
            getPage().getHistory()
                    .setHistoryStateChangeHandler(e -> renderViewForRoute(e.getLocation(), e.getTrigger()));
            ...
            handler.onHistoryStateChange(new History.HistoryStateChangeEvent(getPage().getHistory(), event.historyState,
        ...
        if (getForwardToClientUrl() != null) {
            ...
            serverConnected(!getSession().getState().equals(VaadinSessionState.OPEN));
            replaceStateIfDiffersAndNoReplacePending(event.route, location);
        }
    }

    /**
     * Do a history replaceState if the trimmed route differs from the event route and there is no pending replaceState
     * command.
     *
     * @param route
     *            the event.route
    ...
    private void replaceStateIfDiffersAndNoReplacePending(String route, Location location) {
        boolean locationChanged = !location.getPath().equals(route) && route.startsWith("/")
                && !location.getPath().equals(route.substring(1));
        boolean containsPendingReplace = !getInternals().containsPendingJavascript("window.history.replaceState")
    ...
    /**
     * Check that the view can be leave. This method is invoked when the client router tries to navigate to a client
     * route while the current route is a server route.
     * <p>
     ...
     * @param query
     *            route query string
     * @deprecated(forRemoval=true) method is not enabled for client side anymore and leave navigation is triggered by
     *                              DOM event, to be removed in next major 25
     */
    @Deprecated
    public void leaveNavigation(String route, String query) {
        leaveNavigation(new BrowserLeaveNavigationEvent(this, false, route, query));
    }
    ...
    public void leaveNavigation(BrowserLeaveNavigationEvent event) {
        navigateToPlaceholder(new Location(PathUtil.trimPath(event.route), QueryParameters.fromString(event.query)));
    ...
    private void navigateToPlaceholder(Location location) {
        if (clientViewNavigationState == null) {
            clientViewNavigationState = new NavigationStateBuilder(getInternals().getRouter())
        ...
        handleNavigation(location, clientViewNavigationState, NavigationTrigger.CLIENT_SIDE);
    }
    ...
    private boolean sameLocation(Location oldLocation, Location newLocation) {
        return PathUtil.trimPath(newLocation.getPathWithQueryParameters())
                .equals(PathUtil.trimPath(oldLocation.getPathWithQueryParameters()));
    }
    ...
    private boolean handleExceptionNavigation(Location location, Exception exception) {
        ...
        if (maybeLookupResult.isPresent()) {
            ...
            ErrorNavigationEvent errorNavigationEvent = new ErrorNavigationEvent(getInternals().getRouter(), location,
                    this, NavigationTrigger.CLIENT_SIDE, errorParameter);
    ...
    private void handleErrorNavigation(Location location) {
        NavigationState errorNavigationState = getInternals().getRouter().resolveRouteNotFoundNavigationTarget()
        ...
        ErrorParameter<NotFoundException> errorParameter = new ErrorParameter<>(NotFoundException.class,
                notFoundException);
        ErrorNavigationEvent errorNavigationEvent = new ErrorNavigationEvent(getInternals().getRouter(), location, this,
                NavigationTrigger.CLIENT_SIDE, errorParameter);
        errorStateRenderer.handle(errorNavigationEvent);
    }

    private NavigationState getDefaultNavigationError() {
        return new NavigationStateBuilder(getInternals().getRouter()).withTarget(RouteNotFoundError.class).build();
    }

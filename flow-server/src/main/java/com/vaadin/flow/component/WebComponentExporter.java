 ...
 * <p>
 * By extending this class you can export a server side {@link Component} with a given tag name so that it can be
 * included in any web page as {@code <tag-name>}. You can add properties/attributes to the element, which are
 * synchronized with the server and you can fire events from the server, which are available as custom events in the
 * browser.
 * <p>
 * The tag name (must contain at least one dash and be unique on the target web page) is provided through the super
 * constructor. Note that the exporter tag is not related to the tag used by the {@link Component} being exported and
 * they cannot be the same.
 * <p>
 * The component class to exported is determined by the parameter given to {@code WebComponentExporter} when extending
 * it, e.g. {@code extends WebComponentExporter<MyComponent>}.
 * <p>
 * If you want to customize the {@link Component} instance after it has been created, you should override
 * {@link #configureInstance(WebComponent, Component)} which is called for each created instance.
 * <p>
 * Example of exporting {@code MyComponent} to be used as {@code <my-component name="something">}:
 *
 * <pre>
...
public abstract class WebComponentExporter<C extends Component> implements Serializable {
    ...
    private static final List<Class> SUPPORTED_TYPES = Collections
            .unmodifiableList(Arrays.asList(Boolean.class, String.class, Integer.class, Double.class, JsonValue.class));
    ...
    /**
     * Creates a new {@code WebComponentExporter} instance and configures the tag name of the web component created
     * based on this exporter.
     * <p>
     * This constructor is not meant to be overridden unless the {@code
     * exporter} can be extended. Rather, create a non-args constructor and call this constructor from it.
     *
     * @param tag
     *            tag name of the web component created by the exporter, cannot be {@code null}
     */
    ...
    protected WebComponentExporter(String tag) {
        ...
        if (getComponentClass() == null) {
            throw new IllegalStateException(String.format(
                    ...
                    getClass().getName(), WebComponentExporter.class.getSimpleName()));
        }
    }

    private <P extends Serializable> PropertyConfiguration<C, P> addProperty(String name, Class<P> type,
            P defaultValue) {
        Objects.requireNonNull(name, "Parameter 'name' cannot be null!");
        Objects.requireNonNull(type, "Parameter 'type' cannot be null!");

        if (isConfigureInstanceCall) {
            throw new IllegalStateException("The 'addProperty' method cannot be called within "
                    + "the 'configureInstance' method. All properties have " + "to be configured in the "
                    + getClass().getName() + " class constructor.");
        }

        if (!isSupportedType(type)) {
            throw new UnsupportedPropertyTypeException(String.format(
                    "PropertyConfiguration " + "cannot handle type %s. Use any of %s instead.", type.getCanonicalName(),
                    SUPPORTED_TYPES.stream().map(Class::getSimpleName).collect(Collectors.joining(", "))));
        }

        PropertyConfigurationImpl<C, P> propertyConfigurationImpl = new PropertyConfigurationImpl<>(getComponentClass(),
    ...
    /**
     * Add an {@code Integer} property to the exported web component identified by {@code name}.
     *
     * @param name
     *            name of the property. While all formats are allowed, names in camelCase will be converted to
     *            dash-separated form, when property update events are generated, using form "property-name-changed", if
     *            the property is called "propertyName"
     * @param defaultValue
    ...
    /**
     * Add an {@code Double} property to the exported web component identified by {@code name}.
     *
     * @param name
     *            name of the property. While all formats are allowed, names in camelCase will be converted to
     *            dash-separated form, when property update events are generated, using form "property-name-changed", if
    ...
    /**
     * Add an {@code String} property to the exported web component identified by {@code name}.
     *
     * @param name
     *            name of the property. While all formats are allowed, names in camelCase will be converted to
     *            dash-separated form, when property update events are generated, using form "property-name-changed", if
    ...
    /**
     * Add an {@code Boolean} property to the exported web component identified by {@code name}.
     *
     * @param name
     *            name of the property. While all formats are allowed, names in camelCase will be converted to
     *            dash-separated form, when property update events are generated, using form "property-name-changed", if
    ...
    /**
     * Add an {@code JsonValue} property to the exported web component identified by {@code name}.
     *
     * @param name
     *            name of the property. While all formats are allowed, names in camelCase will be converted to
     *            dash-separated form, when property update events are generated, using form "property-name-changed", if
     ...
     * @return fluent {@code PropertyConfiguration} for configuring the property
     */
    public final PropertyConfiguration<C, JsonValue> addProperty(String name, JsonValue defaultValue) {
        return addProperty(name, JsonValue.class, defaultValue);
    }

    /**
     * If custom initialization for the created {@link Component} instance is needed, it can be done here. It is also
     * possible to configure custom communication between the {@code component} instance and client-side web component
     * using the {@link WebComponent} instance. The {@code
     ...
     * {@link #configureInstance(WebComponent, Component)} method. All properties have to be configured inside the
     * exporter's constructor.
     *
     * @param webComponent
     *            instance representing the client-side web component instance matching the component
     * @param component
    ...
    private static final class WebComponentConfigurationImpl<C extends Component>
        ...
        private WebComponentConfigurationImpl(WebComponentExporter<C> exporter) {
            ...
            immutablePropertyMap = Collections.unmodifiableMap(wcExporter.propertyConfigurationMap);
        }
        ...
        public Class<? extends Serializable> getPropertyType(String propertyName) {
            if (hasProperty(propertyName)) {
                return immutablePropertyMap.get(propertyName).getPropertyData().getType();
            } else {
                return null;
            }
        }
        ...
        public Set<PropertyData<? extends Serializable>> getPropertyDataSet() {
            return immutablePropertyMap.values().stream().map(PropertyConfigurationImpl::getPropertyData)
        ...
        public WebComponentBinding<C> createWebComponentBinding(Instantiator instantiator, Element element,
            ...
            final C componentReference = instantiator.createComponent(this.getComponentClass());

            if (componentReference == null) {
                throw new RuntimeException(
                        "Failed to instantiate a new " + this.getComponentClass().getCanonicalName());
            }

            /*
             * The tag check cannot be done before the creation of the Component being exported. This is due to the
             * unavailability of Instantiator before VaadinService has been initialized (which happens after collecting
             * all the exporters).
             */
            ...
            if (this.exporter.getTag().equals(componentTag)) {
                throw new IllegalStateException(String.format(
                        "WebComponentExporter '%s' cannot share a tag with the "
                                + "%s instance being exported! Change the tag " + "from '%s' to something else.",
                        this.getClass().getCanonicalName(), componentReference.getClass().getCanonicalName(),
                        this.exporter.getTag()));
            }

            WebComponentBinding<C> binding = new WebComponentBinding<>(componentReference);

            // collect possible new defaults from attributes as JsonValues
            final Map<String, JsonValue> newDefaultValues = Stream.of(newAttributeDefaults.keys())
                    .collect(Collectors.toMap(key -> key, newAttributeDefaults::get));
            ...
            immutablePropertyMap.values().forEach(propertyConfiguration -> {
                ...
                binding.bindProperty(propertyConfiguration, newDefaultValues.containsKey(key),
        ...
        public int hashCode() {
            ...
            objs[0] = getTag();
            int place = 1;
            for (PropertyConfiguration configuration : immutablePropertyMap.values()) {
        ...
        public boolean equals(Object obj) {
            if (obj instanceof WebComponentConfigurationImpl) {
                ...
                isSame = isSame && (immutablePropertyMap.size() == other.immutablePropertyMap.size());
    ...
    protected Class<C> getComponentClass() {
        return (Class<C>) ReflectTools.getGenericInterfaceType(this.getClass(), WebComponentExporter.class);
    }

    /**
     * Produces {@link WebComponentConfiguration} instances from either {@link WebComponentExporter} classes or
     * instances.
     *
     * @author Vaadin Ltd
     */
    public static final class WebComponentConfigurationFactory implements Serializable {
        ...
        /**
         * Creates a {@link WebComponentConfiguration} for the provided {@link WebComponentExporter} instances.
         *
         * @param exporter
         *            exporter instance, not {@code null}
         * @return a web component configuration matching the instance of received {@code exporter}

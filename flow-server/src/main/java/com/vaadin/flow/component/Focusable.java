...
public interface Focusable<T extends Component> extends HasElement, BlurNotifier<T>, FocusNotifier<T>, HasEnabled {
     ...
     * @see <a href= "https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/tabindex">tabindex at MDN</a>
     */
    ...
    /**
     * Gets the <code>tabindex</code> in the component. The tabIndex indicates if its element can be focused, and
     * if/where it participates in sequential keyboard navigation.
     * <p>
     * If there's no such attribute set, it returns the default setting for the element, which depends on the element
     * and on the browser. If the attribute cannot be parsed to <code>int</code>, then an {@link IllegalStateException}
     * is thrown.
     *
     * @return the tabindex attribute, or 0 if none
     * @throws IllegalStateException
     *             if the returned tabindex from the element is empty or can not be parsed to int
     * @see <a href= "https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/tabindex">tabindex at MDN</a>
     */
    default int getTabIndex() {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "tabindex attribute could not be parsed on element " + getElement().getTag() + ": " + attribute);
        }
    }

    /**
     * Calls the <code>focus</code> function at the client, making the component keyboard focused.
     *
     * @see <a href= "https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/focus">focus at MDN</a>
     */
    default void focus() {
        /*
         * Use setTimeout to call the focus function only after the element is attached, and after the initial rendering
         * cycle, so webcomponents can be ready by the time when the function is called.
         */
    ...
    /**
     * Calls the <code>blur</code> function at the client, making the component lose keyboard focus.
     *
     * @see <a href= "https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/blur">blur at MDN</a>
     */
    default void blur() {
    ...
    /**
     * Adds a shortcut which focuses the {@link Component} which implements {@link Focusable} interface. The shortcut's
     * event listener is in global scope and the shortcut's lifecycle is tied to {@code this} component.
     * <p>
     * Use the returned {@link ShortcutRegistration} to fluently configure the shortcut.
     *
     * @param key
     *            primary {@link Key} used to trigger the shortcut. Cannot be null.
     * @param keyModifiers
     *            {@link KeyModifier KeyModifiers} that need to be pressed along with the {@code key} for the shortcut
     *            to trigger
    ...
    default ShortcutRegistration addFocusShortcut(Key key, KeyModifier... keyModifiers) {
        if (!(this instanceof Component)) {
            throw new IllegalStateException(String.format(
                    ...
                    getClass().getName(), Component.class.getSimpleName(), "addFocusShortcut(Key, KeyModifier...)"));
        }

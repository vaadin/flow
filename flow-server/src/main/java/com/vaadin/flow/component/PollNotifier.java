 ...
 * <p>
 * By implementing this interface, a class publicly announces that it is able to send {@link PollEvent PollEvents}
 * whenever the client sends a periodic poll message to the client, to check for asynchronous server-side modifications.
 *
...
public interface PollNotifier extends Serializable {
    ...
    default Registration addPollListener(ComponentEventListener<PollEvent> listener) {
        if (this instanceof Component) {
            return ComponentUtil.addListener((Component) this, PollEvent.class, listener);
        } else {
            throw new IllegalStateException(String.format(
                    "The class '%s' doesn't extend '%s'. " + "Make your implementation for the method '%s'.",
                    getClass().getName(), Component.class.getSimpleName(), "addPollListener"));
        }
    }

...
public class StreamReceiverRegistry implements Serializable {

    private final Map<URI, StreamReceiver> resources = new HashMap<>();

    private final VaadinSession session;

    private static final class Registration
            implements StreamReceiverRegistration {

        private final StreamReceiverRegistry registry;

        private final URI uri;

        private Registration(StreamReceiverRegistry registry, String id,
        ...
        @Override
        public Optional<StreamReceiver> getResource() {
            return registry.getReceiver(getResourceUri());
        }
    }
     ...
     *            vaadin session
     */
    public StreamReceiverRegistry(VaadinSession session) {
     ...
     */
    public StreamReceiverRegistration registerResource(
            StreamReceiver resource) {
        assert session.hasLock();
        Registration registration = new Registration(this, resource.getId(),
                resource.getNodeId(), resource.getAttributeName());
    ...
    public static URI getURI(StreamReceiver resource) {
        return getURI(resource.getNodeId(), resource.getId(),

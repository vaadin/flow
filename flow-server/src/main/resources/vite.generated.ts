 ...
 */
import { resolve, extname, relative, basename, dirname } from 'path';
...
const frontendFolder = resolve(__dirname, settings.frontendFolder);
const themeFolder = resolve(frontendFolder, settings.themeFolder);
const frontendBundleFolder = resolve(__dirname, settings.frontendBundleOutput);
const devBundleFolder = resolve(__dirname, settings.devBundleOutput);
const devBundle = !!process.env.devBundle;
const jarResourcesFolder = resolve(__dirname, settings.jarResourcesFolder);
const generatedFlowImportsFolder = resolve(__dirname, settings.generatedFlowImportsFolder);
const themeResourceFolder = resolve(__dirname, settings.themeResourceFolder);
const projectPackageJsonFile = resolve(__dirname, 'package.json');

const buildOutputFolder = devBundle ? devBundleFolder : frontendBundleFolder;
const statsFolder = resolve(__dirname, devBundle ? settings.devBundleStatsOutput : settings.statsOutput);
const statsFile = resolve(statsFolder, 'stats.json');
const nodeModulesFolder = resolve(__dirname, 'node_modules');
const webComponentTags = '#webComponentTags#';

const projectStaticAssetsFolders = [
  resolve(__dirname, 'src', 'main', 'resources', 'META-INF', 'resources'),
  resolve(__dirname, 'src', 'main', 'resources', 'static'),
  frontendFolder
];

// Folders in the project which can contain application themes
const themeProjectFolders = projectStaticAssetsFolders.map((folder) => resolve(folder, settings.themeFolder));
...
const themeOptions = {
  ...
  themeResourceFolder: resolve(themeResourceFolder, settings.themeFolder),
  themeProjectFolders: themeProjectFolders,
  projectStaticAssetsOutputFolder: devBundle ? resolve(devBundleFolder, '../assets') : resolve(__dirname, settings.staticOutput),
  frontendGeneratedFolder: resolve(frontendFolder, settings.generatedFolder)
};

const hasExportedWebComponents = existsSync(resolve(frontendFolder, 'web-component.html'));
...
function buildSWPlugin(opts): PluginOption {
  ...
  async function build(action: 'generate' | 'write', additionalPlugins: rollup.Plugin[] = []) {
    ...
    const bundle = await rollup.rollup({
      input: resolve(settings.clientServiceWorkerSource),
      plugins
    });

    try {
      return await bundle[action]({
        file: resolve(buildOutputFolder, 'sw.js'),
        format: 'es',
        exports: 'none',
...
function statsExtracterPlugin(): PluginOption {
  return {
    ...
    async writeBundle(options: OutputOptions, bundle: { [fileName: string]: AssetInfo | ChunkInfo }) {
      ...
      const generatedImports = readFileSync(resolve(generatedFlowImportsFolder, "generated-flow-imports.js"), {encoding: 'utf-8'})
          .split("\n")
      ...
      generatedImports.filter((line: string) => line.includes("generated/jar-resources")).forEach((line: string) => {
        ...
        // \r\n from windows made files may be used ro remove to be only \n
        const fileBuffer = readFileSync(resolve(frontendFolder, filename), {encoding: 'utf-8'}).replace(/\r\n/g, '\n');
      ...
      if (existsSync(themesFolder)) {
        readdirSync(themesFolder).forEach((themeFolder) => {
          const themeJson = resolve(themesFolder, themeFolder, "theme.json");
    ...
    apply(config, { command }) {
      ...
      for (const [name, packageInfo] of Object.entries(vaadinBundleJson.packages)) {
        ...
        try {
          ...
          const installedPackageJsonFile = resolve(modulesDirectory, name, 'package.json');
...
function themePlugin(opts): PluginOption {
  ...
  return {
    ...
    configureServer(server) {
      function handleThemeFileCreateDelete(themeFile, stats) {
        if (themeFile.startsWith(themeFolder)) {
          const changed = relative(themeFolder, themeFile)
    ...
    handleHotUpdate(context) {
      const contextPath = resolve(context.file);
      const themePath = resolve(themeFolder);
      if (contextPath.startsWith(themePath)) {
        const changed = relative(themePath, contextPath);
    ...
    async resolveId(id, importer) {
      ...
      if (resolve(themeOptions.frontendGeneratedFolder, "theme.js") === importer &&
            !existsSync(resolve(themeOptions.frontendGeneratedFolder, id))) {
      ...
      for (const location of [themeResourceFolder, frontendFolder]) {
        const result = await this.resolve(resolve(location, id));
        if (result) {
          return result;
        }
      }
    ...
    async transform(raw, id, options) {
      ...
      return rewriteCssUrls(raw, dirname(bareId), resolve(themeFolder, themeName), console, opts);
    }
  };
}
...
const allowedFrontendFolders = [
  frontendFolder,
  resolve(generatedFlowImportsFolder), // Contains only generated-flow-imports
  nodeModulesFolder
];
...
export const vaadinConfig: UserConfigFn = (env) => {
  ...
  return {
    ...
    build: {
      ...
      rollupOptions: {
        input: {
          indexhtml: resolve(frontendFolder, 'index.html'),

          ...(hasExportedWebComponents ? { webcomponenthtml: resolve(frontendFolder, 'web-component.html') } : {})
        },
...
function getVersion(module: string):string {
  const packageJson = resolve(nodeModulesFolder, module, 'package.json');

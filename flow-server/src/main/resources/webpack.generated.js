...
const clientSideWebComponentHTML = '[to-be-generated-by-flow]';
const clientSideWebComponentEntryPoint = '[to-be-generated-by-flow]';
...
const outputIndexHtmlPath = 'index.html';
const outputWebComponentHtmlPath = 'web-component.html';
...
const hasExportedWebComponents = fs
  .readdirSync(path.dirname(fileNameOfTheFlowGeneratedMainEntryPoint))
  .filter((fileName) => {
    return (
      !fileName.startsWith(path.basename(fileNameOfTheFlowGeneratedMainEntryPoint)) &&
...
if (hasExportedWebComponents) {
  ...
  webPackEntries.webcomponent = clientSideWebComponentEntryPoint;
}
...
module.exports = {
  ...
  plugins: [
    ...
    // Includes the JS bundle in `index.html`.
    useClientSideIndexFileForBootstrapping &&
      new HtmlWebpackPlugin({
        template: clientSideIndexHTML,
        filename: outputIndexHtmlPath,
        inject: 'head',
        scriptLoading: 'defer',
        chunks: ['bundle']
      }),

    // Includes JS chunks needed for exported web components in `web-component.html`.
    hasExportedWebComponents &&
      new HtmlWebpackPlugin({
        template: clientSideWebComponentHTML,
        filename: outputWebComponentHtmlPath,
        inject: 'head',
        scriptLoading: 'defer',
        chunks: ['webcomponent']
      }),

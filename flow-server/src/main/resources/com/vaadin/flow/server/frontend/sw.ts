...
// Deliberately do NOT eagerly activate this worker (no immediate-activation
// call): a worker generated by a new frontend build must not activate while
// pages from the previous build are still open. If it did, it would take over
// those pages mid-session and 404 the old-hash bundle they reference (the
// previous build's files are gone from both the new precache and the
// redeployed server), leaving a permanent blank screen. Staying in "waiting"
// until all clients are gone hands control over cleanly between page loads
// instead of mid-load. See #24800.
//
// clientsClaim() is kept: it only takes effect once this worker activates
// (after the old clients have closed) and on the very first install, where it
// lets the initial page load be served by the worker without a reload - which
// is what makes offline-on-first-load work.
clientsClaim();

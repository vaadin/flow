...
target=$(curl -s -H "Authorization: token $GITHUB_TOKEN" https://api.github.com/repos/$GITHUB_REPOSITORY/pulls/$PR|jq .base.ref|cut -d\" -f 2)
head=$(curl -s -H "Authorization: token $GITHUB_TOKEN" https://api.github.com/repos/$GITHUB_REPOSITORY/pulls/$PR|jq .head.sha|cut -d\" -f 2)
commits=$(git log origin/$target..$head --reverse --oneline|cut -d' ' -f 1)
prtitle=$(curl -s -H "Authorization: token $GITHUB_TOKEN" https://api.github.com/repos/$GITHUB_REPOSITORY/pulls/$PR|jq .title|sed "s/^.//"|sed "s/.$//")
BRANCH=pr-$PR
git branch -D $BRANCH

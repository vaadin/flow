...
public abstract class ComponentTest {
    ...
    @Before
    public void setup()
            throws IntrospectionException, InstantiationException, IllegalAccessException, ClassNotFoundException {
        component = createComponent();
        whitelistProperty("visible");
        addProperties();
        BeanInfo componentInfo = Introspector.getBeanInfo(component.getClass());
        Stream.of(componentInfo.getPropertyDescriptors())
                .filter(descriptor -> !WHITE_LIST.contains(descriptor.getName())).forEach(this::assertProperty);
    }
    ...
    protected void addStringProperty(String propertyName, String defaultValue, boolean removeDefault) {
    ...
    protected <U> void addProperty(String propertyName, Class<U> propertyType, U defaultValue, boolean isOptional) {
        addProperty(propertyName, propertyType, defaultValue, isOptional, true);
    }

    protected <U> void addProperty(String propertyName, Class<U> propertyType, U defaultValue, boolean isOptional,
            boolean removeDefault) {
        addProperty(propertyName, propertyType, defaultValue, null, isOptional, removeDefault);
    }

    protected <U> ComponentProperty addProperty(String propertyName, Class<U> propertyType, U defaultValue,
            U otherValue, boolean isOptional, boolean removeDefault) {
        final ComponentProperty property = new ComponentProperty(getComponent().getClass(), propertyName, propertyType,
                defaultValue, otherValue, isOptional, removeDefault);
        properties.add(property);
        return property;
    }

    protected <U> void addProperty(String propertyName, Class<U> propertyType, U defaultValue, U otherValue,
            boolean isOptional, boolean removeDefault, String propertyOrAttributeTag) {
        ComponentProperty property = new ComponentProperty(getComponent().getClass(), propertyName, propertyType,
                defaultValue, otherValue, isOptional, removeDefault);
    ...
    protected void testHasAriaLabelIsImplemented() {
        if (!(component instanceof HasAriaLabel)) {
            Assert.fail("Component " + component.getClass() + " did not implement HasAriaLabel anymore.");
        }
    }

    protected void testHasAriaLabelIsNotImplemented() {
        if (component instanceof HasAriaLabel) {
            Assert.fail("Component " + component.getClass() + " implemented HasAriaLabel. This is not valid."
    ...
    public void testSetterGetterTypes() throws Exception {
        for (ComponentProperty property : properties) {
            ...
            if (property.optional) {
                String message = "Getter for " + property.name + " should return Optional<"
                        + property.type.getSimpleName() + ">";
                Assert.assertEquals(message, Optional.class, getterType);
                Assert.assertEquals(message, property.type,
                        getOptionalType(property.getGetter().getGenericReturnType()));
            } else {
                Assert.assertEquals(property.type, getterType);
            }

            Assert.assertEquals(property.type, property.getSetter().getParameterTypes()[0]);
        }
    }
    ...
    public void testNullForOptionalNonStringProperties() {
        Stream<ComponentProperty> optionalNonStringProperties = properties.stream()
                .filter(ComponentProperty::isOptional).filter(p -> p.type != String.class);

        optionalNonStringProperties.forEach(this::testNullForOptionalNonStringProperty);
    }

    @Test
    public void testNullForOptionalStringProperties() {
        getOptionalStringProperties().forEach(this::testNullForOptionalStringProperty);
    }

    @Test
    public void testEmptyStringForOptionalStringProperties() {
        getOptionalStringProperties().forEach(this::testEmptyStringForOptionalStringProperty);
    }

    @Test
    public void testNullForStringPropertiesWithEmptyStringDefault() {
        properties.stream().filter(p -> !p.isOptional()).filter(p -> p.type == String.class)
                .forEach(p -> testNullForStringPropertyWithEmptyStringDefault(p));
    }
    ...
    public void setTitle() {
        ...
        Assert.assertEquals("myTitle", component.getElement().getAttribute("title"));
    ...
    private Stream<ComponentProperty> getOptionalStringProperties() {
        return properties.stream().filter(ComponentProperty::isOptional).filter(p -> p.type == String.class);
    }

    private void testNullForStringPropertyWithEmptyStringDefault(ComponentProperty p) {
        try {
            p.setUsingSetter(component, null);
            Assert.fail("Setting '" + p.name + "' with \"\" default to null should throw an exception");
    ...
    private void testEmptyStringForOptionalStringProperty(ComponentProperty p) {
        try {
            ...
            Assert.assertEquals("The getter for '" + p.name + "' should return an empty optional after setting \"\"",
    ...
    private void testNullForOptionalNonStringProperty(ComponentProperty p) {
        try {
            ...
            throw new AssertionError(
                    "The optional property '" + p.name + "' does not work when setting the value to null", e);
        }
    ...
    private void testNullForOptionalStringProperty(ComponentProperty p) {
        try {
            p.setUsingSetter(component, null);
            Assert.fail("Setting an optional String attribute to null should throw an exception");
        } catch (InvocationTargetException e) {
            Assert.assertEquals(IllegalArgumentException.class, e.getCause().getClass());
        } catch (IllegalAccessException | IllegalArgumentException | NoSuchMethodException | SecurityException e) {
            throw new AssertionError(e);
    ...
    private void assertDefaultValues() throws Exception {
        for (ComponentProperty property : properties) {
            if (property.optional) {
                String message = "Default value for " + property.name + " should be an empty Optional";
                Assert.assertEquals(message, Optional.empty(), property.getUsingGetter(component));
            } else {
                Assert.assertEquals(property.defaultValue, property.getUsingGetter(component));
            }
    ...
    private void assertNonDefaultValues() throws Exception {
        for (ComponentProperty property : properties) {
            if (property.optional) {
                AssertUtils
                        .assertEquals(
                                "Getter for " + property.name + " should return Optional<"
                                        + property.type.getSimpleName() + ">",
                                Optional.ofNullable(property.otherValue), property.getUsingGetter(component));
            } else {
                Assert.assertEquals(property.otherValue, property.getUsingGetter(component));
            }
            assertPropertyOrAttribute(component, property.propertyOrAttributeTag);
        }
    }
    ...
    private static void assertProperty(Component component, String property, boolean present) {
        Assert.assertEquals(present, component.getElement().hasProperty(property));
    }

    private static void assertAttribute(Component component, String attribute, boolean present) {
        Assert.assertEquals("Component should " + (present ? "" : "not ") + "have the attribute " + attribute, present,
                component.getElement().hasAttribute(attribute));
    }

    private static void assertNoPropertyOrAttribute(Component component, String propertyOrAttribute) {
        assertProperty(component, propertyOrAttribute, false);
        assertAttribute(component, propertyOrAttribute, false);
    }

    /**
     * Asserts that the component has either property or attribute with given name; fails if both are present or neither
     * is present.
     *
     * @param component
    ...
    private static void assertPropertyOrAttribute(Component component, String propertyOrAttribute) {
        ...
        Assert.assertNotEquals(
                propertyOrAttribute + ": attribute=" + element.hasAttribute(propertyOrAttribute) + ", property="
                        + element.hasProperty(propertyOrAttribute),
                element.hasAttribute(propertyOrAttribute), element.hasProperty(propertyOrAttribute));
    }
    ...
    private void assertProperty(PropertyDescriptor descriptor) {
        if (descriptor.getReadMethod() != null && descriptor.getWriteMethod() != null) {
            ...
            if (descriptor.getReadMethod().isDefault() && descriptor.getWriteMethod().isDefault()) {
                return;
            }
            if (!hasProperty(descriptor.getName())) {
                throw new IllegalStateException("Property information for '" + descriptor.getName() + "' missing");
            }
        }
    }
}

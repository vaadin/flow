...
The `dom-repeat` element marks the template content that is generated for each item in a list.
...
[NOTE]
There is another obsolete way to use the `dom-repeat` element. The `template is="dom-repeat"` declaration 
allows do the same except that there is no inner `template` element. 

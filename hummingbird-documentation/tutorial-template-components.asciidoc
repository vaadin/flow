...
The `@Id` annotation is used here to map an element defined in the template with the `"content"` identifier.
An html element instance mapped to the appropriate template element and declared type 
is created automatically and injected to the `container` field. 
...
[NOTE]
Declared type used in `@Id` injection declaration must have default constructor to be able to instantiate it. 

...
PolymerTemplate defines a special syntax `on-_event_="javascriptCode"` for attaching client side event handlers to elements:
...
To wire the event to a Polymer.Element method you can write a template as:
[source,html]
----
...
So to listen to handleClick on the server you could have the template as:
[source,html]
----

...
[NOTE]
If you use the `pushState` or `replaceState`, the framework internal scroll position restoration
on navigation won't work, as it uses the `history.state` to keep track of the scroll positions.

...
== Templates
Instead of writing Java code for building the DOM from individual elements, it's also possible to use the `Template` component to define the overall DOM structure in a HTML template file and then use a model to control the contents of the elements.

...
The framework provides a helper called `PropertyDescriptor` which makes managing attributes and properties in a component easier. You can use `PropertyDescriptor` to define the property name and the default value in a single place and use the descriptor from the setter and getter methods:

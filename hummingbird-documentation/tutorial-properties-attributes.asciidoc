...
You can fetch the text of an element using the `getText` or `getTextRecursively` methods. `getText` only looks at the text inside the actual element, whereas `getTextRecursively` looks at all the text in the element tree.

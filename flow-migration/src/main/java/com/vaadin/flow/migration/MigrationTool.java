...
public class MigrationTool {
    ...
    public static void main(String[] args)
        ...
        Logger logger = LoggerFactory.getLogger(MigrationTool.class);

        logger.debug("The base dir  is {}", command.getOptionValue(BASE_DIR));

        File compiledClasses = new File(command.getOptionValue(CLASSES_DIR));
        builder.setCompiledClassDirectory(compiledClasses);

        logger.debug("The classes directory is {}",
                command.getOptionValue(CLASSES_DIR));

        if (command.hasOption(STOP_ON_ERROR)) {
            builder.setIgnoreModulizerErrors(false);
            logger.debug("Ignore modulizer errors value is false");
        } else {
            logger.debug("Ignore modulizer errors value is true");
        }
        ...
        if (resourceDirs != null) {
            ...
            logger.debug("The resource directories are {}", folders);
        } else {
            logger.debug(
        ...
        if (tempMigrationFolder != null) {
            ...
            logger.debug("The temporary migration directory is {}",
                    tempMigrationFolder);
        } else {
            logger.debug(
        ...
        if (targetDir != null) {
            ...
            logger.debug("The target directory is {}", targetDir);
        } else {
            logger.debug(
        ...
        if (command.hasOption(KEEP_ORIGINAL)) {
            ...
            logger.debug("Keep original resources value is true");
        } else {
            logger.debug("Keep original resources value is false");
        }
        ...
        logger.debug("The java source directories are {}", sourceRoots);

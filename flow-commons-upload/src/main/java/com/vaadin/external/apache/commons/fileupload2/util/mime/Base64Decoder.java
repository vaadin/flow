...
final class Base64Decoder {
    ...
    /**
     * Decoding table value for padding bytes, so can detect PAD after
     * conversion.
     */
     ...
     * unsigned int, so must be at least as large as the number of different
     * byte values, positive and negative and zero.
     */
     ...
     * @throws IOException
     *             thrown when the padding is incorrect or the input is
     *             truncated.
     */
    public static int decode(final byte[] data, final OutputStream out)
            throws IOException {
        int outLen = 0;
        ...
        for (final byte b : data) {
            ...
            if (cachedBytes == INPUT_BYTES_PER_CHUNK) {
                ...
                out.write((b1 << 2) | (b2 >> 4)); // 6 bits of b1 plus 2 bits of
                                                  // b2
                outLen++;
                if (b3 != PAD_BYTE) {
                    // CHECKSTYLE IGNORE MagicNumber FOR NEXT 1 LINE
                    out.write((b2 << 4) | (b3 >> 2)); // 4 bits of b2 plus 4
                                                      // bits of b3
                    outLen++;
                    if (b4 != PAD_BYTE) {
                        // CHECKSTYLE IGNORE MagicNumber FOR NEXT 1 LINE
                ...
                } else if (b4 != PAD_BYTE) { // if byte 3 is pad, byte 4 must be
                                             // pad too

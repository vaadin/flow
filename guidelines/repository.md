...
to `false` in its `pom.xml` to get one; the parent POM skips it otherwise.
`vaadin-spring` and `vaadin-cdi` are left out on purpose, since neither
runs under Quarkus, and so are tooling and test modules.

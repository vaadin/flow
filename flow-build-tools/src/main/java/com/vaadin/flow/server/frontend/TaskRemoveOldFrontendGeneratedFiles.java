...
public class TaskRemoveOldFrontendGeneratedFiles implements FallibleCommand {
    ...
    private Predicate<Path> isKnownUnhandledFile() {
        ...
        // Files listed by Hilla endpoint generation.
        knownFiles.addAll(filesFromGeneratedList("generated-file-list.txt"));
        // Files generated by a previous full Flow generation run (e.g. the dev
        // server with hot deploy), which the current run may not regenerate.
        knownFiles.addAll(filesFromGeneratedList(
    ...
    /**
     * Reads a manifest of generated file paths, relative to the frontend
     * generated folder, and resolves them against that folder. The manifest
     * itself is always treated as a known file.
     *
     * @param listFileName
     *            name of the manifest file inside the frontend generated folder
     * @return the set of known generated files, including the manifest itself
     */
    private Set<Path> filesFromGeneratedList(String listFileName) {
        Set<Path> generatedFiles = new HashSet<>();
        Path generatedFilesList = frontendGeneratedFolder.resolve(listFileName);
        generatedFiles.add(normalizePath(generatedFilesList));
        if (Files.exists(generatedFilesList)) {
            try {
                Files.readAllLines(generatedFilesList).stream()
                ...
                LOGGER.debug("Cannot read generated files list {}",
                        listFileName);
            }
        }

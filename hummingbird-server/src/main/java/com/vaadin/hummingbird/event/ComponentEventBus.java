...
public class ComponentEventBus implements Serializable {
    ...
    HashMap<Class<? extends ComponentEvent>, EventRegistrationHandle> domEventRemovers = new HashMap<>();
    HashMap<Class<? extends ComponentEvent>, List<Consumer<? extends ComponentEvent>>> allEventListeners = new HashMap<>();
    ...
    public <T extends ComponentEvent> EventRegistrationHandle addListener(
        ...
        List<Consumer<? extends ComponentEvent>> listeners = allEventListeners
                .computeIfAbsent(eventType, t -> new ArrayList<>(1));
    ...
    public void fireEvent(ComponentEvent event) {
        List<Consumer<ComponentEvent>> listeners = (List) allEventListeners
                .get(event.getClass());
        if (listeners == null) {
            return;
        }

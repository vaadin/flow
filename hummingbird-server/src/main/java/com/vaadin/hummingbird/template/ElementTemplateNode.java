...
public class ElementTemplateNode extends AbstractElementTemplateNode {
    ...
    protected void populateJson(JsonObject json) {
        ...
        bindingsToJson(properties).ifPresent(v -> json.put("properties", v));

        bindingsToJson(attributes).ifPresent(v -> json.put("attributes", v));

        bindingsToJson(classNames).ifPresent(v -> json.put("classNames", v));

        mapToJson(eventHandlers, Json::create)
                .ifPresent(v -> json.put("eventHandlers", v));

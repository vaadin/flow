...
public class TemplateModelBeanUtil {
    ...
    static void importBeanIntoModel(StateNode parentNode, Object bean,
        ...
        // resolve the state node for the nested bean when needed
        Supplier<StateNode> stateNodeSupplier = () -> {
            ModelMap parentLevel = parentNode.getFeature(ModelMap.class);
            if (parentLevel.hasValue(beanPath)) {
                Serializable value = parentLevel.getValue(beanPath);
                if (value instanceof StateNode
                    ...
                    // just override
                    return createModelMap(parentLevel, beanPath);
                }
            } else {
                return createModelMap(parentLevel, beanPath);
            }
        ...
        importBeanIntoModel(stateNodeSupplier, bean);
    }
    ...
    static Object getModelValue(ModelMap modelMap, String propertyName,
        ...
        if (returnType instanceof Class<?>) {
            ...
            if (returnClazz.isPrimitive()) {
                ...
                // #731 consider as a "sub" model or nested bean and return a
                // new proxy to model
            }
        ...
        throw new UnsupportedOperationException(
                "Template model does not yet support type "
                        + returnType.getTypeName() + " (" + propertyName
                        + "), supported types are:"
                        + getSupportedTypesString());
    }
    ...
    private static boolean isSupportedPrimitiveType(Class<?> primitiveType) {
        return primitiveType == int.class || primitiveType == double.class

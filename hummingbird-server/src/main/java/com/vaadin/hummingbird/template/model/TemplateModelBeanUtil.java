...
public class TemplateModelBeanUtil {
        ...
        void mapBeanToModel(Method beanGetter) {
            try {
                ...
                invocationHandler.setModelValue(
                        ReflectTools.getPropertyNameFromMethod(beanGetter),
    ...
    static void importBeanIntoModel(TemplateModel model, Object bean) {
        if (bean == null) {
        ...
        if (Proxy.isProxyClass(model.getClass())) {
            Stream.of(bean.getClass().getMethods())
                    ...
                    .filter(TemplateModelBeanUtil::filterClassMethod)
                    .forEach(new BeanToModelMapper(
                            (TemplateModelProxyHandler) Proxy
                                    .getInvocationHandler(model),
                            bean)::mapBeanToModel);
        } else {
            throw new IllegalArgumentException(
                    "Invalid template model given. You should not try to create model instances, but just return correct type in your template's getModel.");
        }

    }

    private static boolean filterClassMethod(Method method) {

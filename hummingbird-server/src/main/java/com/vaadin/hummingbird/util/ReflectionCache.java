 ...
 * class. Cached values are never expired since it's assumed that the there is a
...
public class ReflectionCache<C, T> {
    ...
    public ReflectionCache(Function<? extends Class<C>, T> valueProvider) {
        if (valueProvider == null) {
            throw new IllegalArgumentException("value provider cannot be nul");
        }
    ...
    public T get(Class<? extends C> type) {
         ...
         */
        return values.computeIfAbsent(type, (Function) valueProvider);
    }

...
public class FileUploadHandler implements RequestHandler {
     ...
     * FailedEvent against this class, it is possible to determine if an upload
     * was interrupted by code or aborted due to any other exception.
     */
    ...
    private void cleanStreamVariable(VaadinSession session,
        ...
        session.accessSynchronously(new Runnable() {
            @Override
            public void run() {
                owner.getUI().getConnectorTracker().cleanStreamVariable(

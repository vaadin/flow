...
public class StaticFileServer {
    ...
    /**
     * Constructs a file server for the given context.
     *
     * @param servletContext
     *            the context to serve files for
     */
    public StaticFileServer(ServletContext servletContext) {
    ...
    public boolean isStaticResourceRequest(HttpServletRequest request) {
        URL resource;
        try {
            resource = servletContext.getResource(request.getRequestURI());
    ...
    public boolean serveStaticResource(HttpServletRequest request,
        ...
        String filenameWithPath = getDecodedURI(request);
        URL resourceUrl = servletContext.getResource(filenameWithPath);
        ...
        writeModificationTimestamp(filenameWithPath, resourceUrl, request,
                response);
    ...
    protected void writeContentType(String filenameWithPath,
        ...
        final String mimetype = servletContext.getMimeType(filenameWithPath);
        if (mimetype != null) {
     ...
     *            the response object
     */
    protected void writeModificationTimestamp(String filenameWithPath,
            URL resourceUrl, HttpServletRequest request,
    ...
    /**
     * Decodes and returns the request URI.
     *
     * @param request
     *            the request object
     * @return the decoded request URI
     */
    private String getDecodedURI(HttpServletRequest request) {
        try {
            return URLDecoder.decode(request.getRequestURI(), "UTF-8");
        } catch (UnsupportedEncodingException e) {
            // If this happens, the world has exploded
            getLogger().log(Level.SEVERE, "UTF-8 encoding is not available", e);
        }
        return null;
    }
     ...
     * containing ".cache." return one year, all other return the value defined
     * in the web.xml using resourceCacheTime (defaults to 1 hour).
     *
     * @param filename
     ...
     *
     * @param filenameWithPath
     *            the decoded filename of the file being sent
     * @param resourceUrl
    ...
    protected void writeData(String filenameWithPath, URL resourceUrl,
        ...
        if (acceptsGzippedResource(request, filenameWithPath)) {
            // try to serve a gzipped version if available
            String gzippedFilenameWithPath = filenameWithPath + ".gz";
            try {
                URL url = servletContext.getResource(gzippedFilenameWithPath);
                if (url != null) {
    ...
    protected boolean acceptsGzippedResource(HttpServletRequest request,
        ...
        return accept != null && accept.contains("gzip")
                && (filenameWithPath.endsWith(".js")
                        || filenameWithPath.endsWith(".css")
                        || filenameWithPath.endsWith(".html"));
    }

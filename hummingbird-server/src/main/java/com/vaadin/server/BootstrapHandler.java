...
public class BootstrapHandler extends SynchronizedRequestHandler {
    ...
    private static void setupDocumentHead(BootstrapContext context,
        ...
        Optional<String> title = getInitialPageTitle(context);
        if (title.isPresent()) {
     ...
     */
    protected static Optional<String> getInitialPageTitle(
            BootstrapContext context) {
        // check for explicitly set page title, eg. by PageTitleGenerator or
        // View level title
        Optional<String> title = context.getUI().getPage().getTitle();
        if (!title.isPresent()) {
            // check for Title annotation in UI
            title = Optional.ofNullable(
                    AnnotationReader.getPageTitle(context.getUI().getClass()));
            if (title.isPresent()) {
                // make the title accessible from page.getTitle() without
                // causing js invocation
                context.getUI().getFrameworkData().setTitle(title.get());
            }
        } else {
            // cancel the unnecessary execute javascript
            context.getUI().getPage().setTitle(null);
        }
        return title;
    }

...
public abstract class BootstrapHandler extends SynchronizedRequestHandler {
    ...
    private static final String CLIENT_ENGINE_NOCACHE_FILE = ApplicationConstants.CLIENT_ENGINE_FOLDER_PATH
            + "client.nocache.js";
    ...
    static {
        ...
        try (InputStream prop = BootstrapHandler.class.getResourceAsStream(
                ApplicationConstants.CLIENT_ENGINE_FOLDER_PATH
                        + "compile.properties")) {
            // null when running SDM or tests
            if (prop != null) {
                Properties p = new Properties();
                p.load(prop);
                clientEngineFile = ApplicationConstants.CLIENT_ENGINE_FOLDER_PATH
    ...
    private static Element getClientEngineScript(BootstrapContext context) {
        return new Element(Tag.valueOf("script"), "")
                ...
                .attr("src", getClientEngineUrl(context.getSession()
    ...
    private static String getClientEngineUrl(boolean productionMode) {
        ...
        if (!productionMode && BootstrapHandler.class
                .getResource(CLIENT_ENGINE_NOCACHE_FILE) != null) {
            return CLIENT_ENGINE_NOCACHE_FILE;
        }

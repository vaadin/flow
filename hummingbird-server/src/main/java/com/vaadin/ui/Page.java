...
public class Page implements Serializable {
     ...
     * which the used {@link VaadinServlet} is mapped. New locations inside the
     * application (i.e. a relative URL without <code>..</code> segments) will
     * immediately trigger a {@link LocationChangeEvent} to be fired. New
     * locations outside the application (e.g. an absolute URL) causes the user
     * to leave the application without firing any {@link LocationChangeEvent}.
     *
     * @param location
     ...
     * {@link VaadinServlet} is mapped. New locations inside the application
     * (i.e. a relative URL without <code>..</code> segments) will immediately
     * trigger a {@link LocationChangeEvent} to be fired. New locations that are
     * not guaranteed to be inside the application (e.g. any absolute URL)
     * causes the user to leave the application without firing any
    ...
    public void setLocation(String location, JsonObject state) {
        ...
        try {
            ...
            if (isExternal) {
                ...
                updateLocation(new Page.LocationChangeEvent(this, state,
                        resolvedLocation));
            }

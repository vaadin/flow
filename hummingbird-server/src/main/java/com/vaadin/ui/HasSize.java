...
public interface HasSize extends HasElement {
    ...
    default void setWidth(String width) {
        if (width == null) {
            getElement().getStyle().remove(PROPERTY_WIDTH);
        } else {
            getElement().getStyle().set(PROPERTY_WIDTH, width);
        }
    }
    ...
    default String getWidth() {
        return getElement().getStyle().get(PROPERTY_WIDTH);
    }
    ...
    default void setHeight(String height) {
        if (height == null) {
            getElement().getStyle().remove(PROPERTY_HEIGHT);
        } else {
            getElement().getStyle().set(PROPERTY_HEIGHT, height);
        }
    }
    ...
    default String getHeight() {
        return getElement().getStyle().get(PROPERTY_HEIGHT);
    }
}

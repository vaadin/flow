...
public class BeanModelTypeTest {
    ...
    @Test
    public void userToModel() {
        BeanModelType<Bean> beanType = new BeanModelType<>(Bean.class,
                PropertyFilter.ACCEPT_ALL);

        Bean bean = new Bean(3);

        StateNode userToModel = beanType.applicationToModel(bean,
                PropertyFilter.ACCEPT_ALL);

        ModelMap model = ModelMap.get(userToModel);

        assertThreeBean(model);
    }

    @Test
    public void userToModel_filtered() {
        BeanModelType<Bean> beanType = new BeanModelType<>(Bean.class,

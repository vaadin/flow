...
public class ComponentEventBusTest {
    ...
    private void assertNoListeners(ComponentEventBus eventBus) {
        Assert.assertTrue(eventBus.allEventListeners.isEmpty());
        Assert.assertTrue(eventBus.domEventRemovers.isEmpty());
    }
    ...
    public void domEvent_constructorCached() {
        TestComponent component = new TestComponent();
        ComponentEventBus.cache.clear();
        assertEmpty(ComponentEventBus.cache
                .getEventConstructor(MappedToDomEvent.class));
        component.addListener(MappedToDomEvent.class, e -> {
        });
        assertPresent(ComponentEventBus.cache
                .getEventConstructor(MappedToDomEvent.class));
    }
    ...
    public void domEvent_dataExpressionCached() {
        ...
        ComponentEventBus.cache.clear();
        assertEmpty(ComponentEventBus.cache
                .getDataExpressions(MappedToDomEvent.class));
        component.addListener(MappedToDomEvent.class, e -> {
        });
        assertPresent(ComponentEventBus.cache
                .getDataExpressions(MappedToDomEvent.class));

 ...
 * linked to a specific history position.
 *
 * @author Vaadin Ltd
 */
public class ScrollPositionHandler {
    ...
    /**
     * The current position in the scroll position arrays.
     */
    private int currentHistoryPosition;

    /**
     * Unique identifier used to bind scroll position arrays and history
     * position together. Used to avoid case of applying wrong scroll positions
     * to a history position with the same index.
     */
    ...
    private void readAndRestoreScrollPositionsFromHistoryAndSessionStorage(
        ...
        if (state != null && state.hasKey(HISTORY_POSITION)
            ...
            if (jsonString != null) {
                ...
                Console.warn(
                        "History.state has scroll history position, but no scroll positions found from session storage matching token <"
                                + historyResetToken
    ...
    private void resetScrollPositionTracking() {
        ...
        historyResetToken = createHistoryResetToken();
    }
    ...
    public void onPopStateEvent(PopStateEvent event,
        ...
        if (state == null || !state.hasKey(HISTORY_POSITION)
                || !state.hasKey(HISTORY_TOKEN)) {
            // state doesn't contain history position info, just log & reset
     ...
     * fragment change events are fired. This is just to distinct the case where
     * a fragment change event fired by framework (to get scroll to fragment)
     * causes a pop state event that should be ignored here.
     *
    ...
    private void restoreScrollPosition(boolean delayAfterResponse) {
        ...
        if (yPositions.length() < currentHistoryPosition
            ...
            Console.warn("No matching scroll position found (entries Y:"
                    ...
                    + ") for opened history position (" + currentHistoryPosition
    ...
    private static native double createHistoryResetToken()
    /*-{
        return Date.now();
    }-*/;

...
public class TreeChangeProcessor {
    public static void processChanges(StateTree tree, JsonArray array) {
        int length = array.length();
        // Attach all nodes before doing anything else
        for (int i = 0; i < length; i++) {
            JsonObject change = array.getObject(i);
            if (isAttach(change)) {
    ...
    private static void processPutChange(JsonObject change, StateNode node) {
        ...
        if (change.hasKey("value")) {
            ...
            Object value = getValue(jsonValue);
    ...
    private static void processSpliceChange(JsonObject change, StateNode node) {
        ...
        if (change.hasKey("add")) {
            ...
            Object[] add;
            if (GWT.isScript()) {
                add = WidgetUtil.crazyJsCast(addJson);
            } else {
                add = new Object[addJson.length()];
                for (int i = 0; i < add.length; i++) {
                    add[i] = getValue(addJson.get(i));
                }
            }

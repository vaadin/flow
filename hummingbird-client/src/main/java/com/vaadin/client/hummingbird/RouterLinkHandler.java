...
public class RouterLinkHandler {
    ...
    /**
     * Adds a click event listener for the given element for handling
     * application navigation related events and sending them to server using
     * the given channel.
     *
     * @param element
     ...
     */
    public static void bind(ServerRpcQueue rpcQueue, Element element) {
        element.addEventListener("click", event -> handleClick(rpcQueue, event),
                false);
    }
    ...
    private static void handleClick(ServerRpcQueue rpcQueue, Event clickEvent) {
        if (isRouterLinkClick(clickEvent) && !hasModifierKeys(clickEvent)) {
            AnchorElement target = (AnchorElement) WidgetUtil
    ...
    private static boolean hasModifierKeys(Event clickEvent) {
        ...
        MouseEvent event = WidgetUtil.crazyJsCast(clickEvent);

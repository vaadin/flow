...
public class ClientJsonCodec {
     ...
     * {@link JsonCodec#encodeComplexValue(Object)}.
     *
     * @param tree
     ...
     */
    public static Object decodeComplexValue(StateTree tree, JsonValue json) {
        if (json.getType() == JsonType.ARRAY) {
            ...
            return decodeBasicValue(json);
        }
    }

    /**
     * Decodes a value encoded on the server using
     * {@link JsonCodec#encodeSimpleValue(Object)}. This is a no-op in compiled
     * JavaScipt since the JSON representation can be used as-is, but some
     * special handling is needed for tests running in the JVM.
     *
     * @param json
    ...
    public static Object decodeBasicValue(JsonValue json) {
        if (GWT.isScript()) {
            return json;
        } else {
    ...
    public static JsArray<Object> jsonArrayAsJsArray(JsonArray jsonArray) {
        ...
        if (GWT.isScript()) {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                jsArray.push(decodeBasicValue(jsonArray.get(i)));
            }
        }
        return jsArray;
    }

...
public class ExecuteJavaScriptProcessor {
    ...
    private void handleInvocation(JsonArray execute) {
        StateTree tree = registry.getStateTree();

        // First item is the script, the rest is parameters
        int parameterCount = execute.length() - 1;

        String[] parameterNamesAndCode = new String[parameterCount + 1];
        JsArray<Object> parameters = JsCollections.array();

        for (int i = 0; i < parameterCount; i++) {
            // i + 1 to bypass the first item which is the script
            JsonValue parameterJson = execute.get(i + 1);
            Object parameter = ClientJsonCodec.decodeComplexValue(tree,
                    parameterJson);
        ...
        String expression = execute.getString(0);

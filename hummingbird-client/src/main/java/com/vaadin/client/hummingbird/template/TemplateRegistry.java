...
public class TemplateRegistry {
    ...
    public void importFromJson(JsonObject templatesJson) {
        ...
        for (String key : templatesJson.keys()) {
            ...
            TemplateNode template = WidgetUtil.crazyJsCast(templateJson);

            register(Integer.parseInt(key), template);
        }
    }
     ...
     */
    public void register(int id, TemplateNode template) {
        assert template != null;

        Double key = Double.valueOf(id);
        assert !instanceMap.has(key);

        instanceMap.set(key, template);
    }
    ...
    public TemplateNode get(int id) {
        TemplateNode template = instanceMap.get(Double.valueOf(id));

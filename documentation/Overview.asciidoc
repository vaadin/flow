...
External JavaScript or stylesheet dependencies can be added to the page using `Page.addDependency(Dependency)`.
The URL provided to the `Dependency(Type, String)` constructor can be a regular absolute URL or use one of the special protocols `app://` and `vaadin://`.
...
Relative URLs might technically work, their use is not recommended since they may point to unexpected locations depending on how the application is deployed and what URL is used to open the application.

 ...
 * the License.
 */
package com.vaadin.flow.testutil.jupiter;
...
@LocalExecution
public abstract class AbstractTestBenchTest extends BrowserTestBase {
    ...
    public static abstract class ClientChecker {
         ...
         */
        private static boolean isClientRouter(BrowserTestBase test) {
    ...
    protected void verifyScreenshot(String identifier) throws IOException {
        Assertions.assertTrue(testBench().compareScreen(identifier),
     ...
     * run and before running that, {@code DesiredCapabilities} are set with the

...
public class SplitPanelWithMinimumAndMaximumTest extends MultiBrowserTest {
    ...
    private double parseHorizontalPosition(TestBenchElement splitter, boolean left) {
        if (left) {
            return Double.parseDouble(splitter.getCssValue("left").replace("px", ""));
        } else {
            return Double.parseDouble(splitter.getCssValue("right").replace("px", ""));
        }
    }

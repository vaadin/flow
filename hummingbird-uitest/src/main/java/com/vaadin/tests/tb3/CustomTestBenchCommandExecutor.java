...
public class CustomTestBenchCommandExecutor {
     ...
     */
    public boolean compareScreen(WebElement element, File reference, boolean isIE8) throws IOException {
     ...
     */
    public boolean compareScreen(WebElement element, BufferedImage reference, String referenceName, boolean isIE8) throws IOException {
        for (int times = 0; times < Parameters.getMaxScreenshotRetries(); times++) {
            BufferedImage screenshotImage = cropToElement(element, ImageIO.read(new ByteArrayInputStream(((TakesScreenshot) actualDriver).getScreenshotAs(OutputType.BYTES))), isIE8);
            if (reference == null) {
     ...
     * @throws IOException
     */
    private BufferedImage cropToElement(WebElement element, BufferedImage fullScreen, boolean isIE8) throws IOException {

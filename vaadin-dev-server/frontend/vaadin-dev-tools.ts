        ...
        <button
          @click=${() => {
            ...
            const checkboxes: any[] = Array.from(
              this.renderRoot.querySelector('#inputs')!.querySelectorAll("input[type='checkbox']")
            );
            const locales = checkboxes.filter((cb) => cb.checked).map((cb) => cb.value);
            const uiId = (window.Vaadin as any).Flow.clients.ROOT.getUIId();
            this.frontendConnection!.send('translate', { locales, uiId });
          }}
        >
          Translate it
        </button>

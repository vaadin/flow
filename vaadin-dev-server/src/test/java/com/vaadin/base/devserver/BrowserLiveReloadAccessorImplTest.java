...
public class BrowserLiveReloadAccessorImplTest {
    ...
    public void getLiveReload_liveReloadDisabled_instanceIsCreated() {
        ...
        Mockito.when(context.getAttribute(Mockito.eq(BrowserLiveReload.class), Mockito.any()))
    ...
    public void getLiveReload_devMode_contextHasReloadInstance_instanceIsReturned() {
        ...
        Mockito.when(service.getContext().getAttribute(Mockito.eq(BrowserLiveReload.class), Mockito.any()))
                .thenReturn(reload);

        Assert.assertSame(reload, access.getLiveReload(service));
    }

    private VaadinService mockConfiguration(boolean productionMode, boolean liveReloadEnabled) {
        ...
        Mockito.when(config.isDevModeLiveReloadEnabled()).thenReturn(liveReloadEnabled);

...
safe-outputs:
  create-pull-request:
    ...
    # The documentation pull request goes to whoever wrote the change, which is
    # the one person who can tell whether it describes it correctly. This has to
    # happen here rather than through the `assign-to-user` safe-output: that one
    # needs the number of the pull request to assign, and the documentation pull
    # request does not have one until this handler has created it, so the agent
    # has nothing to name. The handler assigns the pull request it just created,
    # and logs a warning instead of failing the run when the author has no
    # access to vaadin/docs.
...
The pull request is assigned to `@${{ env.PR_AUTHOR }}` for you, so do not try to assign it yourself. An author without access to `vaadin/docs` is left unassigned and the run carries on; this is not a failure.
...
Do not rewrite the pull request description; the reviewer already has it, and the assignee an earlier run set stays as it is.

...
class DatabaseHelper {
    ...
    private void initDatabase() {
        try {
            ...
            case HSQLDB:
                ...
                statement.execute("create table " + LARGE_TABLENAME + " (id integer GENERATED BY DEFAULT AS IDENTITY, field1 varchar(100), primary key(id))");
                break;
            case MYSQL:
                ...
                statement.execute("create table " + LARGE_TABLENAME + " (id integer auto_increment not null, field1 varchar(100), primary key(id))");
                break;
            case POSTGRESQL:
                statement.execute("create table " + TABLENAME + " (\"id\" serial primary key, \"field1\" varchar(100), \"field2\" boolean)");
                statement.execute("create table " + LARGE_TABLENAME + " (\"id\" serial primary key, \"field1\" varchar(100))");
                break;
            }
            ...
            for (int i = 0; i < 400; ++i) {
                statement.executeUpdate("insert into " + LARGE_TABLENAME + " values(default, 'User " + i + "')");
            }
    ...
    private void initConnectionPool() {
        try {
            connectionPool = new SimpleJDBCConnectionPool(SQLTestsConstants.dbDriver, SQLTestsConstants.dbURL, SQLTestsConstants.dbUser, SQLTestsConstants.dbPwd, 2, 5);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }

...
public class ComboBoxSQLContainerFilteredValueChange extends TestBase {
    ...
    private void createTestTable(JDBCConnectionPool connectionPool) {
        ...
        try {
            ...
            statement.execute("CREATE TABLE mytable " + "(id INTEGER GENERATED BY DEFAULT AS IDENTITY, " + "MYFIELD VARCHAR(45), " + "PRIMARY KEY(ID))");
            statement.close();
            conn.commit();
        } catch (SQLException e) {
            e.printStackTrace();
     ...
     * @throws SQLException
     */
    private void insertTestData(JDBCConnectionPool connectionPool) throws SQLException {
        Connection conn = null;
        try {
            conn = connectionPool.reserveConnection();
            Statement statement = conn.createStatement();

...
/**
 * Factory for {@link DefaultI18NProvider}. Factory creates I18N provider for locales based on all .properties files in
 * /vaadin-i18n folder in the classpath including JAR contents.
 */
public class DefaultI18NProviderFactory implements Condition, Serializable {
    ...
    /**
     * Creates new instance of {@link DefaultI18NProvider} with the given location pattern.
     *
     * @param locationPattern
     *            location pattern for {@link ResourcePatternResolver} to find translation files for available locales.
    ...
    public static DefaultI18NProvider create(String locationPattern) {
        try {
            ...
            if (translations.length > 0) {
                List<Locale> locales = I18NUtil.collectLocalesFromFileNames(Arrays.stream(translations)
                        .map(Resource::getFilename).filter(Objects::nonNull).collect(Collectors.toList()));
    ...
    public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
        try {
            String locationPattern = context.getEnvironment().getProperty("vaadin.i18n.location-pattern", String.class,
                    DEFAULT_LOCATION_PATTERN);
            Resource[] translations = getTranslationResources(locationPattern);
            return translations.length > 0;
        } catch (IOException e) {
            LoggerFactory.getLogger(DefaultI18NProviderFactory.class)
                    .error("Unable to detect if DefaultI18NProvider instance is needed.", e);
            return false;
        }
    }
    ...
    private static Resource[] getTranslationResources(String locationPattern) throws IOException {
        ...
        return resourcePatternResolver
                .getResources(locationPattern != null ? locationPattern : DEFAULT_LOCATION_PATTERN);
    }
}

...
public class SpringInstantiator extends DefaultInstantiator {
            ...
            if (loggingEnabled.compareAndSet(true, false)) {
                LoggerFactory.getLogger(SpringInstantiator.class.getName())
                        .info("The number of beans implementing '{}' is {} why spring could not provide a unique bean. "
                                ...
                                I18NProvider.class.getSimpleName(), context.getBeanNamesForType(I18NProvider.class).length);
            }
            ...
            if (loggingEnabled.compareAndSet(true, false)) {
                LoggerFactory.getLogger(SpringInstantiator.class.getName())
                        .info("The number of beans implementing '{}' is {} why spring could not provide a unique bean. "
                                ...
                                MenuAccessControl.class.getSimpleName(), context.getBeanNamesForType(MenuAccessControl.class).length);
            }
    ...
    public <T> T getOrCreate(Class<T> type) {
        try {
            ...
            // there are multiple beans of this type in the context, but spring couldn't figure out which one to use
            // to stay compatible with previous implementation we try to create a new bean
            // this will always fail if type is an interface or abstract though so we won't try but give a meaningful message instead

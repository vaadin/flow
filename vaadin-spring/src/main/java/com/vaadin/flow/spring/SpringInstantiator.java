...
/**
 * Default Spring instantiator that is used if no other instantiator has been registered. This implementation uses
 * Spring beans.
 *
 * @author Vaadin Ltd
 */
public class SpringInstantiator extends DefaultInstantiator {
    ...
    public SpringInstantiator(VaadinService service, ApplicationContext context) {
        ...
        UsageStatistics.markAsUsed("SpringFramework",
                Optional.ofNullable(SpringVersion.getVersion()).orElse("unknown"));
        if (SpringUtil.isSpringBoot()) {
            UsageStatistics.markAsUsed("SpringBoot", SpringBootVersion.getVersion());
        }
    }

    /**
     * Gets all service init listeners to use. Registers by default an init listener that publishes ServiceInitEvent to
     * application context.
     *
    ...
    public Stream<VaadinServiceInitListener> getServiceInitListeners() {
        ...
        }), context.getBeansOfType(VaadinServiceInitListener.class).values().stream());
        ...
        if (beansCount == 1) {
            ...
            if (loggingEnabled.compareAndSet(true, false)) {
                LoggerFactory.getLogger(SpringInstantiator.class.getName()).info(
                        "The number of beans implementing '{}' is {}. Cannot use Spring beans for Menu Access Control, "
     ...
     * <li>If more than one bean is present, it tries to instantiate one but in case of a Bean instantiation exception
     * this exception is catched and rethrown with a hint. Reason for this is, that users may expect it to "use" a bean
     * but have multiple in the context. So the hint helps them find the problem.</li>
     * </ul>
     */
    @Override

 ...
 * <p>
 * To configure Flow navigation access control, a Spring bean on type {@link NavigationAccessControlConfigurer} should
 * be defined.
 * <p>
 * </p>
 * In Spring Boot applications, a default {@link NavigationAccessControlConfigurer} bean is provided. It activates
 * {@link AnnotatedViewAccessChecker}, but it disables the {@link NavigationAccessControl}, for backward compatibility.
 * <p>
 * </p>
 * However, if Spring Security is configured extending {@link VaadinWebSecurity}, the {@link NavigationAccessControl} is
 * enabled automatically.
 * <p>
 * </p>
 *
 * Default settings can be overridden by defining a custom {@link NavigationAccessControlConfigurer} bean.
 *
 * <pre>
 * {@code @Bean
 ...
 * <p>
 * </p>
 * NOTE: if the bean in exposed in a configuration class that extends {@link VaadinWebSecurity}, the method must be
 * defined {@code static} to prevent cyclic dependencies errors.
 *
 * <pre>
 * {@code @Bean
 ...
 * <p>
 * </p>
 * The {@link NavigationAccessControl} will automatically be disabled if no navigation access checkers are provided.
 *
...
public final class NavigationAccessControlConfigurer {
    ...
    /**
     * Enables the {@link com.vaadin.flow.server.auth.AnnotatedViewAccessChecker}.
     *
    ...
    /**
     * Adds the given {@link NavigationAccessChecker} to the collection of checker that will be used by
    ...
    public NavigationAccessControlConfigurer withNavigationAccessChecker(NavigationAccessChecker accessChecker) {
        additionalCheckers.add(Objects.requireNonNull(accessChecker, "navigation access checker must not be null"));
        return this;
    }
    ...
    public NavigationAccessControlConfigurer withNavigationAccessCheckers(
        ...
        additionalCheckers.addAll(accessChecker.stream().filter(Objects::nonNull).toList());
        return this;
    }

    /**
     * Adds to {@link com.vaadin.flow.server.auth.NavigationAccessControl} all the registered
     * {@link NavigationAccessChecker} beans that matches the given filter.
     * <p>
     * Note that the filter will not be applied to out-of-the-box checkers enabled with
     * {@link #withAnnotatedViewAccessChecker()} or {@link #withRoutePathAccessChecker()}, not to checkers added within
     * this configurer instance through {@link #withNavigationAccessChecker(NavigationAccessChecker)} and
    ...
    /**
     * Sets the {@link AccessCheckDecisionResolver} for the navigation access control.
     * <p>
     * The {@link AccessCheckDecisionResolver} is responsible for taking the final decision on target view access grant,
     * based on the response of the navigation access checkers.
     *
     * @param resolver
    ...
    /**
     * Builds a {@link NavigationAccessControl} instance, configured according to this configurer instance settings.
     *
     * @param factory
     *            a function that build the desired type of navigation access control.
     * @param availableCheckers
     *            the list of all available navigation access checkers registered in the system.
     * @return a configured {@link NavigationAccessControl} instance, never {@literal null}.
     * @param <T>
    ...
    public <T extends NavigationAccessControl> T build(
        ...
        List<NavigationAccessChecker> availableCopy = new ArrayList<>(availableCheckers);
        List<NavigationAccessChecker> checkerList = new ArrayList<>();
        if (enableViewAccessChecker) {
            checkerList.add(extractRegisteredChecker(AnnotatedViewAccessChecker.class, availableCopy));
        }
        if (enablePathAccessChecker) {
            checkerList.add(extractRegisteredChecker(RoutePathAccessChecker.class, availableCopy));
        }
        ...
        if (accessCheckersFilter != null) {
            availableCopy.stream().filter(accessCheckersFilter).collect(Collectors.toCollection(() -> checkerList));
        }
    ...
    private static NavigationAccessChecker extractRegisteredChecker(
            Class<? extends NavigationAccessChecker> checkerType, List<NavigationAccessChecker> checkerList) {
        NavigationAccessChecker result = checkerList.stream().filter(checker -> checkerType.equals(checker.getClass()))
                .findFirst().orElseThrow(() -> new IllegalStateException(checkerType.getName() + " is not available"));

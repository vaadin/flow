 ...
 * <p>
 *
 * It allows to access authenticated user information and to initiate the logout process.
 *
 * An instance of this class is available for injection as bean in view and layout classes. The class is not
 * {@link java.io.Serializable}, so potential referencing fields in Vaadin views should be defined {@literal transient}.
 *
 * @author Vaadin Ltd
 * @since 23.3
 */
public class AuthenticationContext {
    ...
    /**
     * Gets an {@link Optional} with an instance of the current user if it has been authenticated, or empty if the user
     ...
     * @return an {@link Optional} with the current authenticated user, or empty if none available
     * @throws ClassCastException
     *             if the current user instance does not match the given {@code userType}.
     */
    ...
    /**
     * Gets an {@link Optional} containing the authenticated principal name, or an empty optional if the user is not
     * authenticated.
     *
     * The principal name usually refers to a username or an identifier that can be used to retrieve additional
     * information for the authenticated user.
     *
     * Anonymous users are considered not authenticated.
     *
     * @return an {@link Optional} containing the authenticated principal name or an empty optional if not available.
     */
     ...
     * Anonymous users are considered not authenticated.
     *
     * @return {@literal true} if a user is currently authenticated, otherwise {@literal false}
     */
    ...
    /**
     * Initiates the logout process of the current authenticated user by invalidating the local session and then
     * notifying {@link org.springframework.security.web.authentication.logout.LogoutHandler}.
     */
    public void logout() {
        ...
        if (ui.getPushConfiguration().getTransport() == Transport.WEBSOCKET
            ...
            ui.getPage().executeJs("return true").then(ignored -> {
                ...
                LOGGER.debug("Failed to switch to WEBSOCKET_XHR transport mode for logout operation. "
    ...
    private void doLogout(UI ui) {
        ...
        HttpServletResponse response = Optional.ofNullable(VaadinServletResponse.getCurrent())
                .map(VaadinServletResponse::getHttpServletResponse).orElse(null);
     ...
     *
     * @return an unmodifiable collection of {@link GrantedAuthority}s or an empty collection if there is no
     *         authenticated user.
     */
    public Collection<? extends GrantedAuthority> getGrantedAuthorities() {
        return getAuthentication().filter(Authentication::isAuthenticated).map(Authentication::getAuthorities)
                .orElse(Collections.emptyList());
    }

    private Stream<String> getGrantedAuthoritiesStream() {
        return getGrantedAuthorities().stream().map(GrantedAuthority::getAuthority);
    }

    /**
     * Gets the roles granted to the current authenticated user.
     *
     * @return an unmodifiable collection of role names (without the role prefix) or an empty collection if there is no
     *         authenticated user.
     */
    ...
    private Stream<String> getGrantedRolesStream() {
        ...
        return getGrantedAuthoritiesStream().filter(ga -> ga.startsWith(rolePrefix))
     ...
     * <p>
     * </p>
     * The role must be provided without the role prefix, for example {@code hasRole("USER")} instead of
     * {@code hasRole("ROLE_USER")}.
     *
     * @param role
     *            the role to check, without the role prefix.
     * @return {@literal true} if the user holds the given role, otherwise {@literal false}.
     */
     ...
     * <p>
     * </p>
     * Roles must be provided without the role prefix, for example {@code hasAnyRole(Set.of("USER", "ADMIN"))} instead
     * of {@code hasAnyRole(Set.of("ROLE_USER", "ROLE_ADMIN"))}.
     *
     * @param roles
     *            a collection containing at least one role, without the role prefix.
     * @return {@literal true} if the user holds at least one of the given roles, otherwise {@literal false}.
     ...
     * <p>
     * </p>
     * Roles must be provided without the role prefix, for example {@code hasAnyRole("USER", "ADMIN")} instead of
     * {@code hasAnyRole("ROLE_USER", "ROLE_ADMIN")}.
     *
     * @param roles
     *            an array containing at least one role, without the role prefix.
     * @return {@literal true} if the user holds at least one of the given roles, otherwise {@literal false}.
     * @throws IllegalArgumentException
     ...
     * <p>
     * </p>
     * Roles must be provided without the role prefix, for example {@code hasAllRoles(Set.of("USER", "ADMIN"))} instead
     * of {@code hasAllRoles(Set.of("ROLE_USER", "ROLE_ADMIN"))}.
     *
     * @param roles
     *            a collection containing at least one role, without the role prefix.
     * @return {@literal true} if the user holds all the given roles, otherwise {@literal false}.
     * @throws IllegalArgumentException
     *             if the given collection is empty.
     */
    public boolean hasAllRoles(Collection<String> roles) {
        ...
        return getGrantedRolesStream().collect(Collectors.toSet()).containsAll(roles);
    }
     ...
     * <p>
     * </p>
     * Roles must be provided without the role prefix, for example {@code hasAllRoles("USER", "ADMIN")} instead of
     ...
     * @return {@literal true} if the user holds the given authority, otherwise {@literal false}.
     */
     ...
     * @return {@literal true} if the user holds at least one of the given authorities, otherwise {@literal false}.
     * @throws IllegalArgumentException
     *             if the given collection is empty.
     */
     ...
     * @param authorities
     *            a collection containing at least one authority.
     * @return {@literal true} if the user holds all the given authorities, otherwise {@literal false}.
     * @throws IllegalArgumentException
     *             if the given collection is empty.
     */
    public boolean hasAllAuthorities(Collection<String> authorities) {
        ...
        return getGrantedAuthoritiesStream().collect(Collectors.toSet()).containsAll(authorities);
    }
    ...
    /**
     * Sets the role prefix holder to use when checking the current user's roles.
     *
     * @param rolePrefixHolder
     *            {@link VaadinRolePrefixHolder} instance, or {@literal null} to revert to the default {@code ROLE_}
     *            prefix.
     */
    ...
    private String getRolePrefix() {
        return Optional.ofNullable(rolePrefixHolder).map(VaadinRolePrefixHolder::getRolePrefix).orElse("ROLE_");
    }

    /**
     * Augments the given {@link AuthenticationContext} with Spring Security.
     *
     * This method can be used to configure the {@link AuthenticationContext} when {@link VaadinWebSecurity} is not used
     * to set up Spring Security.
     *
     * @param httpSecurity
    ...
    public static void applySecurityConfiguration(HttpSecurity httpSecurity, AuthenticationContext authCtx) {
        ...
        LogoutConfigurer<?> logoutConfigurer = httpSecurity.getConfigurer(LogoutConfigurer.class);
    ...
    static class CompositeLogoutHandler implements LogoutHandler, Serializable {
        ...
        public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
            for (LogoutHandler handler : this.logoutHandlers) {
                try {
                    ...
                    // invalidated
                    if (response != null || !isContinueToNextHandler(request, handler)) {
                        throw e;
                    }
                }
            }
        ...
        private boolean isContinueToNextHandler(HttpServletRequest request, LogoutHandler handler) {
            return handler instanceof SecurityContextLogoutHandler
                    && (request.getSession() == null || !request.isRequestedSessionIdValid());
        }
    }

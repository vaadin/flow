...
class NavigationAccessControlConfigurerTest {
    ...
    AnnotatedViewAccessChecker viewAccessChecker = new AnnotatedViewAccessChecker(new AccessAnnotationChecker());
    RoutePathAccessChecker routePathAccessChecker = new RoutePathAccessChecker((path, principal, roleChecker) -> false);
    ...
    @Test
    void build_defaults() {
        TestNavigationAccessControl accessControl = configurer.build(factory, registeredCheckers);
    ...
    @Test
    void build_disable() {
        TestNavigationAccessControl accessControl = configurer.disabled().build(factory, registeredCheckers);
    ...
    void build_customDecisionResolver() {
        AccessCheckDecisionResolver customResolver = (results, context) -> context.deny("CUSTOM");
        TestNavigationAccessControl accessControl = configurer.withDecisionResolver(customResolver).build(factory,
                registeredCheckers);

        Assertions.assertThat(accessControl.decisionResolver).isSameAs(customResolver);
    }

    @Test
    void build_enableViewAccessChecker() {
        TestNavigationAccessControl accessControl = configurer.withAnnotatedViewAccessChecker().build(factory,
                registeredCheckers);

        Assertions.assertThat(accessControl.checkerList).containsExactly(viewAccessChecker);
    }
    ...
    void build_enableViewAccessChecker_checkerNotAvailable_throws() {
        ...
        Assertions.assertThatIllegalStateException()
                .isThrownBy(() -> configurer.withAnnotatedViewAccessChecker().build(factory, registeredCheckers))
                .withMessageContaining(AnnotatedViewAccessChecker.class.getName())
                .withMessageContaining(" is not available");
    }

    @Test
    void build_enableRoutePathAccessChecker() {
        TestNavigationAccessControl accessControl = configurer.withRoutePathAccessChecker().build(factory,
                registeredCheckers);

        Assertions.assertThat(accessControl.checkerList).containsExactly(routePathAccessChecker);
    }
    ...
    void build_enableRoutePathAccessChecker_checkerNotAvailable_throws() {
        ...
        Assertions.assertThatIllegalStateException()
                .isThrownBy(() -> configurer.withRoutePathAccessChecker().build(factory, registeredCheckers))
    ...
    void build_customAccessChecker() {
        ...
        TestNavigationAccessControl accessControl = configurer.withNavigationAccessChecker(accessChecker).build(factory,
                registeredCheckers);

        Assertions.assertThat(accessControl.checkerList).containsExactly(accessChecker);
    }

    @Test
    void build_customAccessCheckers() {
        List<NavigationAccessChecker> accessCheckers = List.of(new TestAccessChecker(99), new TestAccessChecker(100),
                new TestAccessChecker(101));
        TestNavigationAccessControl accessControl = configurer.withNavigationAccessCheckers(accessCheckers)
                .build(factory, registeredCheckers);

        Assertions.assertThat(accessControl.checkerList).containsExactlyElementsOf(accessCheckers);
    }
    ...
    void build_customAccessChecker_calledMultipleTimes() {
        ...
        TestNavigationAccessControl accessControl = configurer.withNavigationAccessChecker(accessChecker1)
                .withNavigationAccessChecker(accessChecker2).withNavigationAccessCheckers(accessCheckers)
                .withNavigationAccessChecker(accessChecker3).build(factory, registeredCheckers);
        ...
        Assertions.assertThat(accessControl.checkerList).containsExactlyElementsOf(expectedOrder);
    }
    ...
    void build_outOfTheBoxAndCustomAccessCheckers_outOfTheBoxFirst() {
        List<NavigationAccessChecker> accessCheckers = List.of(new TestAccessChecker(99), new TestAccessChecker(100),
                new TestAccessChecker(101));
        TestNavigationAccessControl accessControl = configurer.withRoutePathAccessChecker()
                .withAnnotatedViewAccessChecker().withNavigationAccessCheckers(accessCheckers)
                .build(factory, registeredCheckers);
    ...
    void build_filterAvailableAccessCheckers() {
        ...
        TestNavigationAccessControl accessControl = configurer.withAvailableNavigationAccessCheckers(checker -> true)
                .build(factory, registeredCheckers);
        ...
        accessControl = configurer.withAvailableNavigationAccessCheckers(checker -> false).build(factory,
                registeredCheckers);
        ...
        accessControl = configurer
                .withAvailableNavigationAccessCheckers(checker -> checker instanceof RoutePathAccessChecker
                        || (checker instanceof TestAccessChecker t && t.id % 15 == 0))
                .build(factory, registeredCheckers);
    ...
    void build_filterAvailableAccessCheckers_outOfTheBoxCheckerPreserved() {
        ...
        TestNavigationAccessControl accessControl = configurer
                .withAvailableNavigationAccessCheckers(
                        checker -> (checker instanceof TestAccessChecker t && t.id % 15 == 0))
                .withAnnotatedViewAccessChecker().withRoutePathAccessChecker()
                .withNavigationAccessChecker(additionalChecker).build(factory, registeredCheckers);
    ...
    void build_loginView() {
        TestNavigationAccessControl accessControl = configurer.withLoginView(LoginView.class).build(factory,
                registeredCheckers);

        Assertions.assertThat(accessControl.getLoginView()).isEqualTo(LoginView.class);
        Assertions.assertThat(accessControl.getLoginUrl()).isNull();
    }

    @Test
    void build_loginViewPath() {
        TestNavigationAccessControl accessControl = configurer.withLoginView("/login").build(factory,
                registeredCheckers);
    ...
    void build_bothLoginViewAndPath_throws() {
        Assertions.assertThatIllegalStateException()
                .isThrownBy(() -> configurer.withLoginView(LoginView.class).withLoginView("/login").build(factory,
                        registeredCheckers))
                .withMessageContaining("Already using").withMessageContaining("as the login view");
    }

    private static class LoginView extends Component {
    }

    private static class TestNavigationAccessControl extends NavigationAccessControl {

        final Collection<NavigationAccessChecker> checkerList;
        final AccessCheckDecisionResolver decisionResolver;

        public TestNavigationAccessControl(Collection<NavigationAccessChecker> checkerList,

...
class SpringViewAccessCheckerTest {
    ...
    void viewAccessControlWorksWithoutRequest() {
        ...
        Mockito.when(annotationChecker.hasAccess(Mockito.any(Class.class), Mockito.any(), Mockito.any()))
                .thenAnswer(answer -> {
                    Principal principal = answer.getArgument(1);
                    Function<String, Boolean> roleChecker = answer.getArgument(2);

                    Assert.assertEquals("Principal from security context should have been passed to checker",
                            authentication, principal);

                    Assert.assertTrue("Role should have been checked from the context holder",
                            roleChecker.apply("fake"));
                    Assert.assertFalse("Role should have been checked from the context holder",
        ...
        Mockito.when(beforeEnterEvent.getNavigationTarget()).thenReturn((Class) TestView.class);
        checker.beforeEnter(beforeEnterEvent);
        Assert.assertTrue("Annotation checker should have been invoked", accessChecked.get());
    }
}

...
public class VaadinSavedRequestAwareAuthenticationSuccessHandlerTest {
    ...
    @Test
    public void directLogin_nonTypescriptClientsAndDefaultTargetUrl_redirectToDefaultTargetUrl() throws Exception {
        MockHttpServletRequest loginRequest = RequestUtilTest.createRequest("/login");
        MockHttpServletResponse loginResponse = new MockHttpServletResponse();
        vaadinSavedRequestAwareAuthenticationSuccessHandler.setDefaultTargetUrl("/foo");
        vaadinSavedRequestAwareAuthenticationSuccessHandler.onAuthenticationSuccess(loginRequest, loginResponse,
                new UsernamePasswordAuthenticationToken("foo", "bar"));
    ...
    @Test
    public void savedUrl_nonTypescriptClients_alwaysUseDefaultTargetUrl_redirectToDefaultTargetUrl() throws Exception {
        ...
        MockHttpServletResponse loginResponse = new MockHttpServletResponse();
        vaadinSavedRequestAwareAuthenticationSuccessHandler.setAlwaysUseDefaultTargetUrl(true);
        vaadinSavedRequestAwareAuthenticationSuccessHandler.setDefaultTargetUrl("/foo");
        vaadinSavedRequestAwareAuthenticationSuccessHandler.onAuthenticationSuccess(loginRequest, loginResponse,
                new UsernamePasswordAuthenticationToken("foo", "bar"));
    ...
    @Test
    public void savedUrl_nonTypescriptClients_targetParameter_redirectToTargetParameter() throws Exception {
        HttpSessionRequestCache cache = new HttpSessionRequestCache();
        MockHttpServletRequest firstRequest = RequestUtilTest.createRequest("/the-saved-url");
        HttpSession session = firstRequest.getSession();
        cache.saveRequest(firstRequest, new MockHttpServletResponse());
    ...
    @Test
    public void savedUrl_nonTypescriptClients_redirectToSavedUrl() throws Exception {
        HttpSessionRequestCache cache = new HttpSessionRequestCache();
        MockHttpServletRequest firstRequest = RequestUtilTest.createRequest("/the-saved-url");
        HttpSession session = firstRequest.getSession();
        cache.saveRequest(firstRequest, new MockHttpServletResponse());
        ...
        Assert.assertEquals("http://localhost/the-saved-url?continue", loginResponse.getHeader("Location"));
    }

    @Test
    public void viewAccessCheckerSavedUrl_nonTypescriptClients_redirectToSessionStoredUrl() throws Exception {

        MockHttpServletRequest loginRequest = RequestUtilTest.createRequest("/login");
        HttpSession session = loginRequest.getSession();
        // Simulate NavigationAccessControl
        session.setAttribute(NavigationAccessControl.SESSION_STORED_REDIRECT_ABSOLUTE, "http://localhost/last-route");
        MockHttpServletResponse loginResponse = new MockHttpServletResponse();
        vaadinSavedRequestAwareAuthenticationSuccessHandler.onAuthenticationSuccess(loginRequest, loginResponse,
                new UsernamePasswordAuthenticationToken("foo", "bar"));
        ...
        Assert.assertNull(loginResponse.getHeader("Result"));
        Assert.assertEquals(302, loginResponse.getStatus());
        Assert.assertEquals("http://localhost/last-route", loginResponse.getHeader("Location"));
    }

    @Test
    public void savedUrlAndViewAccessCheckerSavedUrl_nonTypescriptClients_redirectToSavedUrl() throws Exception {
        HttpSessionRequestCache cache = new HttpSessionRequestCache();
        MockHttpServletRequest firstRequest = RequestUtilTest.createRequest("/a-previous-saved-url");
        HttpSession session = firstRequest.getSession();
        cache.saveRequest(firstRequest, new MockHttpServletResponse());
        // Simulate NavigationAccessControl
        session.setAttribute(NavigationAccessControl.SESSION_STORED_REDIRECT_ABSOLUTE, "http://localhost/last-route");
        ...
        // HttpSessionRequestCache uses request parameter "continue",
        // see HttpSessionRequestCache::setMatchingRequestParameterName
        Assert.assertEquals("http://localhost/a-previous-saved-url?continue", loginResponse.getHeader("Location"));
    }
    ...
    public void saved_url_sent_to_typescript_client() throws Exception {
        ...
        // HttpSessionRequestCache uses request parameter "continue",
        // see HttpSessionRequestCache::setMatchingRequestParameterName
        Assert.assertEquals("http://localhost/the-saved-url?continue", loginResponse.getHeader("Saved-url"));
        Assert.assertEquals("/", loginResponse.getHeader("Default-url"));
    }

    @Test
    public void csrfs_sent_to_typescript_client() throws Exception {
        DefaultCsrfToken springCsrfToken = new DefaultCsrfToken("spring-csrf-header-name", "spring-csrf-parameter-name",
        ...
        Assert.assertEquals("spring-csrf-header-name", loginResponse.getHeader("Spring-CSRF-header"));
        Assert.assertEquals("spring-csrf-token-value", loginResponse.getHeader("Spring-CSRF-token"));
    }
}

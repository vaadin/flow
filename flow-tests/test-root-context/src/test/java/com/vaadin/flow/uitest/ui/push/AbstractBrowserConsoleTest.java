...
public abstract class AbstractBrowserConsoleTest extends ChromeBrowserTest {
    ...
    protected void open() {
        ...
        getCommandExecutor().executeScript("window.logs = [];" + "var origConsole = window.console; window.console = {"
    ...
    protected List<?> getBrowserLogs(boolean reset) {
        if (reset) {
            return (List<?>) getCommandExecutor()
                    .executeScript("var result = window.logs; window.logs=[]; return result;");
        } else {
            return (List<?>) getCommandExecutor().executeScript("return window.logs;");
        }
    }

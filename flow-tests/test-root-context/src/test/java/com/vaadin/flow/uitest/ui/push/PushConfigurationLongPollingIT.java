...
public class PushConfigurationLongPollingIT extends PushConfigurationTest {
    ...
    public void testLongPolling() throws InterruptedException {
        findElement(By.id("transport")).findElement(By.id(Transport.LONG_POLLING.name().toLowerCase(Locale.ENGLISH)))
                .click();

        Assert.assertThat(getStatusText(), CoreMatchers.containsString("fallbackTransport: long-polling"));
        Assert.assertThat(getStatusText(), CoreMatchers.containsString("transport: long-polling"));

        findElement(By.id("push-mode")).findElement(By.id(PushMode.AUTOMATIC.name().toLowerCase(Locale.ENGLISH)))
                .click();
        waitUntil(driver -> Transport.LONG_POLLING.getIdentifier().equals(getTransport()), 10);

...
public class NavigationEventsIT extends ChromeBrowserTest {
    ...
    private void assertMessages(int expectedSize) {
        ...
        Assert.assertEquals("Unexpected amount of navigation events", expectedSize, messages.size());
        Assert.assertEquals("Second to last event should be BeforeEnter", NavigationEventsView.BEFORE_ENTER,
                messages.get(expectedSize - 2));
        Assert.assertEquals("Last event should be AfterNavigation", NavigationEventsView.AFTER_NAVIGATION,
                messages.get(expectedSize - 1));

    }

    private List<String> getMessages() {
        return findElement(By.id("messages")).findElements(By.cssSelector("div")).stream().map(WebElement::getText)

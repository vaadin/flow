...
public class DynamicallyRegisteredRouteIT extends ChromeBrowserTest {
    ...
    public void testServiceInitListener_canRegisterRoutes() {
        ...
        WebElement element = findElement(By.id(DynamicallyRegisteredRoute.ID));

        Assert.assertEquals("Route registered during service init not present",
                DynamicallyRegisteredRoute.TEXT, element.getText());
    }
}

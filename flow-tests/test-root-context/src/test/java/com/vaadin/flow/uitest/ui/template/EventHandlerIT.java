...
public class EventHandlerIT extends ChromeBrowserTest {
    ...
    public void handleEventOnServer() {
        ...
        Assert.assertEquals("Unexpected 'button' event data in the received event handler parameter", "button: 0",
                divs.get(1).getText());
        Assert.assertEquals("Unexpected 'type' event data in the received event handler parameter", "type: click",
                divs.get(2).getText());
        Assert.assertEquals("Unexpected 'tag' event data in the received event handler parameter", "tag: button",
                divs.get(3).getText());
        ...
        Assert.assertTrue("Unable to find server event handler invocation confirmation.",
        ...
        Assert.assertEquals("Server-side message should be present", "Call from client, message: foo, true",
                $("div").id("client-call").getText());
        Assert.assertEquals("Message from awaiting return value should be present", "FOO",

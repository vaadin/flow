...
public class ListInsideListBindingIT extends ChromeBrowserTest {
    ...
    private void checkMessagesRemoval(TestBenchElement template, int initialSize) {
        for (int i = 0; i < initialSize; i++) {
            List<TestBenchElement> currentMessages = template.$(TestBenchElement.class).attribute("class", "submsg")
                    .all();
            ...
            Assert.assertEquals("Expected removed message text to appear", "Removed message: " + messageToRemoveText,
    ...
    private void checkAllElementsUpdated(TestBenchElement template, int initialSize) {
        ...
        List<TestBenchElement> msgs = template.$(TestBenchElement.class).attribute("class", "submsg").all();
        Assert.assertEquals("Wrong amount of nested messages", initialSize, msgs.size());
        msgs.forEach(msg -> Assert.assertEquals("Message was not updated", ListInsideListBindingView.UPDATED_TEXT,
                msg.getText()));
    }
}

...
public class ResynchronizationIT extends ChromeBrowserTest {
    ...
    /*
     * If a component is only added in a lost message, it should be present after resynchronization.
     */
    @Test
    public void resynchronize_componentAddedInLostMessage_appearAfterResync() {
        ...
        waitUntil(driver -> findElements(By.className(ResynchronizationView.ADDED_CLASS)).size() == 2);
    }

    /*
     * If a @ClientCallable is invoked in a lost message, the promises waiting for the return value from the server
     * should be rejected rather than remain pending.
     */
    @Test

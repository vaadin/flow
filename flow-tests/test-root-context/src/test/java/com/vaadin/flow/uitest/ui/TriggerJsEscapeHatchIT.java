...
public class TriggerJsEscapeHatchIT extends ChromeBrowserTest {
    ...
    public void jsTriggerActionAndOutputWireUpEndToEnd() {
        ...
        Assert.assertEquals("(initial)", result.getText());

        button.click();

        WebElement updated = waitUntil(d -> {
            WebElement r = d.findElement(By.id("result"));
            return "js-escape-hatch".equals(r.getText()) ? r : null;
        });
        Assert.assertEquals("js-escape-hatch", updated.getText());
    }
}

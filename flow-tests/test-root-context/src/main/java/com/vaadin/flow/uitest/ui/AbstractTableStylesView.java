...
/**
 * Shows what an opt-in table stylesheet does to a native
 * <code>&lt;table&gt;</code>, both on its own and through each of the
 * {@code theme} variants it supports.
 * <p>
 * The subclasses differ only in the {@code @StyleSheet} they carry: that
 * annotation is the whole opt-in, and nothing on the tables themselves asks to
 * be styled.
 */
public abstract class AbstractTableStylesView extends Div {

    protected AbstractTableStylesView(String stylesheetName) {
        add(new H2("Native tables with " + stylesheetName));

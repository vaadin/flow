...
public class NavigationIT extends ChromeBrowserTest {
    ...
    public void testNavigation() {
        ...
        Assert.assertEquals("NavigationView", $(SpanElement.class).first().getText());

        $(AnchorElement.class).id(NavigationView.ANCHOR_ID).click();
        Assert.assertEquals("AnchorView", $(SpanElement.class).first().getText());
        $(AnchorElement.class).id(NavigationView.ANCHOR_ID).click();
        Assert.assertEquals("NavigationView", $(SpanElement.class).first().getText());

        $(NativeButtonElement.class).id(NavigationView.SERVER_ID).click();
        Assert.assertEquals("ServerView", $(SpanElement.class).first().getText());
        $(NativeButtonElement.class).id(NavigationView.SERVER_ID).click();
        Assert.assertEquals("NavigationView", $(SpanElement.class).first().getText());

        $(AnchorElement.class).id(NavigationView.ROUTER_LINK_ID).click();
        Assert.assertEquals("RouterView", $(SpanElement.class).first().getText());
        $(AnchorElement.class).id(NavigationView.ROUTER_LINK_ID).click();
        Assert.assertEquals("NavigationView", $(SpanElement.class).first().getText());
    }
    ...
    public void testNavigationPostpone_anchor() {
        ...
        Assert.assertEquals("PostponeView", $(SpanElement.class).first().getText());

        $(AnchorElement.class).id(PostponeView.NAVIGATION_ID).click();

        Assert.assertEquals("Navigation should have postponed", "PostponeView", $(SpanElement.class).first().getText());

        Assert.assertEquals(2, $(NativeButtonElement.class).all().size());
        Assert.assertTrue($(NativeButtonElement.class).id(PostponeView.CONTINUE_ID).isDisplayed());
        Assert.assertTrue($(NativeButtonElement.class).id(PostponeView.STAY_ID).isDisplayed());

        $(NativeButtonElement.class).id(PostponeView.STAY_ID).click();

        Assert.assertEquals(0, $(NativeButtonElement.class).all().size());

        Assert.assertTrue("Url should not have changed", getDriver().getCurrentUrl().endsWith("PostponeView"));

        $(AnchorElement.class).id(PostponeView.NAVIGATION_ID).click();

        Assert.assertEquals("Navigation should have fired and be postponed again", "PostponeView",
        ...
        Assert.assertEquals("Navigation should have continued to NavigationView", "NavigationView",
                $(SpanElement.class).first().getText());

        Assert.assertTrue("Url should have updated to NavigationView was : " + getDriver().getCurrentUrl(),
    ...
    public void testNavigationPostpone_routerLink() {
        ...
        $(AnchorElement.class).id(PostponeView.NAVIGATION_ROUTER_LINK_ID).click();
    ...
    public void testreactNavigationBrowserHistoryBack_anchor() {
        ...
        Assert.assertEquals("This is a simple view for a React route", $(ParagraphElement.class).id("react").getText());
        getDriver().navigate().back();
        Assert.assertEquals("NavigationView", $(SpanElement.class).first().getText());
    ...
    public void testReactNavigation_flowContentCleaned() {
        ...
        Assert.assertFalse("Flow navigation view contents should not exist",
                $(AnchorElement.class).attribute("id", NavigationView.REACT_ANCHOR_ID).exists());
    }
    ...
    public void testRouterLinkWithQueryNavigation() {
        ...
        Assert.assertEquals("Exception on router-link navigation with query parameters", "AnchorView",
                $(SpanElement.class).first().getText());
        System.out.println(getDriver().getCurrentUrl());
        Assert.assertTrue("Query was missing in url", getDriver().getCurrentUrl().endsWith("?test=value"));
    }
    ...
    public void testAnchorWithQueryNavigation() {
        ...
        Assert.assertEquals("Exception on router-link navigation with query parameters", "AnchorView",
                $(SpanElement.class).first().getText());
        System.out.println(getDriver().getCurrentUrl());
        Assert.assertTrue("Query was missing in url", getDriver().getCurrentUrl().endsWith("?test=anchor"));
    }

    private void checkNavigatedEvent(String log) {
        Object message = ((JavascriptExecutor) getDriver()).executeScript("return window.testMessage;");
    ...
    public void testNavigatedEvent() {
        ...
        Assert.assertEquals("NavigationView", $(SpanElement.class).first().getText());
        checkNavigatedEvent("navigated to /view/com.vaadin.flow.NavigationView");
    ...
    public void testNavigation_HasUrlParameter_setParameterCalledOnce() {
        ...
        $(NativeButtonElement.class).id(NavigationView.SERVER_ID).click();
        Assert.assertEquals("ServerView", $(SpanElement.class).first().getText());
        Assert.assertEquals("1", $(SpanElement.class).id(NavigationView.SET_PARAMETER_COUNTER_ID).getText());
    }
}

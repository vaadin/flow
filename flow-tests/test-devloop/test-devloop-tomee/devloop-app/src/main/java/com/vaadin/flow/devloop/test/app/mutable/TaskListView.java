...
import com.vaadin.flow.devloop.test.app.ImportingLayout;
import com.vaadin.flow.devloop.test.shared.DueDateFormatter;
import com.vaadin.flow.router.Route;

/**
 * The view this fixture is driven through.
 * <p>
 ...
 * <p>
 * In the {@code mutable} package by convention: edits go to sources in this
 * package and are reverted afterwards, so a run never leaves the working tree
 * dirty. That is the convention the Jetty and Spring fixtures' ITs already
 * follow, and it is why this fixture keeps the same files in the same places -
 * those ITs can be pointed at this module without moving anything.
 * <p>
 * No dependency injection, because a plain WAR has none - Flow builds a route
 * target from its no-argument constructor. That is the point of this fixture:
 * everything here reaches the browser through a Tomcat that Cargo installed,
 * started and deployed the packaged WAR into, with no framework in between.
 */
@Route("")
@StyleSheet("task-list.css")
public class TaskListView extends ImportingLayout {

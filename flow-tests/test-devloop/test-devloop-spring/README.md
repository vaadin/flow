...
`VaadinDevCli` and `SourcePatch` live in `flow-tests/test-devloop/test-devloop-support` rather than
here, because `test-devloop-jetty` next door drives the same CLI against a WAR and the two harnesses
must not drift apart. So do the ITs both fixtures run - `DevLoopApplyIT`, `DevLoopCssIT`, `DevLoopMultiModuleIT`,
`DevLoopLifecycleIT`, `DevLoopBrowserIT`, `DevLoopCliContractIT`, `DevLoopDaemonSurvivalIT`,
`DevLoopDeletionIT` and `DevLoopPomEditIT`, on the common `AbstractDevLoopIT` base. Nothing here extends them: this module's
pom points failsafe's `<dependenciesToScan>` at that jar and they run against this application under
their own names. The `DevLoopSpring*IT` classes here hold what is true only of an application with an
entry point of its own: the Spring context and everything it builds at startup - bean proxies, an
ORM's metamodel, the set of bean definitions. A live proxy is why a new member on a bean escalates
here and is a clean hot swap in `test-devloop-jetty`, where the same class is a plain one. All of
it stays in this package and stays package-private: every consumer puts its ITs in
`com.vaadin.flow.devloop.test.it` too, so the classes are visible where they are used and nowhere
else.
...
the two cold JVM starts that `DevLoopPomEditIT` and `DevLoopSpringRestartIT` need.
...
mvn -o -pl flow-tests/test-devloop/test-devloop-spring/devloop-app verify
```
...
mvn -o -pl flow-tests/test-devloop/test-devloop-spring/devloop-shared install -DskipTests
```
...
Targeting the `flow-tests/test-devloop/test-devloop-spring` aggregator instead needs one more flag:
...
```bash
cd flow-tests/test-devloop/test-devloop-spring/devloop-app

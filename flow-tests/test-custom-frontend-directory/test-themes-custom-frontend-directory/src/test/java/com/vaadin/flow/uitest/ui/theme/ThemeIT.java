...
public class ThemeIT extends ChromeBrowserTest {
    ...
    public void nodeAssetInCss_pathIsSetCorrectly() {
        open();
        final String resourceUrl = getRootURL() + "/path/themes/app-theme/fortawesome/icons/snowflake.svg";
        WebElement cssNodeSnowflake = findElement(By.id(CSS_SNOWFLAKE));
        final String expectedImgUrl = "url(\"" + resourceUrl + "\")";
        Assert.assertEquals("Background image has been referenced in styles.css and " + "expected to be applied",
    ...
    public void applicationTheme_onlyStylesCssIsApplied() {
        ...
        Assert.assertEquals("body background-image should come from styles.css",
                "url(\"" + getRootURL() + "/path/VAADIN/themes/app-theme/img/bg.jpg\")",
    ...
    public void parentTheme_isApplied() {
        ...
        Assert.assertEquals("Color from parent theme should be applied.", "rgba(0, 255, 255, 1)",
                $(SpanElement.class).id(FONTAWESOME_ID).getCssValue("color"));

        Assert.assertEquals("Child theme should override parent theme values", "5px",
        ...
        Assert.assertEquals("Polymer text field should get parent border radius", "0px",
                input.getCssValue("border-radius"));

        Assert.assertEquals("Polymer text field should use green as color", "rgba(0, 128, 0, 1)",
    ...
    public void subCssWithRelativePath_urlPathIsNotRelative() {
        ...
        Assert.assertEquals("Imported css file URLs should have been handled.",
                "url(\"" + getRootURL() + "/path/VAADIN/themes/app-theme/icons/archive.png\")",
                $(SpanElement.class).id(SUB_COMPONENT_ID).getCssValue("background-image"));
    }
    ...
    public void documentCssImport_externalUrlLoaded() {
        ...
        Assert.assertTrue("Font should have been loaded",
                (boolean) executeScript("return document.fonts.check(arguments[0])", "10px Itim"));
    ...
    public void customFrontendDirectory_generatedFilesNotInDefaultFrontendFolder() {
        ...
        File expectedGeneratedFolder = new File(baseDir, "side-src/main/frontend/generated");
        File defaultGeneratedFolder = new File(baseDir, FrontendUtils.DEFAULT_FRONTEND_DIR + "generated");

        String[] generatedFiles = { "theme.d.ts", "theme.js", "theme-app-theme.generated.js",
                "theme-parent-theme.generated.js", "vaadin.ts" };
        for (String generatedFile : generatedFiles) {
            Assert.assertTrue("Expecting " + generatedFile + " to be present in " + expectedGeneratedFolder.getPath()
                    + ", but was not", new File(expectedGeneratedFolder, generatedFile).exists());
            Assert.assertFalse("Expecting " + generatedFile + " not to be present in "
                    + defaultGeneratedFolder.getPath() + ", but was not",

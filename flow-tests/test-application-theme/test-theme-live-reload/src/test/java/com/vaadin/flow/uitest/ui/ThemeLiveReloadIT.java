...
public class ThemeLiveReloadIT extends ChromeBrowserTest {
    ...
    private static final String THEME_FOLDER = FrontendUtils.DEFAULT_FRONTEND_DIR + "/themes/app-theme/";
    ...
    public void webpackLiveReload_newCssAndFontCreatedAndDeleted_stylesUpdatedOnFly() {
        open();
        Assert.assertFalse("Red background is not expected before applying the styles", isCustomBackGroundColor());
        Assert.assertTrue("Expected styles.css file to be auto-generated by flow", stylesCSSFile.exists());
        Assert.assertFalse("Expected no test-styles.css file exists before " + "applying the styles",
                testStylesCSSFile.exists());
        Assert.assertFalse("Expected no font file in the theme folder before " + "applying the font styles",
                fontFile.exists());
    ...
    private void createStylesCssWithFontAndRedBackground() {
        try {
            ...
            // @formatter:on
            FileUtils.write(testStylesCSSFile, fontStyle, StandardCharsets.UTF_8.name());
            FileUtils.write(stylesCSSFile, stylesCssContent, StandardCharsets.UTF_8.name());
            waitUntil(driver -> testStylesCSSFile.exists());
        } catch (IOException e) {
            throw new RuntimeException("Failed to apply font and background styles", e);
        }
    }
    ...
    private void copyFontFile() {
        try {
            File copyFontFrom = new File(baseDir,
                    FrontendUtils.DEFAULT_FRONTEND_DIR + "/fonts/ostrich-sans-regular.ttf");
    ...
    private void doActionAndWaitUntilLiveReloadComplete(Runnable action) {
        ...
        executeScript("window.Vaadin.Flow.clients[\"blocker\"] = {isActive: () => true};");
        action.run();

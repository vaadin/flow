...
public class LayoutIT extends AbstractSpringTest {
    ...
    public void divInLayoutComponentIsPreserved_buttonInLayoutIsNotPreserved_serviceIsPreserved_noOwnerBeanIsNotPreserved() {
        ...
        String serviceCallInsideButtonView = findElement(By.id("route-service")).getText();

        Assert.assertEquals(
                "Service call in the " + DivInLayout.class
        ...
        Assert.assertEquals("Only one bean " + ButtonScopedBean.class + " instance is expected", String.valueOf(1),
        ...
        Assert.assertEquals(DivInLayout.class + " instance is not preserved", id,
                findElement(By.id("div-id")).getText());

        Assert.assertTrue(isElementPresent(By.id("button-scoped-bean-destroy")));

        // navigate to the button view again
        navigateTo("button-link");

        Assert.assertNotEquals("Button view should not be preserved on navigation", buttonViewId,
                findElement(By.id("component-id")).getText());
        Assert.assertNotEquals("No owner bean should not be preserved on navigtaion", noOwnerBeanCall,
                findElement(By.id("no-owner-bean")).getText());

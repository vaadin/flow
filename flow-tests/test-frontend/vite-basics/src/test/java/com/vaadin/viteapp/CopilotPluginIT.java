...
public class CopilotPluginIT extends ChromeBrowserTest {
    ...
    public void expectAvailableFeaturesInjectedIntoWindow() {
        final Set<String> expectedFeaturesSet = new HashSet<>(
                Set.of("COMPONENT_ANALYZER"));
        List<String> features = (List<String>) executeScript(
                "return window.Vaadin.copilot.VITE_COPILOT_PLUGIN.availableFeatures");
    ...
    public void expectUnExpectedRequestsHandled() {
        executeScript(
                "window.META_HOT.send('vaadin-copilot:analyze-component', { });");
        WebElement element = findElement(By.cssSelector("div#response"));
        waitUntil(driver -> StringUtils.isNotEmpty(
                findElement(By.cssSelector("div#response")).getText()));
    ...
    public void expectAllNodesAreAnalyzedInGivenFilesWhenRequested() {
        String buttonContainingFilePath = getFilePathOfNode(
                findElement(By.cssSelector("button")));
        executeScript(
                "window.META_HOT.send('vaadin-copilot:analyze-component', { filePaths: [ \""
                        + buttonContainingFilePath + "\"] });");
        waitUntil(driver -> StringUtils.isNotEmpty(
                findElement(By.cssSelector("div#response")).getText()));

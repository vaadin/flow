...
public class ReactComponentPropertiesIT extends ChromeBrowserTest {
    ...
    public void inlinePropsCanFound() {
        var componentWithInlinePropsElement = $("*").withAttribute(DATA_TEST_ID, "component-with-inline-props").waitForFirst();
        Map<String, Object> debugPropertiesFromFiberNode = getDebugPropertiesFromFiberNode(componentWithInlinePropsElement);
        var error = (Boolean) debugPropertiesFromFiberNode.get("error");
        Assert.assertFalse(error);
        debugPropertiesFromFiberNode.get("properties");
        List<?> properties = (List<?>) debugPropertiesFromFiberNode.get("properties");
        Assert.assertFalse(properties.isEmpty());
    }

    @Test
    public void intrinsicPropsCanFoundInWindow() {
        var h1 = $("*").withAttribute(DATA_TEST_ID, "simple-header").waitForFirst();
        Map<String, Object> debugPropertiesFromFiberNode = getDebugPropertiesFromFiberNode(h1);
        Assert.assertNull(debugPropertiesFromFiberNode);
        Map<String, Object> debugPropertiesFromWindow = getDebugPropertiesFromWindow(h1);
        Assert.assertNotNull(debugPropertiesFromWindow);
        Assert.assertFalse((Boolean) debugPropertiesFromWindow.get("error"));
        Assert.assertFalse(((List<?>) debugPropertiesFromWindow.get("properties")).isEmpty());
    }

    @Test
    public void interfacePropsCanFound() {
        var compWithInterfaceProps = $("*").withAttribute(DATA_TEST_ID, "component-with-interface-props").waitForFirst();
        Map<String, Object> debugPropertiesFromFiberNode = getDebugPropertiesFromFiberNode(compWithInterfaceProps);
    ...
    public void componentWithoutPropsShouldHaveAnError() {
        var element = $("*").withAttribute(DATA_TEST_ID, "component-with-any-props").waitForFirst();
        Assert.assertNull(getDebugPropertiesFromFiberNode(element));
        Map<String, Object> debugPropertiesFromWindow = getDebugPropertiesFromWindow(element);
    ...
    public void errorShouldBeRegisteredWhenIntrinsicElementsNotLoaded() {
        ...
        Map<String, Object> divProperties = getDebugPropertiesFromWindow(anyDiv);
    ...
    private Map<String, Object> getDebugPropertiesFromWindow(WebElement element) {
    ...
    private Map<String, Object> getDebugPropertiesFromFiberNode(WebElement element) {
        return (Map<String, Object>) executeScript(
                """

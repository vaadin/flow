...
@Route("exported-js-function")
@PageTitle("Exported JS function view")
public class ExportedJSFunctionView extends Div {
    ...
    protected void onAttach(AttachEvent attachEvent) {
        ...
        String versionJs = "var msg = '';" + "var versionInfoMethod = " + client
                ...
                + "}" //
                + "$0.innerHTML = msg;";
        ui.getPage().executeJs(versionJs, version);
        ...
        ui.getPage().executeJs(productionModeJs, productionMode);

        Div div = new Div();
        div.setId("poll");
        div.setText("Click to poll using JS API");
        ui.getPage()
                .executeJs("$0.addEventListener('click', function() {" + client
                        + ".poll();" //
                        + "});", div);
        add(div);

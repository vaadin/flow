...
public abstract class AbstractBasicModelType<T> implements ModelType {
    ...
    public boolean accepts(Type applicationType) {
        if (type.isPrimitive() && ReflectTools.convertPrimitiveType(type) == applicationType) {
            return true;
        }
    ...
    public void createInitialValue(StateNode node, String property) {
        ...
        if (!feature.hasProperty(property)) {
            feature.setProperty(property, (Serializable) modelToApplication(null));
        }
    }

    /**
     * Converts the given model value to the application type of this model type. The conversion automatically handles
     * default values for primitive types and conversion of client-originated numbers to the expected Java number type.
     *
     * @param modelValue
     *            the model value to convert
     * @return the converted value, not <code>null</code> if the application type is a primitive
     */
    protected Object convertToApplication(Serializable modelValue) {
        ...
        Class<?> convertedJavaType = ReflectTools.convertPrimitiveType(getJavaType());

        // Numeric value from the client is always Double
        if (modelValue instanceof Double && convertedJavaType == Integer.class) {
        ...
        if (convertedJavaType == modelValue.getClass()) {
            ...
            throw new IllegalArgumentException(String.format(
                    ...
                    modelValue, modelValue.getClass().getName(), getJavaType().getName()));
        }
    }

    protected static <M> Map<Class<?>, M> loadBasicTypes(Function<Class<?>, M> factory) {
        Map<Class<?>, M> map = Stream
                .of(int.class, Integer.class, boolean.class, Boolean.class, double.class, Double.class, String.class)
        ...
        assert map.keySet().stream().map(Class::getSimpleName).distinct().count() == map.size();

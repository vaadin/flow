...
/**
 * A model type that can convert values between a representation suitable for users and a representation suitable for
 * storage in a {@link StateNode}.
 * <p>
 * Model type instances are shared between all instances of a given {@link TemplateModel} type and should therefore be
 * immutable to prevent race conditions. The root type for a model can be found using
...
public interface ModelType extends Serializable {
    /**
     * Creates a representation of the provided model value that is intended for use in application code. For mutable
     * values, this is typically a proxy that is directly connected to the underlying model value.
     *
     * @param modelValue
     ...
     * <p>
     * For application values that contain properties (i.e. beans), the provided filter is used to determine which
     * properties from the bean should be included in the model representation.
     *
     * @param applicationValue
     *            the user value to convert
     * @param filter
     *            the filter to use to determine which properties to include, not <code>null</code>
    ...
    /**
     * Checks whether this type can accept application values of the given type. The method only considers this actual
     * type, not the types of sub properties or list items.
     *
     * @param applicationType
     *            the application type to check, not <code>null</code>
     * @return <code>true</code> if the provided type is acceptable, <code>false</code> otherwise
     */
    ...
    static String getSupportedTypesString() {
        ...
        BasicModelType.TYPES.keySet().forEach(type -> sb.append(type.getSimpleName()).append(", "));
    ...
    /**
     * Create initial value for the given {@code property} and set it for the {@code node}.
     *
     * @param node
     *            the node where the initial value should be set the {@code property}
     * @param property
     *            the property in the {@code node} whose initial value needs to be created
     */

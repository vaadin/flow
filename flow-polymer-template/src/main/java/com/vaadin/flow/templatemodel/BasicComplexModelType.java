 ...
 * <p>
 * There is a similar class {@link BasicModelType} which do the same but it keeps handles the values as is. This class
 * wraps them into {@link StateNode} to be able to use them in side lists.
 * <p>
...
public class BasicComplexModelType<T> extends AbstractBasicModelType<T> implements ComplexModelType<T> {
    ...
    private static final Map<Class<?>, BasicComplexModelType<?>> TYPES = loadBasicTypes(BasicComplexModelType::new);
     ...
     * @param type
     *            type to check
     * @return {@code true} is the {@code type} is basic supported type, {@code false} otherwise
     */
    ...
    public T modelToApplication(Serializable modelValue) {
        ...
        Serializable value = stateNode.getFeature(BasicTypeValue.class).getValue();
    ...
    public <C> ComplexModelType<C> cast(Class<C> proxyType) {
        if (getJavaType() != proxyType) {
            throw new IllegalArgumentException("Got " + proxyType + ", expected " + getJavaType());
        }

 ...
 *             Vaadin blog.</a> For lit templates, you can use {@code @Id} mapping and the component API or the element
...
public class ListModelType<T> implements ComplexModelType<T> {
    ...
    public List<T> modelToApplication(Serializable modelValue) {
        if (modelValue instanceof StateNode) {
            ...
            throw new IllegalArgumentException(String.format(
                    "The stored model value '%s' type '%s' " + "cannot be used as a type for a model list property",
    ...
    public StateNode applicationToModel(Object applicationValue, PropertyFilter filter) {
        ...
        StateNode node = new StateNode(Collections.singletonList(ModelList.class));
    ...
    public <C> ComplexModelType<C> cast(Class<C> proxyType) {
        if (getItemType() instanceof ListModelType<?> && GenericTypeReflector.erase(proxyType).equals(List.class)) {
     ...
     * @param type
     *            the type to check
     * @return <code>true</code> if the given type will be handled as a list of beans, <code>false</code> otherwise
     */

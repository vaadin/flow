...
/**
 * A {@link ModelType} implementation that wraps a model type for performing type conversions on together with a
 * {@link ModelEncoder}.
 * <p>
...
public class ConvertedModelType<A, M extends Serializable> implements ModelType {
    ...
    public Object modelToApplication(Serializable modelValue) {
        ...
        M wrappedApplicationValue = (M) wrappedModelType.modelToApplication(modelValue);
    ...
    public boolean accepts(Type applicationType) {
        return ReflectTools.convertPrimitiveType(converter.getDecodedType())
                .isAssignableFrom(ReflectTools.convertPrimitiveType((Class<?>) applicationType));
    }

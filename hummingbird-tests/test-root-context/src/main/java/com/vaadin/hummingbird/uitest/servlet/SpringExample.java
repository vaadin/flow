...
public class SpringExample implements RouterConfigurator {
    ...
    private static class Spring {
        ...
        public static boolean isAccessAllowed(String[] requiredRoles) {
            return requiredRoles == null
                    || Arrays.asList(requiredRoles).contains("noboy");
        }
    ...
    // example
    public static class NoPermisisonView extends AbstractDivView {
        public NoPermisisonView() {
    ...
    private void createRouteForBean(String beanName,
        ...
        configuration.setRoute(route, new ViewRenderer() {
            ...
            public List<Class<? extends HasChildView>> getParentViewTypes() {
                ...
                if (getViewType() == NoPermisisonView.class) {
            ...
            // This is the new method that is needed
            @Override
            protected <T> T getView(Class<T> viewType, NavigationEvent event) {
                if (viewType == NoPermisisonView.class) {
                    return (T) new NoPermisisonView();
                } else {
                    return (T) Spring.getBean(beanName);
                }

                /*
                 * default implementation looks for existing instance of the
                 * same type in
                 * event.getUI().getInternals().getActiveViewChain(). If not
                 * found viewType.newInstance() is returned.
                 */
            }
        });

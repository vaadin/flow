...
public class DependencyIT extends PhantomJSTest {
    ...
    public void htmlInjection() {
        ...
        Assert.assertEquals("Message received: HTML import loaded",
        ...
        // For now at least, loads JS before HTML.
        // Could be fixed if there are cases where HTML depends on JS imports
        Assert.assertEquals("Message received: JS File loaded",
                messages.get(messages.size() - 3));
        Assert.assertEquals("Message received: HTML import 2 loaded",
                messages.get(messages.size() - 2));
        Assert.assertEquals("Message received: HTML import 3 loaded",
                messages.get(messages.size() - 1));
    }

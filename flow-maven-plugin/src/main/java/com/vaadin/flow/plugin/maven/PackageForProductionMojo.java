...
/**
 * Goal that prepares all web files from {@link PackageForProductionMojo#transpileEs6SourceDirectory} for production mode: minifies,
 * transpiles and bundles them.
 */
@Mojo(name = "package-for-production", requiresDependencyResolution = ResolutionScope.COMPILE_PLUS_RUNTIME,
...
public class PackageForProductionMojo extends AbstractMojo {
  ...
  /**
   * Directory where the source files to use for transpilation are located. <b>Note!</b> This should match <code>copyOutputDirectory</code>
  ...
  /**
   * Target base directory where the transpilation output should be stored. The default is
  ...
  /**
   * If <code>false</code> the ES5 and ES6 code will not be minified. This will help in debugging if there are JS exception in runtime.
   */
  @Parameter(property = "minify", defaultValue = "true", required = true)
  private boolean minify;

  /**
   * If <code>false</code> then the bundle will not receive a hash for the content. This will make the bundle not update on content change
   * after it is cached in the browser.
   */
  ...
  /**
   * Defines the path to node executable to use. If specified, {@code nodeVersion} parameter is ignored.
   */
  @Parameter(name = "nodePath")
  private File nodePath;

  /**
   * Defines the node version to download and use, if {@code nodePath} is not set. The default is <code>v8.11.1</code>.
   */
  @Parameter(name = "nodeVersion", defaultValue = "v8.11.1")
  private String nodeVersion;

  /**
   * Defines the path to yarn executable to use. If specified, {@code yarnVersion} parameter is ignored.
   */
  @Parameter(name = "yarnPath")
  private File yarnPath;

  /**
   * Defines the yarn version to download and use, if {@code yarnVersion} is not set. The default is <code>v1.6.0</code>.
   */
  ...
  /**
   * Set the maven session which will be used for maven proxies if they are not ignored.
   */
  ...
  public void execute() {
    ...
    final FrontendDataProvider frontendDataProvider = new FrontendDataProvider(
    ...
    final FrontendToolsManager frontendToolsManager = new FrontendToolsManager(
  ...
  private ProxyConfig.Proxy createProxy(final Proxy proxy) {
  ...
  private Map<String, Set<String>> getFragmentsData(
      final List<Fragment> mavenFragments) {
  ...
  private void verifyFragment(final Fragment fragment) {
